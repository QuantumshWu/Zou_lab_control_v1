"""Installation-owned physical asset identity map.

The experiment config chooses a logical role.  It never invents a physical identity,
resource key, or adapter matcher: those facts live in this machine-level document and
are content-addressed as one immutable revision.
"""
...
class InstallationAssetMap:
    """Validated immutable AssetMap whose revision is its canonical content digest."""
    ...
    def ephemeral(
        ...
        """Build the deterministic in-memory installation used only by virtual/test rigs."""
