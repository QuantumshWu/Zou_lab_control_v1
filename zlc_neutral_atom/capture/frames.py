...
_FRAME_EVENT_RECORDS_PER_BLOB = 256
...
def _capture_frame_record_geometry(
    ...
    frames_per_chunk = _FRAME_RECORDS_PER_BLOB
...
def _decode_frame_event_chunk(
    *,
    ...
    start = (
        nonnegative_integer(chunk_index, "chunk_index")
        * _FRAME_EVENT_RECORDS_PER_BLOB
    )
    if start >= event_count:
        raise IndexError("capture frame-event chunk is outside the event domain")
    expected_count = min(_FRAME_EVENT_RECORDS_PER_BLOB, event_count - start)
    ...
    def admit_event_chunk(events) -> None:
        for event in events:
            ...
            if (
                ...
                and event.length > _FRAME_EVENT_RECORDS_PER_BLOB
            ):
                raise ValueError(
                    "capture frame-event blob differs from its canonical partition"
                )
    ...
    def __init__(
        ...
        *,
        ...
        expected_event_chunks = (
            event_count + _FRAME_EVENT_RECORDS_PER_BLOB - 1
        ) // _FRAME_EVENT_RECORDS_PER_BLOB
    ...
    def iter_cells(
        ...
        with self._root_lease.borrow() as read_borrow:
            ...
            for cell in cells:
                ...
                event_chunk = ordinal // _FRAME_EVENT_RECORDS_PER_BLOB
                ...
                persisted_cell, metadata = event_records[
                    ordinal % _FRAME_EVENT_RECORDS_PER_BLOB
                ]
...
def _frame_event_chunk_tree(
    ...
    if stop - start > _FRAME_EVENT_RECORDS_PER_BLOB:
        raise ValueError("frame-event blob differs from its canonical partition")
    return {
...
def _stage_capture_frame_source(
    *,
    ...
    for start in range(0, len(schedule), _FRAME_EVENT_RECORDS_PER_BLOB):
        stop = min(len(schedule), start + _FRAME_EVENT_RECORDS_PER_BLOB)
    ...
    *,
    ...
    def streamed_cells() -> Iterator[DatasetCellAddress]:
        for chunk_index, reference in enumerate(event_chunk_refs):
            ...
            start = chunk_index * _FRAME_EVENT_RECORDS_PER_BLOB
