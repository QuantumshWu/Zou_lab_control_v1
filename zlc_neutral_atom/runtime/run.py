...
class RunContext:
    ...
    def _detach_for_post_safety(self) -> "_PostSafetySeed":
        with self._hardware_condition:
            ...
            seed = _PostSafetySeed(handle, self.deadline)
            self._handle = None
            return seed
...
class PostSafetyContext:
    """Evidence that device access is revoked before domain finalization."""
...
class RunHandle(Generic[FinalT]):
    ...
    def _begin_finalize(self, deadline: float | None) -> None:
        """Atomically choose finalization over any later cancellation request."""

        with self._condition:
            if self._state.terminal:
                raise RuntimeError("finalize cannot run after terminal state")
            self._token.checkpoint()
            if deadline is not None and time.monotonic() >= deadline:
                raise TimeoutError(f"run {self.run_id} exceeded its monotonic deadline")
            if threading.current_thread() is not self._owner_thread:
                raise CapabilityRevoked("finalize belongs to the Run owner thread")
            if self._cancel_gate_closed:
                raise RuntimeError("Run is already finalizing")
    ...
    def _publish_terminal(
        ...
        *,
        ...
        with self._condition:
            ...
            if state is RunState.SUCCEEDED and self._token.is_cancelled:
...
class RunController:
    ...
    def _finish_before_execution(
        ...
        try:
            ...
            seed = _PostSafetySeed(handle, None)
    ...
    def _run_execution(
        ...
        try:
            ...
            if primary is None and handle._token.is_cancelled:
    ...
    def _finalize_and_publish(
        *,
        ...
        if primary_error is None and not cleanup_errors:
            try:
                handle._begin_finalize(seed.deadline)
