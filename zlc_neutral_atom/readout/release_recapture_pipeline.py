...
class ReleaseRecapturePairMetadata:
    ...
    def __post_init__(self) -> None:
        ...
        contract.validate(self.initial)
        contract.validate(self.recaptured)
...
class ReleaseRecaptureMetadataContract:
    source: CameraFrameMetadataContract = CameraFrameMetadataContract()
...
class ReleaseRecaptureSampleContract:
    ...
    metadata_contract: ReleaseRecaptureMetadataContract = (
        ReleaseRecaptureMetadataContract()
    )
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(
            ...
            raise TypeError("metadata_contract has another owner")
...
class ExactReleaseRecaptureTransaction:
    ...
    def _consume_one(self, context: RunContext) -> None:
        ...
        source = self.source_reservation._stream
        source._validate_consumer_delivery(
    ...
    def complete(self, context: RunContext) -> ReleaseRecapturePipelineResult:
        ...
        source = self.source_reservation._stream
        source._validate_consumer_completion(
            self.source_reservation,
            completion.eos,
            self,
        )
    ...
    def fail(self, error: BaseException) -> None:
        ...
        if self.source_reservation.state in (
            ...
            try:
                self.source_reservation._stream._abort_consumer(
                    self.source_reservation,
                    self,
                    lambda: None,
...
def open_exact_release_recapture(
    ...
    try:
        ...
        reducer = ExactReleaseRecaptureTransaction(
            ...
            readiness=None,
            ordered_inputs=OrderedEventSpanHasher(
                source_reservation._stream.stream_id,
                source_reservation._stream.generation,
                source_reservation.start_sequence,
            ),
        )
        readiness = source_reservation._stream._claim_consumer(
            source_reservation,
            reducer,
