...
from zlc_neutral_atom.readout.calibration_projection import CalibrationSiteMapContext
...
CALIBRATION_LIVE_OUTPUT_NAMES = ("frame",)
...
def write_calibration_task_outputs(
    ...
    *,
    ...
    """Write task pointers and an optional checked raw capture export.

    Capture and calibration repositories remain the only canonical artifact
    owners.  The task folder contains references plus an optional reproducible
    operator export admitted by :func:`admit_calibration_capture_export`.
    """
    ...
    calibration_artifact = calibration_repository.load(calibration)
    ...
    try:
        ...
        frames = root / "frames"
        if frames.exists() and (
            frames.is_symlink()
            or not frames.is_dir()
            or frames.resolve().parent != root
        ):
            raise ValueError("calibration frames output is not a task-owned directory")
        if not save_frames:
            # This directory is wholly owned by the optional raw export.  A new
            # task that declines the export must not silently inherit frames
            # from an older calibration run.
            if frames.exists():
            ...
            values_temp = frames / f".values.{nonce}.tmp.npy"
            validity_temp = frames / f".validity.{nonce}.tmp.npy"
            metadata_temp = frames / f".capture.{nonce}.tmp"
            temporary_paths.extend((values_temp, validity_temp, metadata_temp))
            np.save(values_temp, block.values, allow_pickle=False)
            np.save(validity_temp, validity, allow_pickle=False)
            ...
            # ``capture.json`` is the export's commit record.  All payloads are
            # completely written before any replace, and the record is replaced
            # last, so a concurrent/crash-time reader can only see an old valid
            # export, the new valid export, or a fail-closed mismatch.
            os.replace(values_temp, frames / "values.npy")
            os.replace(validity_temp, frames / "validity.npy")
            os.replace(metadata_temp, frames / "capture.json")
        # The task pointer is the whole application output's final commit record.
...
class CalibrationTaskLiveOutputPort(Protocol):
    """Attach the task-owned live preview without interpreting its event roles."""

    def open_calibration_preview(
        self,
...
class PreparedCalibrationTask:
    ...
    @property
    def live_output_names(self) -> tuple[str, ...]:
        return CALIBRATION_LIVE_OUTPUT_NAMES if self.has_live_output else ()
    ...
    def _start_capture(
        ...
        def attach(spec: CapturePreviewSpec) -> CapturePreviewPort:
            port = live_output.open_calibration_preview(
                spec,
                output_owner=self,
            )
            if port.spec != spec:
    ...
    def _write_outputs(
        ...
        self._dependencies.write_calibration_task_outputs(
            ...
            save_frames=(
                intent.save_frames if intent.source_mode == "live" else False
            ),
    ...
    def final_dataset_outputs(
        ...
        from .calibration_projection import calibration_final_outputs
    ...
    def site_map_context(
        ...
        reference = self._require_own_success(result)
        computation = self._dependencies.load_calibration_computation(reference)
        from .calibration_projection import calibration_site_map_context
