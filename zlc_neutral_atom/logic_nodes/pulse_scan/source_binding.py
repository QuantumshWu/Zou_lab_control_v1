...
from zlc_neutral_atom.node_input import BoundDatasetInput
...
_OCCUPANCY_SCAN_OUTPUT_CONTRACT_IDS = frozenset(
...
def _require_occupancy_output_name(value: str) -> str:
    ...
    if output_name not in exact_names:
        raise ValueError(
            "Occupancy scan output must be one of "
            f"{exact_names!r}"
        )
    return output_name
...
class OccupancyScanSource:
    ...
    A single-frame Camera capture makes its sole ``frame_0`` input unambiguous;
    ``output_name`` names the classified result and is intentionally limited
    to the two lossless exact outputs.  Reduced ``rate`` is not a formal scan
    source.
    """
...
def bind_scan_source(binding: BoundDatasetInput) -> ScanSourceBinding:
    """Interpret one framework Dataset binding as PulseScan's physical source."""

    if not isinstance(binding, BoundDatasetInput):
        raise TypeError("binding must be BoundDatasetInput")
    if binding.output.contract_id == CAMERA_FRAME_OUTPUT_CONTRACT_ID:
        request = binding.producer_request
        if not isinstance(request, CameraMeasurementRequest):
            raise TypeError(
                "the selected Camera producer has no CameraMeasurementRequest"
            )
        return DirectCameraScanSource(
            request,
            binding.output.name,
            binding.transform_spec,
        )

    if binding.output.contract_id in _OCCUPANCY_SCAN_OUTPUT_CONTRACT_IDS:
        request = binding.producer_request
        if not isinstance(request, OccupancyProcessorRequest):
            raise TypeError(
                "the selected Occupancy producer has no Processor request"
            )
        ...
        if request.camera_output_name != source.camera_output_name:
            raise ValueError(
    ...
    raise ValueError(
        "PulseScan Dataset input resolved an unaccepted output contract "
        f"{binding.output.contract_id!r}"
    )
