...
class PulseScanParameterForm(QtWidgets.QWidget):
    ...
    def __init__(
        ...
        *,
        ...
        self._signal_form = FluentParameterForm(
            FormSpec((spec.y_signal_field,)),
            runtime=runtime,
            parent=self,
        )
    ...
    @property
    def spec(self) -> PulseScanFormSpec:
        return self._spec
    ...
    def widget_for(self, key: str) -> QtWidgets.QWidget:
        ...
        if key == "pulse_slots":
            return self._slots
        if key == "y_signal":
            return self._signal_form.widget_for(key)
    ...
    def is_empty(self, key: str) -> bool:
        ...
        if key == "y_signal":
            return self._signal_form.is_empty(key)
        if key == "pulse_slots":
    ...
    def missing_required_labels(self) -> tuple[str, ...]:
        ...
        if self.is_empty("y_signal"):
            missing.append(self._spec.y_signal_field.label)
    ...
    def read_all(self) -> dict[str, object]:
        ...
        return {
            ...
            "y_signal": self._signal_form.read_value("y_signal"),
        }
    ...
    def populate(self, values: Mapping[str, object]) -> None:
        ...
        if signal is not None and not isinstance(signal, str):
            raise TypeError("PulseScan y_signal must be a signal key string")
        if not isinstance(slots, Mapping):
            raise TypeError("PulseScan pulse_slots must be a mapping")

        with signals_blocked(self, self._pulse_form, self._signal_form, self._slots):
            self._pulse_form.populate({"pulse": pulse})
            self._signal_form.populate({"y_signal": signal})
