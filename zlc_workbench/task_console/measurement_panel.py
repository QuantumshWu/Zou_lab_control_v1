...
class MeasurementPanel(QtWidgets.QWidget):
    ...
    def __init__(
        ...
        *,
        ...
        self._running = False
        self._parameter_form: FluentParameterForm | PulseScanParameterForm | None = None
    ...
    def _rebuild_form(self) -> None:
        ...
        if spec is None:
            return
        if isinstance(spec.form, PulseScanFormSpec):
            form = PulseScanParameterForm(
                spec.form,
                runtime=self._runtime(),
                pulse_template_reader=self._pulse_template_reader,
            )
        else:
            form = FluentParameterForm(spec.form, runtime=self._runtime())
        form.changed.connect(lambda *_: self._refresh_start_enabled())
