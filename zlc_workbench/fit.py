...
result.  For a committed Capture/Scan the opaque value is the neutral-owned
``FitExecution`` capability; for an already-frozen ``DataFigure`` it is the
exact ``FitResultBatch`` returned by ``BoundFit.run``.  The authority does not
know either persistence format and remains the one draft owner for both UI
entrances.
"""
...
class FitDraftAuthority:
    """Own exactly one unsaved opaque execution behind non-saving commands."""

    def __init__(
        self,
        execute_fit: Callable[
            [FitSpec, Callable[[], bool], float],
            tuple[object, FitResultBatch],
        ],
        save_fit: Callable[[object, object, object], object],
    ) -> None:
    ...
    def execute(
        ...
        admitted = execute_fit(spec, cancel_check, deadline_monotonic)
        if (
            not isinstance(admitted, tuple)
            or len(admitted) != 2
            or not isinstance(admitted[1], FitResultBatch)
        ):
            raise TypeError(
                "fit executor must return one opaque execution and FitResultBatch"
            )
        ...
        with self._lock:
            ...
            self._generation += 1
            draft = FitDraftResult(self._generation, result)
            self._draft = draft
    ...
    def save(
        ...
        try:
            if save_fit is None:
                raise FitCancelled("fit draft authority is closed")
            saved = save_fit(execution, destination, display)
            if saved is None:
                raise TypeError("fit saver returned no admitted save result")
    ...
    def _require_current_locked(self, draft: FitDraftResult) -> object:
        if not isinstance(draft, FitDraftResult):
            raise TypeError("draft must be FitDraftResult")
        if self._draft is not draft or self._execution is None:
