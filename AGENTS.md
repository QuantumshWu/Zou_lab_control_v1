...
- **能并行就并行,优先省用户的时间**:凡是能并行加速的工作——多文件搜索/审查、彼此独立的子任务、对抗式验证、多方案对比、跨子系统盘点——就**开多 agent / workflow 并行跑**,不要串行慢慢做。**不必顾虑 token 消耗**;首要目标是缩短用户的等待(wall-clock)。默认就用这种"铺开并行"的心态做事;只有真正琐碎或纯对话的一两步才单线程。(大规模 workflow 编排沿用既有 opt-in:ultracode 开着 / 用户明确要 / 技能触发时。)
