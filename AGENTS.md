...
- **对照用户的验收标准验证,不是对照自己的代理指标**(2026-06 反复栽在这):测试绿 / 像素一致 / 对象已挂上 ≠ 完成。每件事先问"**用户真正会怎么用它,我验证的是不是那个**"。具体落地:
  ...
  - **先确认用户指定的权威边界，不能把“可读参考”擅自升级成 production spec。** calibration/readout 的物理与算法唯一权威是用户指定的 `main` 已验证实现；`references/`、历史归档和外部脚本只能用于追溯或提供测试输入，不能反向定义 topology、feature、threshold、quality gate 或流程合同。若用户明确指定另一实现作为目标，才完整对照其流程形态（含文件 IO/编排），不得只挑算法移植。
