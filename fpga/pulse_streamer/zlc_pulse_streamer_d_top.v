`timescale 1ns / 1ps
// =============================================================================
// zlc_pulse_streamer_d_top -- board top for the Architecture-D edge-table pulse
// streamer (BRAM edge/scan tables + depth-1 prefetch engine), JTAG-to-AXI control.
//
// Reaches 2048 edges + thousands of scan points at <=75% of the 35T by keeping
// the edge + scan tables in BLOCK RAM (bus tables stay in LUTRAM inside the
// engine).  The host writes the program over JTAG-to-AXI; the engine reads the
// edge/scan BRAMs directly (no staging copy) via the depth-1 prefetch.
//
// Control path (all behind ONE proven axi_bram_ctrl, so the AXI handshakes are
// the vendor IP -- only a SIMPLE combinational write-address decoder is custom):
//   jtag_axi_0 -> axi_bram_ctrl_0 -> {bram_addr_a, bram_we_a, bram_wrdata_a,
//   bram_rddata_a} -> top decoder -> by word-address region:
//     R_CTRL  (regfile: scalars + COMMAND/STATUS mailbox + bus_counts)
//     R_EDGE  (edge BRAM port A, 32-bit; engine reads port B = packed edge row)
//     R_SCAN  (scan BRAM port A, 32-bit; engine reads port B = slot vector)
//     R_BUS   (bus-image BRAM port A; a mini-loader copies it into the engine's
//             bus LUTRAM via bus_prog_*)
// The engine's scalar inputs come straight from the regfile; LOAD copies the bus
// image, FIRE pulses start, STATUS is read back.  Geometry (addr widths, BRAM
// depths, region bases) is derived by edgetable_image.solve_capacity and written
// into the localparams below + the create-project tcl from the SAME source.
//
// *** First integration draft: structurally complete + contract-tested; the
// engine + control FSM are proven by Python models, but the multi-BRAM AXI
// integration needs on-board bring-up (no Verilog sim in repo). ***
// =============================================================================

module zlc_pulse_streamer_d_top #(
    parameter integer CHANNEL_COUNT = 62,
    parameter integer EDGE_ADDR_WIDTH = 11,     // 2048 edges
    parameter integer SCAN_ADDR_WIDTH = 12,     // 4096 scan points
    ...
    localparam integer EDGE_BITS = TICK_WIDTH + COEFF_BITS + CHANNEL_COUNT;     // 158
    localparam integer EDGE_PORTB_BITS = 256;          // asymmetric pad (pow2 x32)
    localparam integer EDGE_WORDS = EDGE_PORTB_BITS / 32;                       // 8
    localparam integer SCAN_PORTB_BITS = SLOT_BITS;    // 128 = 4x32, exact
    ...
    // --- word-address region bases (32-bit words) -----------------------------
    localparam integer R_CTRL_BASE = 0;
    localparam integer R_CTRL_WORDS = 64;
    localparam integer R_EDGE_BASE = R_CTRL_BASE + R_CTRL_WORDS;
    localparam integer R_SCAN_BASE = R_EDGE_BASE + MAX_EDGES * EDGE_WORDS;
    localparam integer R_BUS_BASE = R_SCAN_BASE + MAX_SCAN_POINTS * SCAN_WORDS;
    localparam integer R_TOTAL_WORDS = R_BUS_BASE + BUS_ROWS * BUS_WORDS;

    // CTRL regfile word offsets (host writes these; mirror axi-side host map).
    localparam integer C_COMMAND = 1;   // bit0 LOAD bit1 FIRE bit2 RESET bit3 SAFE
    localparam integer C_STATUS = 2;    // bit0 LOADED bit1 RUNNING bit2 DONE
    ...
    wire sel_ctrl = (word_addr >= R_CTRL_BASE) && (word_addr < R_EDGE_BASE);
    wire sel_edge = (word_addr >= R_EDGE_BASE) && (word_addr < R_SCAN_BASE);
    wire sel_scan = (word_addr >= R_SCAN_BASE) && (word_addr < R_BUS_BASE);
    wire sel_bus  = (word_addr >= R_BUS_BASE)  && (word_addr < R_TOTAL_WORDS);
    wire [29:0] edge_word_off = word_addr - R_EDGE_BASE[29:0];
    ...
    // loader-driven STATUS write-back into the regfile (separate from AXI writes)
    reg ldr_status_we;
    ...
    // --- read mux back to AXI (CTRL words + a tie-low elsewhere) ---------------
    always @(*) begin
        if (sel_ctrl) bram_douta = ctrl_reg[word_addr[5:0]];
        else bram_douta = 32'b0;
    end

    // --- EDGE BRAM (asymmetric: port A 32b write, port B EDGE_PORTB_BITS read) -
    wire [EDGE_PORTB_BITS-1:0] edge_doutb;
    wire [EDGE_ADDR_WIDTH-1:0] edge_raddr;
    blk_mem_gen_edge zlc_edge_bram_i (
        .clka(axi_clk), .ena(bram_ena && sel_edge), .wea(bram_wea),
        .addra(edge_word_off[($clog2(MAX_EDGES*EDGE_WORDS))-1:0]), .dina(bram_dina), .douta(),
        .clkb(axi_clk), .enb(1'b1), .web({(EDGE_PORTB_BITS/8){1'b0}}),
        .addrb(edge_raddr), .dinb({EDGE_PORTB_BITS{1'b0}}), .doutb(edge_doutb)
    );

    // --- SCAN BRAM (asymmetric: port A 32b write, port B 128b read) -----------
    wire [SCAN_PORTB_BITS-1:0] scan_doutb;
    wire [SCAN_ADDR_WIDTH-1:0] scan_raddr;
    blk_mem_gen_scan zlc_scan_bram_i (
        .clka(axi_clk), .ena(bram_ena && sel_scan), .wea(bram_wea),
        .addra(scan_word_off[($clog2(MAX_SCAN_POINTS*SCAN_WORDS))-1:0]), .dina(bram_dina), .douta(),
        .clkb(axi_clk), .enb(1'b1), .web({(SCAN_PORTB_BITS/8){1'b0}}),
        .addrb(scan_raddr), .dinb({SCAN_PORTB_BITS{1'b0}}), .doutb(scan_doutb)
    );
    ...
    // Reads COMMAND from the regfile; on LOAD copies the bus image (R_BUS region)
    // into the engine's bus LUTRAM via bus_prog_*, holds engine reset during load,
    // releases + pulses start on FIRE, writes STATUS back.  Edge/scan are NOT
    // copied (the engine reads those BRAMs directly).  Bus segments are walked
    // bus-major; each is 7 words = [start_tick, stop_tick, sc_lo, sc_hi, ec_lo,
    // ec_hi, flags] (edgetable_image layout).  Rising-edge-detected commands.
    localparam CMD_LOAD = 4'b0001, CMD_FIRE = 4'b0010, CMD_RESET = 4'b0100, CMD_SAFE = 4'b1000;
    localparam ST_LOADED = 4'b0001, ST_RUNNING = 4'b0010, ST_DONE = 4'b0100;
    ...
    reg [3:0] cmd_seen;
    integer ic;
    initial begin for (ic=0; ic<7; ic=ic+1) cap[ic]=32'b0; wi=0; bcur=0; baddr=0; bcnt=0; settle=0; cmd_seen=0; end
    ...
    // relative word base of bus segment (b,a) within the bus-image BRAM.
    ...
    always @(posedge clk) begin
        bus_prog_we <= 1'b0;        // default 1-cycle strobe handled by toggle below
        ldr_status_we <= 1'b0;
        ldr_cmd_clear <= 1'b0;
        eng_start <= 1'b0;
        case (lstate)
        ...
        L_EMIT: begin
            ...
            baddr <= baddr + 1'b1;
            settle <= 2'd2; lstate <= L_RUN;       // brief hold then next segment
        end
        L_RUN: begin
        ...
        endcase
        // surface DONE while running
        if ((ctrl_reg[C_STATUS] & ST_RUNNING) && zlc_done) begin
            ldr_status_we <= 1'b1; ldr_status_val <= {28'b0, (ST_RUNNING | ST_DONE)};
        end
    end

    // --- the Architecture-D engine --------------------------------------------
    zlc_pulse_streamer_d #(
        .CHANNEL_COUNT(CHANNEL_COUNT), .EDGE_ADDR_WIDTH(EDGE_ADDR_WIDTH),
        .TICK_WIDTH(TICK_WIDTH), .SCAN_ADDR_WIDTH(SCAN_ADDR_WIDTH),
        .NUM_SLOTS(NUM_SLOTS), .COEFF_WIDTH(COEFF_WIDTH), .COEFF_FRAC_BITS(COEFF_FRAC_BITS),
        ...
        .scan_count(ctrl_reg[C_SCAN_COUNT][SCAN_ADDR_WIDTH:0]),
        .edge_raddr(edge_raddr), .edge_rdata(edge_doutb[EDGE_BITS-1:0]),
        .scan_raddr(scan_raddr), .scan_rdata(scan_doutb),
