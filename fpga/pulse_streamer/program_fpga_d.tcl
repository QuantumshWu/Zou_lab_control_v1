# Program the Architecture-D bitstream and leave the JTAG-to-AXI master
...
set project_dir [env_or ZLC_PS_PROJECT_DIR [file join $project_root d]]
set top zlc_pulse_streamer_d_top
set default_bit_path [file join $project_dir d.runs impl_1 ${top}.bit]
set default_ltx_path [file join $project_dir d.runs impl_1 ${top}.ltx]
...
puts "ZLC program_fpga_d contract: CHANNEL_COUNT=62 NUM_SLOTS=4 control=JTAG-to-AXI (Architecture-D BRAM tables)"
puts "ZLC program_fpga_d project_dir: $project_dir"
puts "ZLC program_fpga_d bitstream: $bit_path"

if {![file exists $bit_path]} { error "Bitstream not found: $bit_path. Build it first (set ZLC_PS_VARIANT=d & build_and_program.bat --build-only)." }
...
if {[file exists $ltx_path]} {
    ...
    puts "ZLC program_fpga_d probes: $ltx_path"
} else {
    puts "ZLC program_fpga_d: no .ltx at $ltx_path (continuing; hw_axi may still auto-detect)."
}
