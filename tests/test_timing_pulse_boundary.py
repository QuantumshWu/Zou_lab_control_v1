...
Adjudicated 2026-07-21 (ledger row "timing 双源裁决").  ``pulse.py`` is the target
pipeline's execution-session protocol; ``pulse_table.py``/``sequence_model.py`` are the
authoring model + legacy compiler of the machine-verified production pipeline (C22).
Their zero cross-references are the boundary itself: an import in either direction
would silently marry the two pipelines before the authoring bridge exists, and a
THIRD pulse representation slipping into the package would recreate the very dual
source the adjudication resolved.  Hence three mechanical clamps:
...
3. the adjudication text itself (bridge + real-machine death condition) stays in the
...
#: The legacy production pipeline's side (authoring model + legacy compiler),
#: transitional per C25: dies with the authoring bridge + real-machine verification.
LEGACY_SIDE = {"pulse_table.py", "sequence_model.py", "ports.py", "serialization.py",
               "clock.py", "streamer_geometry.py"}
...
def test_the_package_roster_is_exact():
    ...
    expected = TARGET_SIDE | LEGACY_SIDE | {"__init__.py"}
...
def test_neither_pulse_side_imports_the_other():
    stems = {"target": {f[:-3] for f in TARGET_SIDE},
             "legacy": {f[:-3] for f in LEGACY_SIDE}}
    violations = []
    for side, other in (("target", "legacy"), ("legacy", "target")):
...
def test_the_adjudication_travels_with_the_package():
    ...
    for needle in ("pulse.py", "pulse_table.py", "bridge", "real-machine", "C22", "C25"):
