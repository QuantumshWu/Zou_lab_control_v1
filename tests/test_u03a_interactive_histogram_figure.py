...
def _typed_front(window):
    board = window.findChild(QtRasterBoard, "figureViewerHistogramBoard")
...
def test_generic_histogram_uses_typed_front_edits_and_exact_export(
    ...
    try:
        ...
        origin = board.visible_histogram_origin("generic-histogram")
        assert origin is not None
        window._accept_histogram_interaction(
        ...
        assert not board._numeric_bindings["generic-histogram"].interaction_ready
        ...
        assert board._numeric_bindings["generic-histogram"].applied_span == (
            2.5,
            12.5,
        )
        ...
        assert board._numeric_bindings["generic-histogram"].interaction_ready
...
def test_failed_histogram_rerender_preserves_exact_old_front(
    ...
    try:
        ...
        assert board.visible_histogram_payload("generic-histogram") is old_payload
...
def test_histogram_typed_budget_has_exact_derived_boundary(application) -> None:
    ...
    required = figure_module._histogram_front_required_peak_bytes(
        figure,
        HistogramDisplayState(),
    )
...
def test_frozen_data_figure_budget_cannot_be_widened_by_window(application) -> None:
    ...
    assert figure_module._histogram_front_required_peak_bytes(
        figure,
        HistogramDisplayState(),
    ) == required
    window = open_data_figure_workbench(
...
def test_histogram_front_request_and_authored_state_are_compare_and_swap(
    ...
    try:
        _until(application, lambda: window.worker_idle and window.raster_ready)
        board, old_frame, _old_payload = _typed_front(window)
        original_renderer = window._histogram_renderer
        ...
        window._histogram_renderer = wrong_sequence
        values = histogram_display_form_values(window._display)
        values["bin_count"] = 23
        window._apply_display_form(window._edit_display, 0, values)
        _until(application, lambda: window.worker_idle)
        ...
        window._histogram_renderer = wrong_authored_state
        window._apply_display_form(window._setting_display, 0, values)
        _until(application, lambda: window.worker_idle)
        assert board.front_frame is old_frame
        assert window._display.revision == 0
        ...
        window._histogram_renderer = wrong_provenance
        window._apply_display_form(window._edit_display, 0, values)
        _until(application, lambda: window.worker_idle)
        assert board.front_frame is old_frame
        assert window._display.revision == 0
        ...
        window._histogram_renderer = wrong_exact_series
        window._apply_display_form(window._setting_display, 0, values)
        _until(application, lambda: window.worker_idle)
        assert board.front_frame is old_frame
        assert window._display.revision == 0
        ...
        window._histogram_renderer = wrong_raster_geometry
        window._apply_display_form(window._edit_display, 0, values)
        _until(application, lambda: window.worker_idle)
        assert board.front_frame is old_frame
        assert window._display.revision == 0
