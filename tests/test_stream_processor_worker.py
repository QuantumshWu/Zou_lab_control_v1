...
# --------------------------------------------------------------------------- control topic
def test_control_topic_snapshots_before_accepting_and_owns_the_command_value():
    calls = []

    def snapshot(value):
        calls.append(tuple(value))
        if value == ["reject snapshot"]:
            raise ValueError("synthetic snapshot failure")
        return tuple(value)

    topic, consumer = create_control_topic(snapshot)
    source = [1, 2]
    receipt = topic.publish(source)
    source.append(3)

    with pytest.raises(ValueError, match="snapshot failure"):
        topic.publish(["reject snapshot"])
    assert receipt.snapshot() == ControlAck(1, ControlAckStatus.ACCEPTED)

    command = consumer.take_latest()
    assert command is not None
    assert command.revision == 1
    assert command.value == (1, 2)
    consumer.applied(command)
    assert receipt.wait(0.1) == ControlAck(1, ControlAckStatus.APPLIED)
    assert calls == [(1, 2), ("reject snapshot",)]


def test_control_topic_latest_pending_revision_wins_and_core_stays_bounded():
    topic, consumer = create_control_topic(int)
    first = topic.publish(1)
    second = topic.publish(2)
    third = topic.publish(3)

    assert first.wait(0.1) == ControlAck(
        1,
        ControlAckStatus.SUPERSEDED,
        superseded_by=2,
    )
    assert second.wait(0.1) == ControlAck(
        2,
        ControlAckStatus.SUPERSEDED,
        superseded_by=3,
    )
    assert third.snapshot().status is ControlAckStatus.ACCEPTED
    core = topic._core
    assert core._inflight is None
    assert core._pending is not None
    assert core._pending.command.revision == 3

    command = consumer.take_latest()
    assert command is not None and (command.revision, command.value) == (3, 3)
    assert core._pending is None and core._inflight is not None
    consumer.applied(command)
    assert third.wait(0.1).status is ControlAckStatus.APPLIED
    assert core._pending is None and core._inflight is None


def test_control_topic_inflight_must_finish_before_latest_pending_is_claimed():
    topic, consumer = create_control_topic(int)
    first = topic.publish(10)
    first_command = consumer.take_latest()
    assert first_command is not None

    second = topic.publish(20)
    third = topic.publish(30)
    assert first.snapshot().status is ControlAckStatus.ACCEPTED
    assert second.wait(0.1) == ControlAck(
        2,
        ControlAckStatus.SUPERSEDED,
        superseded_by=3,
    )
    assert consumer.take_latest() is None
    assert topic._core._inflight is not None
    assert topic._core._pending is not None

    consumer.applied(first_command)
    assert first.wait(0.1).status is ControlAckStatus.APPLIED
    third_command = consumer.take_latest()
    assert third_command is not None
    assert (third_command.revision, third_command.value) == (3, 30)
    consumer.applied(third_command)
    assert third.wait(0.1).status is ControlAckStatus.APPLIED


def test_control_topic_reject_is_terminal_but_invalid_reason_leaves_inflight_owned():
    topic, consumer = create_control_topic(dict)
    receipt = topic.publish({"roi": 4})
    command = consumer.take_latest()
    assert command is not None

    with pytest.raises(ValueError, match="reason"):
        consumer.rejected(command, "")
    assert receipt.snapshot().status is ControlAckStatus.ACCEPTED
    assert consumer.take_latest() is None

    consumer.rejected(command, "ROI is outside the frame")
    assert receipt.wait(0.1) == ControlAck(
        1,
        ControlAckStatus.REJECTED,
        reason="ROI is outside the frame",
    )


def test_control_topic_terminate_finishes_inflight_and_pending_then_seals_topic():
    snapshots = []
    ...
    topic, consumer = create_control_topic(snapshot)
    inflight_receipt = topic.publish("inflight")
    inflight = consumer.take_latest()
    assert inflight is not None
    pending_receipt = topic.publish("pending")

    consumer.terminate("camera run ended")
    expected_inflight = ControlAck(
        1,
        ControlAckStatus.TERMINATED,
        reason="camera run ended",
    )
    expected_pending = ControlAck(
        2,
        ControlAckStatus.TERMINATED,
        reason="camera run ended",
    )
    assert inflight_receipt.wait(0.1) == expected_inflight
    assert pending_receipt.wait(0.1) == expected_pending
    assert consumer.take_latest() is None
    assert topic._core._inflight is None and topic._core._pending is None

    with pytest.raises(RuntimeError, match="already terminated"):
        consumer.terminate("again")
    with pytest.raises(RuntimeError, match="no in-flight"):
        consumer.applied(inflight)
    with pytest.raises(RuntimeError, match="terminated"):
        topic.publish("after termination")
    assert snapshots == ["inflight", "pending", "after termination"]


def test_control_receipt_wait_times_out_without_consuming_the_terminal_transition():
    topic, consumer = create_control_topic(int)
    receipt = topic.publish(7)
    with pytest.raises(TimeoutError, match="has not reached terminal state"):
        receipt.wait(0.0)
    assert receipt.snapshot().status is ControlAckStatus.ACCEPTED
    ...
    assert command is not None
    consumer.applied(command)
    assert receipt.wait(0.0).status is ControlAckStatus.APPLIED


def test_control_topic_invalid_identity_and_duplicate_terminal_fail_closed():
    topic, consumer = create_control_topic(int)
    receipt = topic.publish(1)
    command = consumer.take_latest()
    assert command is not None

    other_topic, other_consumer = create_control_topic(int)
    other_receipt = other_topic.publish(2)
    other_command = other_consumer.take_latest()
    assert other_command is not None
    with pytest.raises(TypeError, match="requires ControlCommand"):
        consumer.applied(object())  # type: ignore[arg-type]
    with pytest.raises(PermissionError, match="different topic"):
        consumer.applied(other_command)
    assert receipt.snapshot().status is ControlAckStatus.ACCEPTED

    consumer.applied(command)
    with pytest.raises(RuntimeError, match="no in-flight"):
        consumer.applied(command)
    assert receipt.wait(0.1).status is ControlAckStatus.APPLIED

    other_consumer.applied(other_command)
    assert other_receipt.wait(0.1).status is ControlAckStatus.APPLIED


def test_control_ack_validation_is_status_exact():
    assert not ControlAck(1, ControlAckStatus.ACCEPTED).terminal
    assert ControlAck(1, ControlAckStatus.APPLIED).terminal
    with pytest.raises(TypeError, match="ControlAckStatus"):
        ControlAck(1, "APPLIED")  # type: ignore[arg-type]
    with pytest.raises(ValueError, match="cannot contain a reason"):
        ControlAck(1, ControlAckStatus.APPLIED, reason="unexpected")
    with pytest.raises(ValueError, match="reason"):
        ControlAck(1, ControlAckStatus.REJECTED)
    with pytest.raises(ValueError, match="superseding"):
        ControlAck(2, ControlAckStatus.SUPERSEDED, superseded_by=2)
    with pytest.raises(ValueError, match="superseded_by"):
        ControlAck(1, ControlAckStatus.TERMINATED, reason="done", superseded_by=2)
