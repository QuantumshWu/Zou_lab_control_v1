...
def test_plot_kinds_and_display_forms_share_one_typed_frontend_contract() -> None:
    ...
    expected = (
        (PlotKind.IMAGE, "2d", "2D image", True),
        (PlotKind.SITE_MAP, "sites", "Site map", True),
        (PlotKind.CURVE, "1d", "1D vector", True),
        (PlotKind.METER, "meter", "Meter", False),
        (PlotKind.ROLLING, "monitor", "Rolling trace", True),
        (PlotKind.HISTOGRAM, "hist", "Distribution", True),
        (PlotKind.PULSE, "pulse", "Pulse sequence", False),
        (PlotKind.GRID, "grid", "Site grid", True),
    )
    assert tuple(
        (kind, kind.value, kind.label, kind.panel) for kind in PlotKind
    ) == expected
    assert tuple(kind for kind in PlotKind if not kind.input_format) == (
        PlotKind.PULSE,
    )
    assert "SiteMapPresentation" in PlotKind.SITE_MAP.input_format
    assert "calibration" not in PlotKind.SITE_MAP.input_format.lower()
    assert "occupancy" not in PlotKind.SITE_MAP.input_format.lower()

    image = panel_display_form_spec(PlotKind.IMAGE)
    site_map = panel_display_form_spec(PlotKind.SITE_MAP)
    rolling = panel_display_form_spec(PlotKind.ROLLING)
    histogram = panel_display_form_spec(PlotKind.HISTOGRAM)
    assert image is site_map
    assert panel_display_form_spec(PlotKind.METER) is None
    assert panel_display_param_keys(PlotKind.METER) == frozenset()
    assert image is not None and "colormap" in image.keys
    assert rolling is not None
    assert {field.key: field.kind for field in rolling.fields}["show_dist"] == "bool"
    assert histogram is not None
    assert {
        field.key: field.kind for field in histogram.fields
    }["log_count_axis"] == "bool"
    assert panel_display_form_spec(
        PlotKind.GRID,
        cell_intent=ViewIntent.HISTOGRAM,
    ) is histogram
