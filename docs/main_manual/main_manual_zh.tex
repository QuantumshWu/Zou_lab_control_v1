...
\begin{document}
\frontmatter
\zlctitlepage{Zou\_lab\_control 主手册}{notebook-first + PyQt GUI + remote FPGA pulse-streamer 系统总览}{架构、session/devices/timing 分层、sequencer 生命周期、真实硬件 runbook 与 N-slot 扫描模型}{2026-06-07}
\tableofcontents
\cleardoublepage
\mainmatter
...
默认工程在 \filepath{fpga\build\ps}（短名 \pyapi{ps} 让 Vivado 深层 run/.Xil 临时路径不超过 Windows MAX\_PATH，构建仍留在仓库内），bit/ltx（\filepath{zlc_pulse_streamer_top.bit/.ltx}）在其 \filepath{ps.runs\textbackslash impl_1} 目录下。默认 XDC 是 address-switch 板的引脚图（\filepath{addre.xdc}）。相机成像预设的关键物理输出：
