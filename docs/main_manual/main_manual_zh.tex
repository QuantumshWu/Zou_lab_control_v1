...
\begin{document}
\frontmatter
\zlctitlepage{Zou\_lab\_control 主手册}{notebook-first + PyQt GUI + remote FPGA pulse-streamer 系统总览}{架构、session/devices/timing 分层、sequencer 生命周期、真实硬件 runbook 与 N-slot 扫描模型}{2026-06-05}
\tableofcontents
\cleardoublepage
\mainmatter
...
\begin{description}
  ...
  \item[fpga] pulse-streamer 硬件：Artix-7 35T 上的 RTL、N-slot 仿射扫描引擎、VIO probe 契约、模拟总线 DAC 引擎、上传流程与资源预算。见 \filepath{docs/fpga_manual/}。
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[hw, below=10mm of server] (vivado) {Vivado/VIO 会话\\persistent Tcl};
    \node[hw, below=10mm of remote] (rtl) {zlc\_pulse\_streamer\\FPGA 时钟逻辑};
...
\begin{description}
  \item[\pyapi{devices/}] 硬件适配器与设备契约。设备只负责硬件动作。包含 \filepath{fpga_pulse_streamer.py}（HDL 生成 + Vivado/VIO 上传）、\filepath{sequencer.py}、\filepath{sequencer_server.py}。
...
控制电脑通过 \pyapi{RemoteSequencer} 调用四个动作。它们经 RPyC 到达 FPGA 电脑上的 \pyapi{SequencerService}，再到 \pyapi{VivadoPulseStreamerSession}：
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[stage] (prep) {\textbf{prepare}\\拉高 reset\\逐行写 ticks/masks/\\coeffs/scan/bus\\释放 reset};
    \node[stage, right=8mm of prep] (fire) {\textbf{fire}\\发一个 start\\低-高-低脉冲};
    \node[stage, right=8mm of fire] (wait) {\textbf{wait\_done}\\轮询 done};
...
\begin{description}
  \item[\pyapi{prepare(program)}] 拉高 reset（此时输出寄存器强制为 0，runtime 计数器空闲，写侧使能），通过 VIO 逐行上传边沿表、重复元数据、扫描点和总线段，然后释放 reset。\tfocus{prepare 不启动脉冲}，它只是让 FPGA 持有一张已校验的表。
  \item[\pyapi{fire()}] 发一个显式的 \pyapi{start} 低-高-低脉冲。FPGA 只把同步后的上升沿当作 start。这之后，微秒级时序由 FPGA 时钟拥有；Python、RPyC、Windows 调度、Vivado、JTAG 延迟都不再决定 shot 内的边沿。
  \item[\pyapi{wait_done()}] 轮询 \pyapi{done}，等待有限序列跑完。
  \item[\pyapi{safe_state()}] 复位 sequencer，把输出拉回安全态（\pyapi{Stop Pulse} 走这条路）。
\end{description}

\section{Vivado 持久会话与差分上传}
\pyapi{vivado-session} 后端只启动一次 Vivado Tcl 进程，打开 hardware target，加载一次 \pyapi{.ltx} probe，然后整个 server 生命周期内复用这个会话来做 prepare/fire/wait\_done/safe\_state。慢的 hardware-target 建立发生在 server 启动时，而不是第一次 \pyapi{On Pulse}。

第一次成功 prepare 之后，只要 channel order 和 clock 不变，后续 prepare 可以\tfocus{差分上传}：只重写改变的边沿行、改变的扫描点，以及 shadow-critical 行（第 0 行、loop start 行、最后一行）。
...
\begin{codeblock}[PowerShell]
cd D:\ZLC
.\fpga\build_and_program.bat --check    # 不连板的 HDL/VIO 宽度 + 容量自查
.\fpga\build_and_program.bat            # build + program
\end{codeblock}

默认工程在 \filepath{fpga\build\address_switch}，bit/ltx 在其 \filepath{impl_1} 目录下。默认 XDC 是历史 address-switch 引脚图。相机成像预设的关键物理输出：
...
\begin{itemize}
  ...
  \item 一条 channel 的延时（\pyapi{kind="delay"}）；
...
\section{主机如何编译扫描}
绑定过的 duration/delay 是\tfocus{时间 slot}，它们参与仿射 tick 公式。主机把每行边沿编译成一个 base tick 加 \pyapi{NUM_SLOTS} 个定点系数，FPGA 在运行时计算：
...
\section{端到端的可运行示例}
下面扫描 period 1 的持续时间（slot \pyapi{s0}）和 probe 延时（slot \pyapi{s1}）：
...
# 绑定两个字段 -> s0, s1（bind_field 是幂等的，返回 slot 序号）
s0 = state.bind_field("duration", "0")        # period 0 的持续时间
s1 = state.bind_field("delay", "probe")       # probe channel 的延时

# 构造 5 个扫描点的 (N_points x 2) 表，单位 ns
...
# 11 个点，原始 10-bit DAC code（0..1023）。
...
\begin{notebox}[组合规则]
DAC 扫描可与\tfocus{延时}扫描同用；不能与\tfocus{持续时间}扫描同用（编译器报错）。被扫电平要用 \pyapi{edge} 模式保持，不要用 \pyapi{ramp} 端点去扫。
\end{notebox}
...
\item \tfocus{硬件}：相机怎么曝光、FPGA 怎么收脉冲表、VIO 探针怎么写。这是有副作用、会失败、需要连接的"动作"。
...
\item[真实 qCMOS + 远程 FPGA（RemoteSequencer / RPyC）] 真实相机在本地，脉冲流送给\tfocus{远程} FPGA：本地的 \pyapi{RemoteSequencer} 通过 RPyC 调到 \pyapi{sequencer_server.py} 跑的服务（\pyapi{run_server} / \pyapi{CommandSequencerBackend}），由它去操作 Vivado/VIO。这是真机实验的典型部署。
\end{description}
...
\item[scan\_slots / scan\_table] 扫描槽（有序，每个绑定到一个 duration/delay/dac 字段）和扫描表（N\_points × N\_slots）。表达式里用 s0..s\{N-1\} 引用这些槽。
\end{description}

核心方法（都在 \filepath{Zou\_lab\_control/neutral\_atom/timing/pulse\_table.py}）：

\begin{description}
\item[\pyapi{bind_field(kind, target, *, label, unit, nominal)}] 把一个字段绑定到\tfocus{新的扫描槽}，并把该字段原地改写成 \pyapi{s\{i\}}（i 是新槽号），返回槽号。kind ∈ \{duration, delay, dac\}：duration 的 target 是周期下标，delay 的 target 是通道名，dac 的 target 指向某条总线的某个周期。\tfocus{幂等}：重复绑定同一个已绑定字段，直接返回它已有的槽号。绑定时会给 scan\_table 每行追加该字段的 nominal 标称值。
...
FPGA/Vivado 计算机
  SequencerService           <- 有状态服务, 持有 prepared_program + state
      |  callback
      v
  VivadoPulseStreamerSession <- 常驻 Vivado Tcl 进程, 持有 VIO 探针 + 已上传程序
      |  VIO / JTAG
      v
  zlc_pulse_streamer (FPGA)  <- 真正掌管纳秒时序的硬件
\end{codeblock}

\begin{description}
\item[控制计算机] 跑实验脚本/笔记本。它只有"意图"：一份脉冲表（\pyapi{PulseSequence} 或 \pyapi{PulseTableState}）和扫描表。它不直接碰硬件。
\item[FPGA/Vivado 计算机] 接着 FPGA 的那台机器。它跑 RPyC 服务，背后挂一个\tfocus{常驻}的 Vivado Tcl 进程，通过 VIO（Virtual Input/Output）探针读写 FPGA 内部寄存器。
\end{description}
...
\item[\pyapi{VivadoPulseStreamerSession}（硬件传输层）] 持有 \pyapi{self.\_process}（常驻 Vivado 子进程）、读取线程、以及最关键的 \pyapi{self.\_uploaded\_program}——\tfocus{上一次真正写进 FPGA 的程序}。这个字段是差分上传的全部依据。
\item[FPGA（\pyapi{zlc_pulse_streamer}）] 持有边沿表（abs ticks + 62 位掩码）、repeat 元数据、scan-point 表、bus 段。一旦 reset 释放且收到 start 上升沿，\tfocus{枪内的纳秒时序就完全归 FPGA 时钟所有}。
\end{description}

\begin{notebox}[一条铁律：纳秒时序的所有权会"交接"]
prepare 阶段，时序由软件主导（Tcl 一行行写 VIO，每行多慢都无所谓，因为 reset 拉高、输出被钳在 0）。fire 给出 start 上升沿那一刻，所有权\tfocus{交接}给 FPGA 时钟。此后 Python 的 GIL、RPyC 往返、Windows 调度抖动、Vivado 的延迟、JTAG 的延迟，\tfocus{都不再影响一枪之内的边沿时刻}。这就是为什么"在 Python 里 sleep 然后翻通道"绝对不可接受，而"把整张边沿表交给 FPGA 自己跑"可以做到纳秒确定。
\end{notebox}
...
\begin{enumerate}
\item \tfocus{拉高 reset}（\pyapi{zlc_reset=1}）。复位期间所有输出被强制为 0，同时打开"写入侧"——此时往里写边沿表是安全的，FPGA 不会输出任何脉冲。同一批里还把 \pyapi{start} 预置为 0、写入 repeat\_forever/loop\_start/loop\_end/scan\_enable/各表行数等元数据，然后 \pyapi{after} 等一个可配置的 settle（\pyapi{ZLC_PS_PREPARE_RESET_SETTLE_MS}，默认 5ms）。
\item \tfocus{逐行写边沿表}。每写一行就翻转一次 \pyapi{prog_we}（program write-enable）：先 stage 地址/tick/掩码/slot 系数，再把 \pyapi{prog_we} 翻到新值，\pyapi{zlc_commit_probes} 一次性提交。\pyapi{prog_we} 的"边沿"（翻转）就是 FPGA 的写入脉冲。scan-point 表、bus 段同理，各自有 \pyapi{scan_prog_we}、\pyapi{bus_prog_we}。
\item \tfocus{释放 reset}（在 fire 阶段的脚本里）。准备完成后 FPGA 处于"装好弹、保险还在"的状态。
\item 校验：\pyapi{validate_pulse_streamer_program} 先检查边沿数、scan 点数、tick 位宽、通道数不越界，再上传。越界会在写任何 VIO 之前就报错。
\end{enumerate}

\tfocus{prepare 绝不启动序列}。它返回后 FPGA 静止，输出维持安全电平。这一点对相机实验至关重要：你可以先 prepare、再武装相机、最后才 fire，三步之间想隔多久都行。

服务端的 \pyapi{SequencerService.prepare} 还做了一层缓存：如果新程序的 \pyapi{sequence_id} 和已准备的完全相同，就跳过 \pyapi{prepare_callback}（即跳过整段 VIO 写入），直接复用。是否启用由 \pyapi{ZLC_SEQUENCER_CACHE_PREPARED} 控制。
...
ok = seq.wait_done(timeout=2.0)   # 有限序列: 轮询 done 探针
assert ok, "序列在 2 秒内没有报告 done"
\end{codeblock}

\section{wait\_done：只对有限序列轮询}

\pyapi{wait_done(timeout)} 轮询 done 探针，等一个\tfocus{有限}序列跑完。它有一条硬护栏：
...
\pyapi{VivadoPulseStreamerSession} 是把"慢"集中处理的核心。它在 \pyapi{start()} 时启动\tfocus{一个} Vivado Tcl 子进程（\pyapi{vivado -mode tcl -nolog -nojournal}），然后跑一段公共初始化 Tcl：

\begin{enumerate}
\item 打开硬件目标（\pyapi{open_hw_target}，失败会重试 \pyapi{-jtag_mode on}）。
\item 加载 \filepath{.ltx} 探针文件\tfocus{一次}——这是把 VIO 内部信号名（如 \pyapi{zlc_reset}、\pyapi{zlc_prog_we}）映射到 \pyapi{probe_out\#} 的关键。找不到 VIO 核会明确报错，提示当前 FPGA 镜像不是 ZLC pulse-streamer 比特流，或 \filepath{.ltx} 不匹配。
\item 之后整段服务生命周期里\tfocus{复用}这同一个进程、同一个已打开的硬件目标、同一组探针。
\end{enumerate}

所以"慢的硬件目标 bring-up 发生在服务器启动时，而不是第一次 On Pulse 时"。每个后续动作（prepare/fire/wait/safe）只是往这个活着的进程喂一段带唯一 marker 的 Tcl，读到 marker 即返回。

\begin{notebox}[启动服务器：warm\_start 让你在第一枪之前就付清启动代价]
\pyapi{run_server(backend="vivado-session", warm_start=True)} 会在接受任何客户端连接之前就 \pyapi{hardware_backend.start()}，把 Vivado 启动 + 打开硬件目标 + 加载探针的几十秒一次性付清。等客户端连上来，第一次 prepare 就已经是热的。
\end{notebox}

\begin{codeblock}[PowerShell]
# 在 FPGA/Vivado 计算机上, 启动常驻服务 (PowerShell)
$env:ZLC_PS_VIVADO_BIN = "C:\Xilinx\Vivado\2025.1\bin\vivado.bat"
$env:ZLC_PS_VIVADO_LTX = "C:\zlc\zlc_pulse_streamer_top_address_switch.ltx"
python -m Zou_lab_control.neutral_atom.devices.sequencer_server `
    --backend vivado-session --host 0.0.0.0 --port 18861 `
    --channels cooling repump imaging mot_coil
# 打印 "Starting persistent Vivado session before accepting clients..."
# 慢启动在这里发生; 之后客户端的 prepare/fire 都是热的
\end{codeblock}

\section{差分上传：第二枪只重写改动的行}

每次 prepare 都把整张表重写一遍是浪费的——一次扫描里相邻两点往往只差几行。\pyapi{VivadoPulseStreamerSession} 用 \pyapi{self.\_uploaded\_program} 记住上次写进 FPGA 的程序，下次 prepare 时算\tfocus{差分}。

差分成立的前提（任一不满足就退回全量重写）：

\begin{itemize}
\item 上一次 prepare 成功过（\pyapi{\_uploaded\_program} 非空）；
\item \tfocus{通道顺序不变}（\pyapi{channels} 列表相同）；
\item \tfocus{时钟不变}（\pyapi{clock_hz} 相同）。
\end{itemize}

满足后，要重写的边沿行集合是"改动行 $\cup$ 影子关键行"：

\begin{description}
\item[改动行] tick、62 位掩码、或该行的 slot 系数列与上次不同的行（含新增行）。
\item[影子关键行] 固定包含三行：\tfocus{第 0 行}、\tfocus{loop-start 行}、\tfocus{最后一行}。即使它们内容没变也强制重写，因为这几行对边界/循环/影子寄存器的语义最敏感，重写它们能避免边界状态不同步。
\end{description}

scan-point 表同理走 \pyapi{\_prepare_scan_indices}：点数相同则只重写"逐点比对后不同"的扫描点，点数变了就全量。bus 段也是逐段比对、只重写改动段。

\begin{codeblock}[Python]
# 差分上传的判定 (fpga_pulse_streamer.py, _prepare_indices)
if previous_program is None:                 return full      # 第一次: 全量
if list(previous.channels) != list(now.channels): return full # 通道变: 全量
if float(previous.clock_hz) != float(now.clock_hz): return full # 时钟变: 全量
changed = { i for i,(t,m) in enumerate(zip(now.ticks, now.masks))
            if t/m/coeffs 与 previous 第 i 行不同 }
critical = {0, loop_start_index, last_index}  # 影子关键行恒重写
return sorted(changed | critical)
\end{codeblock}

实际写入时，每个待写行都翻一次 \pyapi{prog_we}：脚本先读出当前 \pyapi{prog_we} 电平，第一行 \pyapi{toggle}，提交；后续行继续逐行 toggle、提交。掩码/tick/coeffs 都先 stage 再随 \pyapi{prog_we} 一起 commit，保证 FPGA 看到 write-enable 翻转时数据已就位。

\begin{notebox}[差分上传让扫描点之间的 prepare 几乎免费]
一次 1000 点的硬件扫描，若点间只改一两行，第二点起的 prepare 只写"改动行 + 三行影子"，VIO 往返从上千行降到个位数行。这是"扫描点在硬件里无缝迭代"之外，主机侧也快的原因。
\end{notebox}
...
\begin{enumerate}
\item \tfocus{FPGA 计算机}：用 \filepath{create_project_address_switch.tcl} 生成工程、编译并烧录 \filepath{zlc_pulse_streamer_top_address_switch.bit} 与配套 \filepath{.ltx}（VIO 探针文件必须与比特流匹配，否则会报"找不到 VIO 核"）。
\item \tfocus{FPGA 计算机}：设置 \pyapi{ZLC_PS_VIVADO_BIN} 与 \pyapi{ZLC_PS_VIVADO_LTX}，\pyapi{run_server(backend="vivado-session", warm_start=True, host="0.0.0.0", port=18861, channels=...)}。等它打印完常驻会话启动信息——慢启动在此付清。
...
本章把“扫描”这件事从头讲透。读完之后，一个新人应该能在 Notebook 里绑定任意字段、装入扫描表、编译成一个 FPGA 程序，并理解为什么一个一百万点的扫描\tfocus{不会}把边沿表撑大、为什么 DAC 值/周期长度/通道延时可以\tfocus{在一次连续运行里同时扫描}。
...
\begin{description}
\item[kind（种类）] 它绑定的是哪一类字段。只有三种合法值：\pyapi{"duration"}（某个周期的时长）、\pyapi{"delay"}（某个通道的延时）、\pyapi{"dac"}（某个周期里、某条模拟总线上的 DAC 输出值）。
\item[target（目标）] 在该种类里具体指哪一个字段。对 \pyapi{duration} 是周期下标（字符串化的整数，如 \pyapi{"1"}）；对 \pyapi{delay} 是通道名（如 \pyapi{"cooling"}）；对 \pyapi{dac} 是 \pyapi{"<bus>@<period_index>"} 形式，例如 \pyapi{"da_dipole@2"}（第 2 个周期、\pyapi{da\_dipole} 总线上的值）。
...
\item 绑定后会把该字段改写成符号：\pyapi{duration} → 周期时长变成 \pyapi{"s0"}；\pyapi{delay} → 该通道延时变成 \pyapi{"s0"}；\pyapi{dac} → 该周期该总线段变成 \pyapi{\{"mode": "edge", "value": "s0"\}}。
...
# 2) 扫 cooling 通道的延时（按 s1）
s1 = state.bind_field("delay", "cooling", unit="ns")     # 返回 1

# 3) 扫第 2 个周期里 da_dipole 总线上的 DAC 值（按 s2）
s2 = state.bind_field("dac", "da_dipole@2")              # 返回 2

print(state.scan_slots)      # 三个 ScanSlot，kind 分别是 duration/delay/dac
\end{codeblock}
...
\pyapi{state.unbind\_slot(index, restore=...)} 删除一个 slot。因为\tfocus{编号即变量名}，删除中间 slot 会把它后面的所有 slot 往前移：原来的 \pyapi{s2} 变成 \pyapi{s1}，依此类推，所有引用它们的字段会被自动重写到新编号（\pyapi{\_renumber\_slot\_bindings}）。\pyapi{restore} 给出解绑后字段恢复成的字面值（不给则时长回到 1000 ns、延时回到 0、DAC 回到 hold）。
...
\item \tfocus{列 j} = slot \pyapi{s\{j\}} 在该点的取值，\tfocus{用该 slot 的显示单位}：时间类 slot 用 ns（或绑定时的单位，内部按 \pyapi{UNITS\_TO\_NS} 折算成 ns），\pyapi{dac} 类用\tfocus{原始 DAC 码}（0..1023，不是电压）。
\end{itemize}
...
# 三列对应 s0(us)、s1(ns)、s2(DAC 码)
table = np.array([
    [10.0,   0.0,   0],     # 点 0：周期=10us，延时=0ns，DAC=0
    [20.0,  50.0, 256],     # 点 1
    [30.0, 100.0, 768],     # 点 2
    [40.0, 150.0, 1023],    # 点 3：DAC 满量程
])
...
时间列写你看到的数（ns/us/ms/s 取决于该 slot 的 unit）；DAC 列写\tfocus{裸 10 位码 0..1023}。1023 是满量程，不是“1.023 V”。电压到码的换算由你在写表前自己完成。
\end{notebox}
...
print(program.slot_count)            # 3
print(program.slot_kinds)            # ['duration', 'delay', 'dac']
print(program.scan_coeff_frac_bits)  # 8
print(len(program.scan_points))      # 4（点数）
print(program.tick_slot_coeffs)      # 每条边沿 × 3 个系数
\end{codeblock}

FPGA 拿到这个程序后，\tfocus{无缝地}遍历所有扫描点：每点对应模板的一次实例化，每点发\tfocus{一次相机触发}，点与点之间不停机。这就是“硬件无缝扫描”。

\section{DAC 值 + 时长 + 延时：一次连续运行同时扫三者}

这是相对早期版本最大的突破。早期 DAC 值的扫描和周期时长的扫描是互斥的（怕段的 tick 不跟着时长平移而失步）。现在不互斥了：\tfocus{DAC 值、周期时长、通道延时可以在同一次无缝运行里同时扫描}。
...
绑定 DAC 用 \pyapi{target = "<bus>@<period_index>"}，扫描表对应列写\tfocus{裸 10 位 DAC 码（0..1023）}。下面是一个把\tfocus{三种 kind 同时绑定}的可运行端到端例子：
...
s1 = state.bind_field("delay", "cooling", unit="ns")# s1: cooling 延时(ns)
s2 = state.bind_field("dac", "da_dipole@2")         # s2: 周期2 上 da_dipole 的 DAC 值

# --- 扫描表：4 个点，列顺序 = s0(us), s1(ns), s2(DAC码) ---
...
assert program.slot_count == 3
assert program.slot_kinds == ["duration", "delay", "dac"]
assert len(program.scan_points) == 4
# DAC 段带 value_select 指向 s2；该段 tick 随 s0 平移（仿射系数非零）
\end{codeblock}

运行这个程序时，FPGA 对四个点依次实例化模板：每点周期 1 的时长、cooling 的延时、周期 2 上 \pyapi{da\_dipole} 的输出码都按该行取值，DAC 段的位置随时长平移，每点打一次相机触发——全程不停机。
...
\item RTL（\filepath{fpga/pulse\_streamer/zlc\_pulse\_streamer.v}）按 \pyapi{effective\_tick} 公式实时实例化模板，无缝遍历 \pyapi{scan\_points}，每点一相机触发。
\end{itemize}
\end{notebox}
...
\begin{description}
\item[时间表达式必须仿射] 所有时长/延时表达式必须是 slot 的一次线性组合（\pyapi{s0 + 2*s1} 可以，\pyapi{s0*s1} 或 \pyapi{s0**2} 不行）。非仿射的时序会被拒绝。
...
\item[DAC 列是码不是电压] 再说一遍：\pyapi{dac} slot 的扫描表列是裸 10 位码 0..1023。
\end{description}
...
\item[FPGA/Vivado 计算机] 物理上连着 Artix-7（xc7a35tfgg484-2）开发板和 Vivado。这台机器跑两件事：一次性的\tfocus{编译+烧写}（\filepath{fpga\textbackslash build\_and\_program.bat}），以及常驻的\tfocus{脉冲流服务器}（\filepath{fpga\textbackslash run\_server.bat}）。服务器内部持有一个常驻 Vivado Tcl 会话，通过 VIO 探针把脉冲表写进 FPGA。
\end{description}
...
\item \tfocus{Server / Vivado 会话层}（FPGA 机）：\filepath{run\_server.bat} 起的进程持有一个常驻 Vivado Tcl 会话（vivado-session 后端），是真正会写 VIO 探针、改 FPGA 内存的人。它拥有"已经烧进去的 bit 文件"和"当前 VIO 探针状态"。
\item \tfocus{硬件层}：FPGA 里跑的 \filepath{zlc\_pulse\_streamer.v}（边沿表 + 仿射扫描引擎 + 模拟总线），以及 62 路物理输出。它拥有"正在跑的脉冲"和真实的电平。
\end{enumerate}

\begin{notebox}[一句话记忆]
Notebook 说"我要打这个序列"，Session 把它编译成边沿表/扫描表通过 RPyC 发给 FPGA 机的 Vivado 会话，Vivado 会话用 VIO 把它写进 FPGA，FPGA 自己按时钟把电平打出来。相机的外部触发也是这 62 路里的一根线。
\end{notebox}

\begin{notebox}[路径长度：一定要短]
把整个仓库放在\tfocus{很短的路径}下，例如 \filepath{D:\textbackslash ZLC}。原因是 Vivado 2019 的 debug-core（VIO/ILA）会在工程目录里生成很深的临时路径，而 Windows 对路径长度敏感；仓库放得太深会让综合/实现在写 debug-core 临时文件时莫名失败。这不是建议，是硬约束——默认工程目录是 \filepath{fpga\textbackslash build\textbackslash address\_switch}，再加上仓库本身的深度，很容易超界。
\end{notebox}
...
\begin{description}
\item[\texttt{-{}-check}] \tfocus{离线自检}：不连硬件、不用真实引脚，只做 HDL/VIO 宽度契约校验 + 容量估算（综合自检）。第一次部署、或改过 RTL/Tcl 之后，先跑这个。它会逐条核对：top 是 1024 边沿、1024 扫描对的 62 路 wrapper，VIO 是 30 个输出探针、各探针宽度（prog\_addr=10、mask=62、prog\_count=11、scan\_count=11、bus\_counts=28 等）都对得上 Python 生成器。
...
默认工程落在 \filepath{fpga\textbackslash build\textbackslash address\_switch}；编译产物 \tfocus{bit 文件和 ltx 探针文件}在它的 \filepath{address\_switch.runs\textbackslash impl\_1} 子目录下，文件名是 \filepath{zlc\_pulse\_streamer\_top\_address\_switch.bit} 和 \filepath{...top\_address\_switch.ltx}。

\begin{notebox}[XDC 引脚映射与 .ltx 探针文件]
默认 XDC 是历史的 address-switch 引脚映射（\filepath{references\textbackslash source\_archives\textbackslash address\_switch\textbackslash address\_switch.srcs\textbackslash constrs\_1\textbackslash new\textbackslash addre.xdc}）。换板子/换线缆映射时用 \pyapi{$env:ZLC_PS_XDC} 覆盖。create-project Tcl 会\tfocus{重新生成 vio\_0 这个 IP}，并把它配成 \pyapi{C_NUM_PROBE_OUT=30}（30 个输出探针，覆盖扫描 + 模拟总线）。这一点很关键：服务器是\tfocus{靠 VIO 探针控制 FPGA}的，所以服务器加载的 \filepath{.ltx} 必须和烧进去的 bit 是\tfocus{同一次编译}产生的，否则探针名字/宽度对不上。
\end{notebox}
...
Project: ...\fpga\build\address_switch\address_switch.xpr
Bit:     ...\impl_1\zlc_pulse_streamer_top_address_switch.bit
LTX:     ...\impl_1\zlc_pulse_streamer_top_address_switch.ltx
Channels: ch00 ch01 ... ch61
Clock:   50000000 Hz
Capacity: max_edges=1024 scan_pairs=1024 bus_segments=4x64
\end{codeblock}
...
默认它在 \tfocus{host 0.0.0.0、port 18861} 上监听，后端是 \tfocus{vivado-session}（持久 Vivado Tcl 会话）。窗口会一直挂着——这是预期行为，\tfocus{不要关掉它}。常用环境变量覆盖：
...
\filepath{run\_server.bat} 在起服务前会硬性要求找到 \tfocus{.ltx 探针文件}。如果报 "no Vivado .ltx probe file was found" 或 ".ltx ... does not exist"，说明你还没烧 bit、或者 bit 和 ltx 不在它找的默认 impl\_1 路径下。解决：先跑 \filepath{build\_and\_program.bat}，或用 \pyapi{$env:ZLC_PS_VIVADO_LTX} 指到 Vivado Program Device 时实际用的那个 Probes 文件。
\end{notebox}
...
\item[现象：HDL/VIO 宽度自检失败（probe 数量/宽度不符）] RTL、top、Tcl 三者漂移了。\filepath{build\_and\_program.bat -{}-check} 会逐条报哪个探针宽度不对（例如期望 \pyapi{C_NUM_PROBE_OUT=30}、mask=62、prog\_addr=10）。按报错把生成器/Tcl 改回契约值，重新 create-project 才能让 vio\_0 IP 重新生成成 30 探针。
