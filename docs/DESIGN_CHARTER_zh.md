...
32. `Processor` 是唯一产品与领域概念；`latest-only`、`exact`、`finite` 是 host/delivery/execution policy，不得派生第二套公开 `MonitorProcessor` 类型、catalog、form、binding 或 lifecycle。当前 live Processor 可以由 framework 的一条内部 latest-only lane 执行，但领域 Definition、declaration、request、output 和算法仍全部属于原 capability。
...
34. `zlc_workbench/task_console` 只拥有 generic declaration projector、host、typed input resolution、Run/Processor lifecycle 与 output routing；具名 DefinitionKey 分支、per-node form/binding/presenter、物理 schema/materializer 与 SiteMap 语义全部禁止。真实启动差异只在 capability 的headless `workbench_adapter.py`，真实特殊UI只在inert `ui/**`；两者都由composition显式接线。SiteMap领域事实归neutral Calibration/Occupancy capability，view/Area/render/selector归frontend；neutral node在发布前交付已通过causal/same-shot atomic closure的typed outputs。Workbench linked-front只做GUI presentation revision gating，不得把它描述成物理join或跨producer same-shot证明。
35. `Zou_lab_control` 必须保留为唯一稳定 public notebook API、installation/repository/runtime binding 与 desktop composition adapter。它不能并入 neutral（否则领域层反向拥有应用 facade），也不能并入 workbench（否则 headless API 被 Qt 产品污染）；它只做显式接线、生命周期与窄委托，不得拥有 capability schema、算法、materializer、presentation 或第二套领域实现。
