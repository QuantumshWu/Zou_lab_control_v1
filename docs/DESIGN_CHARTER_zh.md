...
- **C49 主机内存不做准入策略**(2026-07-22 用户纠正):应用层不得定义主机内存预算、peak-byte估算、按窗口/管线分配额度、caller `memory_limit`或“required bytes”准入；分配失败就让当前操作以真实错误明确失败，性能与副本问题只凭profiling定位并优化，不能把保守估算改写成运行资格。仍须严格验证真实编码payload长度、shape/dtype、硬件/协议固定容量，以及事件/record数量等客观合同；这些不是主机内存预算。
