...
- **C49 先审机制、整闭包收口**(2026-07-22 用户纠正):当现象暴露问题时，先回答“产生它的机制是否本来就应该存在”，再修局部代码；权威顺序为**用户最新明确要求 > 本轮完整goal > 物理/算法事实与main的正式用户行为 > 本宪法/设计文档 > 当前实现 > 测试**，低层不得反向迫使高层保留错误设计。每个非main新机制在保留前必须通过clean-slate审问：如果今天从零设计，能否点名它解决的真实需求、唯一owner、不可由更直接方案替代的收益，以及与复杂度相称的现有consumer；答不出就删除。删除必须覆盖完整依赖闭包（类型、生产者、消费者、分支、导出、文档、测试），不得留下alias、wrapper、兼容层、迁移态或改名残余。测试只约束用户行为、物理/数据不变量和合法架构边界，不锁定某个补丁、私有方法、调用次数或实现形状。一次提交必须关闭一个完整根因闭包；未闭合前不提交、不把该机制复制到下一GUI，也不以更多专项guard代替正确设计。
