...
## 22. AOD atom rearrangement -- defect-free array assembly (#H3x, 2026-06-27)

The standard neutral-atom rearrangement loop, added as a new plugin across the five layers WITHOUT
refactoring them (the seams were already extensible). Flow: **image** the stochastic ~50 % loading ->
**detect** per-site occupancy (the existing `TrapCalibration.detect` contract) -> **plan** the moves
that fill a target defect-free set with minimum displacement -> **drive the AOD** to execute them ->
**re-image** and report the fill fraction.

- **Planner (pure analysis layer)** -- `operations/rearrangement.py`, backend-free (virtual==real safe):
  `target_sites(grid_shape, n_target, layout)` (the central-block / full target set);
  `plan_rearrangement(occupied, grid_shape, targets) -> RearrangePlan` ports the lab reference
  `compress_algorithm.LSAP_compressed` core -- centre-sorted Hungarian assignment
  (`scipy.linear_sum_assignment`) over Manhattan distance, atoms already on a target left in place, then
  a collision-safe SEQUENTIAL move order (each move's dst is empty when it runs);
  `apply_moves_to_occupancy(occ, moves, survival, rng)` is the atomic occupancy-remap oracle (per-move
  transit loss) shared by the virtual device + tests, so the predicted array is single-source. The
  reference's extra per-move waypoint path-collision optimisation (X-then-Y vs Y-then-X) is a real-HW
  refinement noted for follow-up -- it does not change the ASSIGNMENT, and the virtual remap is atomic so
  it is exact without paths. Guard: `tests/test_rearrangement.py`.
- **AOD device role** -- `devices/base.py::AODDevice` (abstract `apply_moves(moves, *, survival)`,
  `ROLE_BASES["aod"]`). The actuator that EXECUTES the plan, swapped at the device so the orchestration
  never branches on backend (the virtual==real seam):
  * `devices/virtual.py::VirtualAOD` wraps the shared `VirtualTrapArray`; `apply_moves` calls
    `VirtualTrapArray.apply_moves` -- the lowest-layer physics fake: it remaps the occupancy src->dst
    (atom carries its temperature; lost in transit at `1-survival`) and PINS the result so the next
    camera image shows the rearranged array (mirrors `set_occupancy`). Wired into `virtual_config` as
    `"aod"` -> `$device:trap_array` (the SAME array the camera images); `move_survival` shortcut in
    `virtual_config_with_overrides`.
  * `devices/aod.py::RampAOD` is the real FPGA backend: it drags atoms by ramping the X/Y DAC buses from
    the source-site codes to the destination-site codes over a move window (reuses the existing analog-bus
    `"ramp"` primitive -- NO new RTL), then holds at the destination during settle, firing one ramp
    program per move (sequential single-tweezer, per the reference `moveoff_atom`). Site->DAC-code
    calibration + board channels (`x_channels`/`y_channels`, LSB-first) + timing are injected via config
    (`configs/remote_template.json` has an `aod` entry), never guessed. Registered in
    `registry.BUILTIN_DEVICE_CLASS_PATHS`; `DeviceSet.aod` is opt-in (a config without an `aod` simply
    has no `exp.devices.aod`). Guard: `tests/test_aod_rearrangement.py` (move program ramps src->dst,
    holds at dst, compiles to a valid sequence).
- **Task + subsystem** -- `operations/logic.py::RearrangeTask` runs the whole loop one-shot, streaming
  before/after frames + occupancy + progress to its dedicated mid-run panel; result (fill_fraction /
  n_filled / n_target / n_loaded / n_moves / moves) on `self.result`. It is backend-free: it takes the
  camera + AOD devices + two closures the subsystem wires (`frame_provider`, `calibration_provider`).
  `subsystems/rearrange.py::RearrangeSubsystem` (on the session as `exp.rearrange`) wires those closures
  to the session imaging path + thresholded calibration: `exp.rearrange.execute(...)` runs one pass
  headless, `exp.rearrange.task(hub, ...)` builds the GUI task. `operations/tasks/rearrange.py` is the
  `@task` factory -> `TaskSpec "Rearrange array"` (prefix `rearrange_`), auto-discovered into
  `exp.readout.task_specs()` and the console's Add-Panel "Task" group; it routes through
  `exp.rearrange.task` so GUI and notebook cannot drift. Mid-run panel shows `frame_after`.
- **Hardware-physics note (real machine)** -- a 2D crossed AOD positions a tweezer by its X/Y RF tones;
  each tone's frequency is set by a DAC voltage -> VCO -> deflection angle, so a frequency chirp src->dst
  is a DAC voltage RAMP. Move ≈ 50-200 us, ≈1-5 % loss/move, final fill ≈ 85-95 % on a real array.
  `move_duration_s`/`settle_s`/`move_survival` model this; the analog-bus ramp IS the chirp.
