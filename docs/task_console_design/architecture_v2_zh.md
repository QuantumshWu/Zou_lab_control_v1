...
- **节点基类与 KIND**(`operations/logic.py`):所有逻辑节点继承 `LogicNode`。`Measurement` 类节点拥有 repeat 轴(`repeat`/`free_run`,填 `(repeat,*points,*data)` 块);`CameraMeasurement` 与 `ScannedMeasurementNode` 都是其子类。`Processor` 为 reactive:`new_inputs()` 按 hub 每信号版本只在输入前进时发,`step()` 返回空 dict = no-op(skip-on-empty);静态 `repeat_contract`(`reduce`/`preserve`)声明它对 repeat 轴的关系,**不是用户 mode**。`OccupancyProcessor(Processor)` 是 `preserve`(#H3q):**逐 repeat 切片**跑 `calibration.detect` → `occupied`/`counts` 为 `(repeat,1,n_sites)` 块、`frame_judged` 为 `(repeat,1,H,W)` 块(repeat 轴穿过 processor,`repeat_mode=average` over `occupied` = 逐站装载概率)、`centers`(N,2)/`thresholds`(N,) 静态、`rate` 标量(本块装载率);删了累积 `rate_sites`/`rate_grid`(= `repeat_mode=average` 的重复)。是**真 detect 流程**:相机 measurement 只发 `frame`,OccupancyProcessor 单独跑 detect,`occupied[r] == cal.detect(frame[r]).occupied`。
