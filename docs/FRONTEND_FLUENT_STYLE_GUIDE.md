...
For visual changes, generate screenshots with both default visible channels and all 40 channels. In headless Qt, text may not render in screenshots; also run object-level checks that button text fits each button, `show_all_channels()` keeps 40 channels visible, and prepare/fire/wait/safe call the passed-in sequencer.
