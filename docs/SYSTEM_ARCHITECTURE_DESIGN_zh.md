...
本文同时区分三种东西：终态不变量、当前冻结硬件上的 baseline capability、以及有明确删除点的迁移脚手架。正常 PulseScan 的执行方式族是现有 bitstream 的 `AUTONOMOUS_STREAMED`，其装载方式分为 fire前全部rows resident的`AUTONOMOUS_RESIDENT`与条件性的`AUTONOMOUS_REFILLED`；近期baseline只开放resident，refilled默认拒绝，只有§15.4强证明后才成为条件capability。Formal资格不是装载方式名称的一部分，而是 execution mode、有效Q0 qualification、association proof、软件exact链和本run EndAttestation共同评估的结果。唯一允许的非自主执行例外是 API-slot 无法在运行中无缝更新时已经存在并被接受的 `API_SLOT_SEGMENTED_EXISTING` 路径；segment之间的host gap属于该历史API语义，必须显式记录且不能称为连续或autonomous。除该例外外，`HOST_STEPPED_GROUP`/逐 cell fire-and-wait 不属于设计。逐沿 stamp、额外 ROM attestation、trigger-return 或新 watchdog 等需要重烧的增强只可在§1.1/H2的证据条件、因果证明与独立批准全部满足后作为硬件修复候选，不能成为软件架构的前置要求。
...
Definition.bind 只能通过 resolver 一次取得自己显式声明的 BoundDevice，并把该对象放进领域私有的 immutable bindings；Port、claim、affinity、capability 和 identity/generation 不能作为平行字段分别传递。generic runtime 不定义公共的泛型bindings容器；领域 composition 把 typed request/bindings 冻结在计划构造边界，随后只向 `RunPlan` 交付 `BoundDevice` 与阶段 callable。execute 不能按字符串回查 installation graph，resolver 也不能从全局 registry 隐式挑“第一个相机”。
...
MonitorTap 可以请求小 backlog（例如 image 2–8 帧），其合同就是允许 overwrite 并报告 missed；可见 rolling history 长度由下游 MonitorDataset 决定，LiveDatasetSlot 只共享 handle/revision 并 coalesce 通知，绝不能再持第二份 window/buffer。同一 source 的多个 panel默认共享一个 admitted slot，再用 ViewSpec 取 `<= capacity` 的窗口，而不是各自动态 attach 大帧 ring。正式采集不能靠“把 history 设成 full size”保证完整：RunController 必须沿 source -> processor -> sink exact chain 汇总 event/byte budget并在 fire 前 reservation。MonitorTap 与其唯一 materializer 都必须在首次 publication 前绑定；这个本地时序门禁仍不等于全拓扑 byte admission，在 S5 composition factory 汇总 tap backlog、materializer window、snapshot/front-buffer 并冻结 topology 以前，禁止把当前构造器接入真实 Workbench。execute 中或 panel 打开时不得新增 raw tap/window。
...
Repository将runtime `CalibrationArtifact`与diagnostic `CalibrationReport`分成两条读取面：current manifest配对一个小型typed artifact blob、一个report-metadata blob和一个codec-stable `CalibrationRuntimeSummary`；全分辨率`reference_average(<f8)`与validity使用两个raw CAS blob，由metadata的owner-encoded `ContentRef`引用。summary按每个持久逻辑array字段计数，绝不依赖进程内ndarray alias；`inspect_final()`仅读FINAL manifest即可得到source capture ref、binding、site/model选择和owner报告的inspection/admission上界，完整load后必须重算并exact compare。`load()/admit()`只读取manifest+artifact，绝不为occupancy拉入report、平均图、validity或SciPy；只有显式`load_report()`才按内存预算materialize diagnostics。写入端在编码或复制全分辨率诊断前先按已知shape/dtype做峰值admission，metadata产生后再做精确第二次检查；staging预算区分raw image/mask copy与其它canonical array的normalize/base64/JSON/decode-roundtrip overlap。在发布manifest前还必须用读取端同一个size/structure/typed decoder做round-trip，不能生成自己无法读取的FINAL。pending recovery对raw arrays使用有界流式hash，不materialize大图，也不重跑detector、fit或threshold。Repository锁只保护open/commit状态与coordinator线性化点，CAS read/write、report decode和大数组复制全部在锁外；因此GUI打开report不能阻塞并发runtime admission。
...
DataTransform 后仍存活的每根非 scan data axis 必须出现在 output_data_axes；batch_axes 只是其语义子集，不改变 ndarray 中 axis 的存在。repeat 是 Formal Scan Dataset 的权威 R 轴，不属于“可被 y transform 顺手 reduce 的非 scan axis”，ScanOutputContract 必须原样保留其 AxisId/coordinates/validity；若某个后续 Analysis 想对 repeats 求均值，它在冻结 ScanArtifact 上另建显式 ReductionSpec，不能改写原始 scan y。终态中，若source本身已经恰好是权威scan dataset且没有选择/降维，committed_transform可以为空；**当前direct-camera producer并非这个用例**：raw Capture的logical cell axes还含singleton `READOUT_EVENT`，所以binder始终提交一条具名`Selection.index(readout_event, 0)`，即使用户没有额外transform，ScanOutputContract也仍持有非空CommittedTransform。raw身份继续属于CaptureArtifact，不能把metadata-only消轴后的output冒充raw snapshot。等第二个真实producer出现后再实现optional分支，不预建raw/committed mode enum。最终 ScanArtifact 保存 scan point_axes、PointLayout、repeat axis和全部 output/batch axes，不能压成 `(repeat, data_points, data_dim)` 三个匿名长度。
...
当前迁移中的 **S4a virtual authority** 只实现上述模型中不依赖真实硬件资格的确定性部分，不能被命名或显示为 Formal：composition 只开放 `virtual`，repository只接受`SIMULATED` terminal evidence。它先把logical PulseDocument绑定到live target，以bound sequencer公布的resident capacity在展开前检查`R*P`；随后由pulse owner把whole-document RepeatRegion确定性展开为repeat-major的有限物理table，使用一次fire、`loop_count=1`和`repeat_forever=False`编译，而ScanPointTable始终从未展开logical document取得，所以physical row duplicate不会污染logical point identity。inspection只对同一canonical ScanIntent做encode/identify，不写磁盘；真正run在runtime start/FIRE前durable publish该intent，并让raw Capture的BlockId携带intent digest。该切片没有API-slot、host-stepped、cursor-wrap stop、RTL或bitstream fallback。真实hardware composition、Q0/H1/EndAttestation、flat Run commit/failure record和§16.2所述Formal提交门齐全前，hardware ScanArtifact继续typed NO-GO。
...
PREPARE_API_SEGMENT(run_id, segment_id, frozen API values,
  ...
  server records PreparedProgramRef(
    server_connection_generation, run_id, segment_id, artifact_digest, segment_schedule_digest
  ) in software
...
任何prepare/upload/identity validation失败都不得调用FIRE。Pulse RPC server 重启或重连会改变`server_connection_generation`，旧PreparedProgramRef在软件侧失效；即使设备仍保留旧active image也不能由正式路径误触发。API-slot segmented路径每段同样冻结自己的values与artifact lineage。
...
S4a的virtual两阶段桥严格按这条“上游独立事实”工作：fire前的current-only ScanIntent只保存repository ids、owner-encoded logical PulseDocument blob ref和CommittedTransform intent，它本身不是成功artifact；FINAL raw Capture以intent digest派生的BlockId绑定该意图；最后的Scan manifest只保存raw Capture ref与ScanIntent ref，不复制PulseDocument、DatasetSchema或transform truth。promotion从FINAL raw ref重新admit全部依赖并幂等发布同一content-addressed Scan ref，进程在raw commit后退出且调用者持有raw ref时可由`promote_scan(raw_ref)`恢复；当前桥不承诺硬进程死亡后自动发现全部待promotion raw，也不增加目录扫描/list-latest API。manifest publish返回异常或KeyboardInterrupt时必须读取并durably确认exact target：已可见就返回成功，确认缺失才报告失败，无法判定则返回typed reconciliation-required而不能谎报失败/取消。inspection走纯identity路径，因此不会留下未FIRE intent。该桥仍不是Formal flat Run的最终提交协调器；真实hardware enablement前必须把同一事实接入RunController的SafetyDispositionBundle、CommitIntent、RunFailureRecord与terminal gate，由该flat Run提供durable terminal与待完成结果发现。
...
6. W3a 已交付 Capture/identity-Fit notebook surface；W3b再由scan owner把`ScanArtifactRef -> canonical output OwnedSnapshot -> DataFigure`接通。output BlockId由scan manifest digest派生，generation/revision继承exact source dataset，schema fingerprint来自ScanOutputContract；zlc_data在同一总预算下直接执行CommittedTransform并一次冻结最终DataBlock，scan/notebook都不重做reshape、validity映射或lineage。任意 committed-transform fit 的忠实 observations display仍需一个真实 transformed-display consumer 才扩 FigureEvaluator 输入；`fit(scan_ref)`当前显式拒绝，不建立通用transformed-dataset或ScanFit框架。
...
| `R01_STREAM_DATASET_PROCESSOR` | `zlc_neutral_atom/processing/*`、`runtime/dataset.py`、`runtime/streams.py` | 6 | COMPLETE | exact/live kernel与W1 capacity-one finite接线READY；rolling/multipanel仍属U01 backlog |
...
| `U01_GUI_WORKBENCH` | frontend render、Workbench shell、旧frontend与`pulse_gui.py` launcher | 17 | COMPLETE：existing primary paths及最新W1差量复审闭合 | board identity/final gate与virtual/offscreen single-event finite capture产品IMPLEMENTED；通用scheduler/PanelHost、rolling/grid/overlay与legacy GUI迁移BACKLOG/NO-GO |
...
**S4a autonomous scan virtual authority纵切现为virtual/offline READY，Formal hardware仍NO-GO。** logical document先绑定当前PulseTarget并以bound sequencer公布的`resident_scan_point_capacity`拒绝超限`R*P`，pulse owner再把whole-document repeat展开为repeat-major physical rows；current oracle中`R=2,P=8`得到`point_count=16, loop_count=1, full_point_loop=True`，cell严格为`r0/p0..7`后`r1/p0..7`。scan domain使用`scan.parameter.da_x/da_y/da_z`和原PointLayout，raw为`(R,P,READOUT_EVENT,*data_shape)`，CommittedTransform只显式删除singleton event axis，public materialization以不可写、no-copy reshape暴露`(R,P,*data_shape)=(2,8,96,128)`并保留`camera.y/camera.x`及component validity；不存在`[:,0]`、flatten、trailing-axis mean或匿名`data_dim`。

同一canonical intent由纯`identify_intent`服务inspection、由`prepare`在runtime start前durable发布；raw BlockId绑定intent digest，final Scan manifest只引用raw+intent，fresh process promotion幂等。手写current-format故障oracle证明：inspect后intent/capture/scan manifest为`0/0/0`；正常run为`1/1/1`；intent可见后注入KeyboardInterrupt为`1/0/0`且没有FIRE；final manifest可见后注入KeyboardInterrupt仍返回同一成功ref；`R*P=4104>4096`在任何manifest前拒绝；fresh process可从同一raw ref恢复。没有运行或修改历史pytest，也没有API-slot、HOST_STEPPED、cursor-wrap、RTL或bitstream变化。

W3b关闭了上段留下的application blocker。ScanRepository现在是output dataset identity的唯一铸造者：`BlockId = scan-output-{ScanArtifact manifest digest}`，stream generation/revision精确继承source dataset，schema fingerprint只取冻结ScanOutputContract；同一artifact跨重复materialize和fresh process得到同一ref，但永不复用raw Capture BlockId。`MaterializedScanData`只保留ScanArtifactRef、exact source DatasetRevisionRef和最终OwnedSnapshot，不长期扣住PulseDocument，也不复制output contract truth。zlc_data新增唯一public的bounded committed-transform snapshot边界：同一个`memory_limit_bytes`先约束raw Capture array materialization，再约束“source retained + selection view bases + reduction/validity/object-sum scratch + final immutable values/validity”的完整data-plane peak；immutable schema/layout/coordinate metadata继续由Scan Repository既有resource policy独立准入，与raw Capture的memory cap口径一致。该边界只承诺当前ScanOutputContract已经挣得的cell-preserving语义：system singleton `READOUT_EVENT` index drop、cell full-index-range no-op以及trailing data select/reduce；generic repeat/point select/reduce仍只属于`apply_transform`，不能因函数可执行就虚假获得bounded snapshot资格。任何data-plane预算不足在transform大数组分配前拒绝；执行直接把内部state一次冻结成最终DataBlock，不再走`TransformedData -> DataBlock`第二次全量相机copy。RowComponentValidity只按output DatasetSchema的repeat、PointLayout storage和具名component axes转换为CellValidity/ComponentValidity，绝不展开成全pixel mask或按rank猜轴。

notebook的`figure_document(scan_ref)`只admit manifest/schema，不物化帧；`figure(scan_ref)`才取得上述bounded snapshot，随后复用同一个role-driven ViewSuggestion和DataFigure renderer。current sparse oracle以`R=2`、三个physical rows、三根scan axes和`96x128`相机数据证明：raw与output逐值相同，raw logical `READOUT_EVENT`消失而scan PointLayout、二维data axes和repeat原样保留；1-byte cap失败后同一artifact可重试，output ref重复/fresh-process稳定，PNG可渲染，`fit(scan_ref)`仍typed拒绝。component oracle另以`(R=2,Pstorage=3,READOUT_EVENT=1,site=3)`手写mask证明dead site不会升级成cell invalid。两个W3b白名单node当前分别通过；没有运行或修改历史pytest，也没有删除legacy PulseScan/TaskConsole路径。

Rule-6仍以既有scan package加integration owner为同一归因口径。W3b相对`ca79922`的七个production owner净增`241 physical / 225 nonblank`，其中同时物理删除旧67行无output identity的wrapper和重复lineage字段；没有新增enum、repository、manager、policy、公开planner或generic derived-data framework。一次把memory admission粗压成全局公式的尝试在二维mean profile中达到`29.27x`过度保守，不能作为可用的用户预算；恢复按Selection/Reduction真实scratch分支计算后，当前二维mean样本的admission为`1,967,328 bytes`，source加实测峰值为`859,953 bytes`，约`2.288x`。最终对抗审查又用`10,000`行EXPLICIT sparse cell reduction证伪过宽承诺：旧generic路径估算`820,000 bytes`、实测source加peak为`6,669,890 bytes`。本轮没有用更大拍脑袋常数或重写无人消费的sparse reducer遮盖，而是在大分配前typed拒绝所有cell reduction，把bounded面收回当前scan真正需要的cell-preserving contract；trailing-data mean另有手写数值/validity oracle。累计physical envelope由`1980`变为`2221`，相对main窄等价`703`为`3.159x`；约三倍检查已触发并完成，超过严格3倍的112行对应main没有的data-plane peak admission、single-freeze output snapshot、stable derived identity、具名validity转换与notebook Figure接缝，以及这次由真实profile要求的bounded-surface防线，不能通过复制公式或重新放宽已证伪边界来伪装压缩。

中间迁移态的“当前 production 调用数”只是一条证据，不能单独裁决目标能力：

| 能力 | 明确的终态 consumer | 审查动作 | 允许物理删除的条件 |
|---|---|---|---|
| `MonitorTap -> MonitorDataset` bounded live | Workbench live image、rolling curve、histogram、board coherent present | W1已把capacity-one finite preview接入public single-panel Qt产品；tap只拥有admitted backlog，materializer拥有keyed cycle/append window、head/EventRef与atomic snapshot；rolling/histogram/multipanel/process-wide composition仍待S5 | 同等 GUI capability 已由更小唯一 owner 交付并迁完全部 panel |
...
| current `ScanArtifact -> DataFigure` | notebook autonomous direct-camera scan inspect/render；后续scan-fit | W3b已交付稳定output snapshot与静态figure；它不是旧TaskConsole external measurement/processor y的等价replacement | source-neutral exact pipeline、TaskConsole SCAN_SLOT consumer及后续scan-fit各自交付后，才按最后consumer删除对应旧路 |
...
规则状态必须诚实记录为 `NOT_STARTED/PARTIAL/COMPLETE`。§22.1固定的11个互斥production owner闭包已经逐一完成，规则1–8的**existing-code追溯审查轴全部COMPLETE**，所以“完成1–7前不得继续迁移”的冻结已经解除；这不等于implementation全部READY。当前明确为GO的是virtual finite capture/pulse owner、BoardController coherence foundation、W1 single-event finite exact capture的public virtual/offscreen Qt产品、S3a“committed capture -> calibration ref -> artifact/report reopen”、S3b“committed capture + calibration ref -> occupancy ref -> reopen”和S3c“installation-qualified short sitemap -> raw FINAL -> calibration FINAL/recoverable failure”的virtual/offline纵切。S3a–S3c结案证据包括完整preflight→execute→FINAL→admit current-format手工oracle、1-byte预算与raw recovery、fresh-process notebook不加载SciPy/Qt/Matplotlib、exact `(R,P,*data_shape)`/`(R,P,site)`/component-validity检查、repository close与run borrow生命周期、static DAG/shape/旧Registry扫描，以及每个切片的两路独立对抗审查均`P0=0/P1=0/GO`；W1另有tracked白名单Qt E2E、snapshot/source invalidation、preview故障隔离、generation/run-id生命周期与三路独立对抗证据。没有运行清单外历史pytest。remote/real qCMOS、正式scan物理资格、完整GUI scheduler、rolling/overlay/save/Setting/Edit层、GUI site-map/gridplot和普通用户真实设备入口仍分别NO-GO。每个后续dependency-closed切片仍须按规则1–8做增量审查，完整迁移结束后还必须对最终分支做一次独立对抗验收；局部GO或owner audit COMPLETE都不能改写成全局实现完成。

最新实现 checkpoint：W3b current Scan application boundary已完成代码与新增白名单纵切；最近闭包提交仍待本切片最终机械审查后写入。public notebook现在可从ScanArtifactRef直接得到metadata-only FigureDocument或bounded DataFigure，scan owner提供稳定output identity、exact source lineage、一次冻结和具名validity；frontend仍不认识neutral repository，notebook不构造BlockId、不reshape PointLayout、不转换RowComponentValidity。W3a的Capture/identity-Fit overlay边界保持不变，`fit(scan_ref)`、FigureArtifact persistence、live selector/session和transformed-fit observations display仍明确deferred。下一切片是 **TaskConsole current SCAN_SLOT panel**：必须从旧generic PulseScan的真实external measurement/processor y consumer出发，建立source-neutral exact pipeline与PanelController接缝；不得把本轮direct-camera ScanArtifact偷换成等价replacement，也不得提前删除旧PulseScanNode、MeasurementSpec或TaskConsole编辑器。W3b当前闭包证据为四个新增白名单node通过、fresh-process reopen、受影响的权威整数SUM node通过、语法/import与owner扫描，以及generic sparse反例profile被bounded-surface拒绝关闭；Rule-6累计scan envelope为`2221/703 = 3.159x`并已完成压缩与真实peak profile说明。唯一一轮独立对抗审查发现的sparse cell reduction低估、schema-metadata口径和冗余no-op虚高均已关闭，最终增量审计为`P0=0/P1=0/GO`。
