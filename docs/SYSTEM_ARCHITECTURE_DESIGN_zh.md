...
本纵切继续固定 `main@6c337d49c7086fa0ff21f879cd159bdf0e753f51`。旧行为的具体证据是 `frontend/live.py::BaseLivePlot/Live1D/LiveLive/LiveLiveDis`、`frontend/selectors.py::AreaSelector/CrossSelector/ZoomPan/attach_interaction` 与 `frontend/task_console.py::_build_plot/_render/set_selectors_enabled`；current 的两个真实产品消费者是 camera ROI scalar 的 `MONITOR_HISTORY -> CURVE` coherent board，以及 occupancy progressive scan 的 `SCAN_POINT -> CURVE` provisional board。纯函数或合成曲线测试不能替代后者。两个 dependency-closed 提交现均已完成：rolling Monitor 与 progressive scan 共享同一个 curve display/interaction owner，同时保留各自 newest-first 与 finite exact point 语义；本小节据此整体 `GO`，但不能外推为 finite capture、Sites、Histogram、Grid 或 launcher 已完成。
...
notebook的`figure_document(scan_ref)`仍只inspect manifest/index/schema，`figure(scan_ref)`才bounded materialize并复用role-driven DataFigure。`exp.scan_gui(frozen_request)`的direct-camera/API-slot产品明确为FINAL-only；autonomous occupancy产品则在同一个Run中先显示exact `SCAN_POINT` PROVISIONAL curve，preview退役后再原子切换canonical FINAL raster。start/result/projection/reap仍在有界worker，GUI只看revision-gated ViewModel与immutable front；新Run立即清除上一轮FINAL，projection失败仍保留FINAL ref，close/cancel异步reap。同一个projection cap按顺序而非相加约束`repository materialize`、`input snapshot retained + FigureEvaluator peak`、`evaluated arrays + Agg canvas/PNG`以及`PNG + source/scaled QPixmap`；Qt禁止放大到source raster以上，避免resize绕过预算。current oracle同时覆盖direct `(2,3,96,128)`、occupancy `(2,2,site)`、ComponentValidity、processor/calibration lineage、fresh-process reopen、metadata-only且禁止heavy-lineage decode的FigureDocument、pre-FIRE static-lineage拒绝、bounded PNG、progressive A/C/Z/H/Setting/Edit与Qt close/stale-raster race；`fit(scan_ref)`仍typed拒绝。没有删除legacy PulseScanNode/MeasurementSpec，因为scan-fit及其余旧消费者尚未迁完；current TaskConsole的scan panel/edit/catalog已经接入但完整多plot产品仍未完成。
...
|---|---|---|---|
| `MonitorTap -> MonitorDataset` bounded live | Workbench live image、rolling curve、histogram、board coherent present | W1已接finite preview；M1/M2与U0.2b/c已把独立virtual free-running MOT camera、raw IMAGE、显式typed ROI scalar、独立长scalar history、同一scalar revision的CURVE/HISTOGRAM/METER、raw/scalar同源四panel及IMAGE/CURVE完整selector/display control接入public notebook/Qt；U0.2d另闭合FINAL exact-cell Sites交互并保留双ComponentValidity和同cell lineage。real Pylon qualification、finite capture、live paired Sites及Histogram/Grid完整交互仍待后续 | 同等 GUI capability 已由更小唯一 owner 交付并迁完全部 panel |
...
规则状态必须诚实记录为 `NOT_STARTED/PARTIAL/COMPLETE`。§22.1固定的11个互斥production owner闭包已逐一完成Rules 1–7 existing-code审查，所以“完成1–7前不得继续迁移”的历史冻结曾解除；规则8/9是持续per-commit gate，不能一次永久COMPLETE。virtual finite capture/pulse owner、BoardController coherence foundation、W1 single-event finite exact capture、S3a/S3b/S3c、W3e/W3f、W-UI1a及U0.2b/c/d已经证明的底层/authority/lifecycle与camera IMAGE/CURVE/SCAN_POINT及exact Sites交互事实保留，但其余缺失UX按§2.2/U0继续MUST_CLOSE。既有结案证据包括preflight→execute→FINAL→admit current-format oracle、预算/recovery、headless import、exact多维轴/component-validity、repository/Run生命周期、DAG/shape扫描与有界对抗审查；这些机械证据必须叠加活动白名单完整collect+run与适用的UX salvage/E2E。remote/real qCMOS、正式scan物理资格、完整GUI scheduler、finite capture、live paired Sites、Histogram/Grid交互、其它panel的Save/Setting/Edit、完整GUI site-map/gridplot和普通用户真实设备入口仍分别NO-GO；不能再把已经完成的camera rolling/overlay、occupancy SCAN_POINT或exact occupancy Sites笼统写成NO-GO，virtual/offline API segmented也不能倒写成真实qCMOS Formal资格。每个后续dependency-closed切片须按Rules 1–9做增量审查，完整迁移结束后再对最终分支做一次独立对抗验收；局部GO或owner audit COMPLETE都不能改写成全局实现完成。
...
聚焦oracle为Sites `32 passed`，并逐文件新进程通过W4 `24`、W5 `8`、W6 `7`、W7 `7`项直接回归；syntax、diff-check、headless import、旧symbol扫描与offscreen性能证据通过。活动manifest共98个文件，最终collect为`1268`项，逐文件新进程实跑`1265 passed + 3 expected skipped`、零失败/错误。两路独立数据/生命周期审查最初抓到exact selection seam和retained-bound cap两项P1，修复与公开window反例后复核为`P0=0/P1=0/GO`；独立Rule-6终审在继续删除test-only accessors、死state和重复array owner后为`P0=0/P1=0/P2=0/GO`。本checkpoint据此只对exact Sites判`GO`，不外推到live paired Sites、Histogram、Grid、finite capture或完整launcher。
