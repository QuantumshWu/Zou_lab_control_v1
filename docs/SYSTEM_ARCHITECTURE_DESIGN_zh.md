...
4. 逐条迁 temperature、MOT、readout、device manager 和 notebook convenience；W4a只完成current artifact的冻结`.figure_gui()`只读显示，rolling/overlay/交互fit/edit、saved gallery与其它convenience仍按各自纵切迁移；每条都按纵向切片删除自己的旧路径。
