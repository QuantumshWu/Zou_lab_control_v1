...
| `U01_GUI_WORKBENCH` | frontend render、Workbench shell、旧frontend与`pulse_gui.py` launcher | 17 | COMPLETE：existing primary paths及最新M2d差量复审闭合 | board identity/final gate、virtual/offscreen single-event finite capture、显式ROI scalar history、coherent四panel及fixed-ROI front-bound selector/overlay产品IMPLEMENTED；raw-only首ROI、通用scheduler/PanelHost、rolling grid与legacy GUI迁移BACKLOG/NO-GO |
