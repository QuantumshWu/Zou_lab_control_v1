...
5. `zlc_workbench` 只拥有 Qt 窗口、卡片、路由、surface-scoped session、cancel/close 和产品布局；它通过 composition 注入的窄 operation-runner port 提交 Fit 等纯计算，不得重新定义领域字段、Dataset shape、Fit 算法、执行后端或 renderer。Qt callback 不得遍历、复制、编码或 hash 大 ndarray。
...
GUI composition 在 `zlc_workbench` 先以一次 prepare/commit 安装 immutable `PresentedSignalPublication`（exact neutral publication borrow/ref + 与该具体revision对应的frontend presentation descriptor），再由同一批配对值提升 `PresentedSignalFront`；不存在独立可变的visible/candidate presentation side index。该front是pair的base-presentation borrow owner，supersede/withdraw/close时整对exact-once release，panel不得各自retrospective retain。只有在Fit/gesture等显式operation的同一个atomic freeze边界，composition才能从该exact publication铸造operation-scoped child borrow；它让旧front被替换后operation仍安全读同一source，并在complete/cancel exact-once release，绝不按name/latest补retain。非 Qt application operation 在 `Zou_lab_control` composition 按同一 pairing contract 执行。任何 required presentation 缺失都必须在route admission/prepare阶段作为typed nonfatal错误拒绝，不能让已可见signal在GUI timer/topology查询中fatal。standalone artifact Figure只持纯FigureFront，仍可画selector/Fit overlay并返回本地FitResult，但没有parent时不得伪造或发布derived signal。translator只消费一次frozen frontend base front，selector/Fit发布端同时消费配对的exact neutral child borrow，绝不能分别读取两个latest：
...
- Fit绝不隐式Hold、Pause或钉住live base。Monitor/Setting激活Fit时创建surface-scoped `LiveFitSession`：base持续呈现每个最新front；每session最多一个active candidate和一个latest-pending完整candidate，新front只替换尚未开始的pending并立即释放被替代candidate的child borrow，不反压producer，不混revision；完成后公平调度最新pending。结果保留exact source/projection provenance，参数以独立`EVENT_RESULT` atomic sibling bundle发布，diagnostics只留FitResult/UI/artifact，不进signal plane，也不发布拟合曲线数组；overlay按上一段的current/lag/incompatible规则独立呈现。source generation/schema/axes/CommittedTransform/model/facet改变时取消旧session或使旧overlay incompatible。
- Edit/DataFigure Fit是只属于该snapshot surface的一次性`SnapshotFitInvocation`；它可以冻结自己的source，但不得读写Monitor surface的Fit spec/source/result、不得改变任何Monitor cadence，也不得进入continuous causal frontier。signal-connected Edit拥有exact parent时仍可把named params发布为独立EVENT_RESULT；standalone artifact无parent时只返回本地结果。显式Pause、press→release手势和Edit snapshot可以Hold自己的base；Fit按钮本身不能。这里的LiveFitSession/SnapshotFitInvocation是必须区分的语义scope，不强制增加两个public wrapper class；最小内部record能守住owner/cancel/result边界即可。
- Fit compute、Matplotlib base render和selector Dataset materialization是三种独立operation class。composition-owned generic runner实行per-session one-active+latest、跨session公平与cancel/shutdown；Qt只O(1)提交immutable ref/spec并接收小结果/overlay primitives。执行后端不预先锁死为thread或process：先消除Python逐元素循环、全图坐标/validity/materialization与Qt侧codec/hash，再以真实profile决定；不得用独立进程掩盖可避免的复制，也不得引入caller-visible memory budget。

Selector也只有一个frontend contract/interaction engine，Curve/Histogram/Image/SiteMap/Grid cell全部复用FigureFront中同一RenderedGeometry/coordinate mapper、Divider与overlay painter，不得每kind自写。Cross发布exact rendered sample/cell的数据值（位置只进provenance，不另发XY signal）；Area发布一个保留source dtype/未选axes/component validity的selected Dataset，并把exact typed bounds/SelectionSpec放在同一signal的schema/provenance中，范围与数据同一authority，不再造第二个lossy bounds array；Grid委托当前cell并携facet address；不发布hover。press到release锁定同一FigureFront+RenderedGeometry+parent pair；new source front可暂存到release后present，但resize/DPR导致geometry invalidation时必须原子cancel gesture且不发布final，再用新geometry compose，绝不用新widget geometry解释旧raster。drag中overlay与monitor-derived值连续更新，overlay用轻量painter/blit，不以每个pointer event整图compose。gesture active输出无FormalAssociationCapability；release冻结selection成新producer generation，之后对每个新parent做one-to-one projection并可获capability。Area crop shape、Fit parameter schema等改变必须换signal generation；monitor panel可重绑，已bind formal consumer遇generation change则失败。
...
- Workbench/application composition先prepare每个exact parent borrow与对应frontend descriptor，全部成功才commit并提升同一个PresentedSignalFront；pointer gesture从press到release固定该pair。Area/Cross等持续route各自发布COHERENT_CONTINUOUS，Fit参数单独发布EVENT_RESULT，不能把它们塞进一个“panel outputs”bundle使任一事件阻塞continuous front。异步Fit invocation同时冻结base front与parent pair；solver完成后通过DerivedSignalPublisher发布该frozen parent的参数bundle，overlay source/lag按§4独立判断。
...
- 先定义 PointTable/GridTopology/PointGroupSpec、AxisSourceRef/SourceViewBinding/CommittedTransform/FitSpec、ProducerTransaction/SignalFront/FormalAssociationCapability、typed pulse observation、PlotKind/Grid cell contract 和 DoD tests 的 public contract。
- 本步不为了旧测试保留任何 alias。

### M1：Point domain 一次性切换（一个不可拆分的合并单元）

- 在 `zlc_data` 建立 PointTable/GridTopology/AxisSourceRef/point row selection/唯一 resolve_point_rows；同一 dependency cut 在 `zlc_frontend` 建唯一 resolve_view_point_projection，FitSpec/Result batch descriptor 同步迁移 typed source。
- 同一个 dependency-closed change 中迁移 codec、DatasetBuilder、MonitorDataset、selection/transform/Fit、artifact schema、frontend evaluator，以及 Capture/PulseScan/MOT/Readout/Calibration/Occupancy producer/consumer。
- 同一 cut 删除被替代的 Dataset point 表达及其 readers/tests。

### M2：Signal transaction

- publication 时创建 ProducerTransactionId并原子发布 sibling bundle；建立 explicit parent graph的SignalFront coherent promotion。
- 实现data-owned EmissionSlotId/PointEmissionMap、neutral-owned FormalAssociationCapability与fixed cardinality reservation；Processor/selector/Fit显式传播parent并声明association语义。
- Workbench/application实现PresentedSignalPublication prepare/commit与atomic PresentedSignalFront pairing；Area/Cross continuous routes、Fit EVENT_RESULT route和SiteMap FINAL都从安装起配对exact presentation。删除mutable presentation side index双生命周期、tick内topology重建与全部retrospective retainer/name reconstruction，但保留same-component coherent present产品语义。
...
- 删除grid-specific renderer/preferences与frontend GUI-specific Fit lane，建立composition-owned generic operation runner；LiveFitSession与SnapshotFitInvocation按surface分离，删除Fit隐式Hold。实现regular raster problem和Fit/render/selector三类调度边界，overlay painter/blit不重画base。
...
- 实现generic CommandContext.start_and_wait + 唯一ExperimentRunAdmission；ResourceArbiter一次返回完整AdmissionConflictSet，application沿exact signal graph求retirement closure，按effective preemptibility全退或全不退，再对原plan一次最终admission。删除TaskConsole conflict scanner与board-wide task lock；public facade只委托narrow ports/Workbench opener，release-recapture helper只留family内。
...
1. 生产代码、public surface、codec、文档与current tests中不存在任何被替代的数据布局、repeat enum、专用grid/Fit renderer、可变presentation side index、GUI冲突裁决、重复display form、numeric文本sentinel、pseudo task、retrospective transaction、持久commit side authority、中央concrete catalog或fake catalog state；死symbol、alias、reader、wrapper与目录均为零。
...
4. selector/Fit/Processor都携exact parent；neutral SignalFront保证raw+derived connected panels只原子提升完整frontier，siblings不拆、无关producer可独立前进。one-to-one/fixed-fanout/fan-in的FormalAssociationCapability判定唯一且PulseScan不靠heuristic；FixedPublicationEvents扩展有grid的base table时slot dimension保持row_to_cell injective。
...
6. Camera live shape固定`(1,1,*frame)`，finite从progress到FINAL固定`(K,1,*frame)`并以validity表示未完成R；history不影响shape且dtype保持`uint8`。N=3的frame_0/1/2同transaction。MOT Ready→Running/live/FINAL，shape`(R,343,1)`、topology`7×7×7`且不O(N²)。Calibration恰有两linked flat RunId、command host无RunId；Occupancy可显式加载已有CalibrationArtifactRef且current ref只预填。Camera(camera live)+MOT并行；Camera(mot_camera live)+MOT只退休该monitor；多资源冲突一次返回完整集合、全退后对同一plan只admit一次；finite/nonpreemptible/外部owner不被自动停止。若camera monitor正供FormalPulseScan使用则不被抢占；纯continuous downstream则随retirement closure正常退出。cancel/post-FINAL warning正确。
...
最终架构不需要dense multidimensional ndarray、异步工作流编排器、硬件重构、persistent safety quarantine、软件内存预算、为Fit预设的独立进程或第二套Figure/form renderer。真正必须做的是：以PointTable修复point identity，以ProducerTransaction和原子PresentedSignalPublication修复same-shot及presentation lineage，以canonical ViewSpec/explicit FitSpec和surface-scoped live/snapshot session修复显示与权威边界，以regular-raster problem修复大图Fit复杂度，以ExperimentRunAdmission+post-SAFE lease release修复误reject，并把SAFE、progress、geometry、storage path、form projection和leaf binding放回唯一owner。
