...
6. W3a 先交付当前已有 consumer 的 Capture/identity-Fit notebook surface；ScanArtifact 的 transformed output 不能伪装成 raw OwnedSnapshot，必须在下一 Scan application slice 由 scan owner 定义 output snapshot identity、validity 与峰值内存后再接。任意 committed-transform fit 的忠实 observations display 同理需要一个真实 transformed-display consumer 才扩 FigureEvaluator 输入；在此之前明确拒绝 overlay，不建立通用 transformed-dataset 框架。
...
Rule-6机械海拔以新增`zlc_neutral_atom/scan/*.py`和13个既有integration owner的净diff为口径：scan package为`1278 physical / 1135 nonblank`，integration净增`702 physical`，合计`1980 physical`；main窄等价`pulse_scan.py + mot_field.py`为`703 physical / 615 nonblank`，physical ratio为`2.817x`，低于约3倍门槛。增量主要是旧树没有的target-bound repeat展开、named sparse axes、durable current-only intent/CAS recovery、commit visibility reconciliation和artifact materialization；终审又删除了零消费者`batch_axes/output_data_axes/repeat_axis/input_schema_fingerprint/point_table` aliases、已由CaptureArtifact/binder owner保证的trigger/frame/join/layout重复校验和singleton event selection的整数组boolean copy。后者把当前样本materialize的tracemalloc峰从`1,319,095`降至`989,431 bytes`（values为`393,216 bytes`）。公开参数明确为`capture_memory_limit_bytes`，只承诺raw Capture owner预算；离线transform OOM不改变任何已提交artifact并可重试，所以通用operation-aware总峰预算暂不在scan复制公式。若未来GUI worker或Formal flat Run声称total bounded，必须先由zlc_data transform owner提供retained/scratch estimate，再由composition与capture owner estimate合并并在大分配前拒绝；在此之前该产品面保持NO-GO。
...
最新实现 checkpoint：W3a 的静态 Capture/identity-Fit DataFigure 已完成代码与新增白名单纵切，下一步是 current Scan application boundary。public DataFigure 对每个 overlay 先把 exact source ref/schema 委托给 zlc_data 唯一 `validate_fit_result_source_binding()`，不在 frontend 复制 fit/batch partition 证明；renderer 只按具名 facet/batch/resolution 地址查询 `FitResultBatch.batch_layout`，sparse logical hole 显式标为 `NOT_PRESENT`，不会拿相邻 storage row 补位，component-invalid/per-cell failure 也不会调用模型求值。任何 display reduction 都把 method、AxisId 与实际 contributor min/max 持续显示在单 series 标题或多 series legend，默认 repeat mean 不得成为隐形物理操作。W3a 不声称 FigureArtifact persistence、live selector/session、ScanArtifact display 或 transformed-fit observations display 已交付；这些边界分别跟随真实 save/live/scan/transform consumer，而不是由 notebook surface 提前造通用机器。Rule-6 按完整 current Figure owner 面（model/evaluate/contract/suggest/render/DataFigure/Agg renderer）计为 `4013 physical / 3593 nonblank`，相对 main 旧综合 `DataFigure` 的 `1518 / 1369` 为 `2.64x / 2.62x`；未超过约三倍门，增量对应 renderer-free 具名轴求值、ComponentValidity、batch fit 与 caller-owned Agg，而不是 wrapper/兼容层。
