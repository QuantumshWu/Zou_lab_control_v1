...
4. 逐条迁 temperature、MOT、readout、device manager 和 notebook convenience；W4a完成current artifact的冻结`.figure_gui()`，W4b完成current calibration artifact/report的冻结只读报告，W4c完成current occupancy artifact按单一真实output选择的冻结Figure/GUI，W4d/W4e完成具名exact cell的同shot原帧+物理occupancy三态图与离散导航，M1完成virtual free-running MOT raw IMAGE monitor，M2a完成显式typed ROI、短bounded raw history和同snapshot IMAGE+CURVE双panel；长scalar derived history、histogram、动态selector/overlay、通用multipanel、交互fit/edit、可zoom/export的live calibration/occupancy grid、saved gallery、real Pylon qualification与其它convenience仍按各自纵切迁移；每条都按纵向切片删除自己的旧路径。
...
| `U01_GUI_WORKBENCH` | frontend render、Workbench shell、旧frontend与`pulse_gui.py` launcher | 17 | COMPLETE：existing primary paths及最新M2a差量复审闭合 | board identity/final gate、virtual/offscreen single-event finite capture及显式ROI coherent双panel产品IMPLEMENTED；通用scheduler/PanelHost、动态selector、长scalar rolling/grid/overlay与legacy GUI迁移BACKLOG/NO-GO |
...
|---|---|---|---|
| `MonitorTap -> MonitorDataset` bounded live | Workbench live image、rolling curve、histogram、board coherent present | W1已接finite preview；M1/M2b已把独立virtual free-running MOT camera、raw IMAGE、显式typed ROI scalar、独立长scalar history与raw/scalar同源coherent board接入public notebook/Qt，并保留sensor clock、compiled coil physics、EventRef/control lineage、ComponentValidity与Stop撤屏；real Pylon qualification、动态selector/control、histogram/meter及剩余legacy panel仍待M2/S5 | 同等 GUI capability 已由更小唯一 owner 交付并迁完全部 panel |
...
**最新实现 checkpoint：M2b typed ROI scalar event + independent long history完成。** public request现在一次冻结raw `history_capacity`（默认8）、显式rectangle `Selection`、`MEAN|SUM`、`ValidityPolicy`与独立`scalar_history_capacity`（默认300）；有request时`.camera_monitor_gui(request, ...)`拒绝任何覆盖option，无request时所有option只经`.camera_monitor_request()`构造，inspect、prepare与初始UI topology消费同一个immutable request。当前ROI binding只接受恰好一根具名`SPATIAL_Y`和一根具名`SPATIAL_X`及两条连续range；其它或额外`data_shape`轴在arm前拒绝，绝不按rank/singleton猜角色。raw schema始终保留`(R=1, P=Hraw, *data_shape)`；只有这个显式物理reducer消费声明的两条空间轴后，derived scalar schema才成为`(R=1, P=Hscalar)`，因此不是把任意多维`data_dim`重排成三项array，也没有`flatten`、`[:,0]`、隐式trailing mean或display-to-authority升级。

运行数据面只有一次相机read与一次raw publish：`AcquisitionStream`的第一条bounded tap进入raw `MonitorDataset`，第二条独占tap由同一acquisition lane上的窄`RoiScalarStreamProjection`按source顺序同步消费，再发布带direct causation的新scalar event到独立stream/tap/`MonitorDataset`。它不是通用async processor engine，也没有第二线程、scheduler或UI `np.roll`。每条scalar metadata冻结完整source `EventRef`、camera metadata、source missed、control revision与binding fingerprint；TraceContext继承run/correlation并把source ref列为直接因果。一个application-owned `RLock`覆盖raw ingest、ROI计算、scalar publish/ingest及raw+scalar materialize；`CameraMonitorSnapshot`再验证scalar head的source ref就是当前raw head，所以Workbench不能分别读取两个“latest”后拼出错位panel。
...
M2b刻意没有预建dynamic selector、revisioned ControlTopic/terminal ACK、history reset/generation、histogram、METER、Formal scan-y、Fit/Save或real qCMOS路径。下一最小纵切只能让真实histogram/meter消费现有scalar history，或让鼠标selector以revisioned control建立新binding/history generation；selection进入权威scan/fit时仍须重新构造对应authority spec。legacy `RoiProcessor`、`LiveLive/LiveLiveDis`、TaskConsole rolling/histogram与camera producer还拥有MAX/crop/scatter/value ROI、dynamic selector、side histogram、scan-y及其它未迁consumer，本轮不提前删除；dependency-closed删除的只是current M2a raw-history display-reduction路径，不保留第二套实现。
