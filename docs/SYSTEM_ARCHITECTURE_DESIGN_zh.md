...
当前完成两块仍有限定的S1子集：S1#1的adapter-neutral record/working-point software seam已经把endpoint对`VirtualCamera`的硬编码移除，非Virtual contract fake可贯通exact command boundary，且virtual capability/artifact identity保持不变；但camera owner I/O lane、real AssetMap/Q0/tail evidence与最终DCAM迁移均未交付，所以real qCMOS仍NO-GO。S1#3的virtual/component子集允许finite exact source附着capacity-one preview，纯FigureEvaluator/GRAY8、单panel BoardController与可选Qt presenter的手写oracle已闭合；仍没有真实app composition call site、overlay/viewport、save、free-running monitor或旧camera panel替换，所以S0.5与S1均不得标COMPLETE，也不得据此删除任何旧GUI producer/consumer。
