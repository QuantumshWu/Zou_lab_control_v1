...
|---|---|---|---|
| `MonitorTap -> MonitorDataset` bounded live | Workbench live image、rolling curve、histogram、board coherent present | W1已接finite preview；M1已把独立virtual free-running MOT camera的capacity-one raw IMAGE接入public notebook/Qt，并保留sensor clock、compiled coil physics、atomic head/EventRef/coverage与Stop撤屏；real Pylon qualification及ROI/scalar/history/histogram/multipanel/selector仍待M2/S5 | 同等 GUI capability 已由更小唯一 owner 交付并迁完全部 panel |
