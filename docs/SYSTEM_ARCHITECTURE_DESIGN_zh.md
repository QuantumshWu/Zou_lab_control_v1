...
设备参数的语义边界必须保持可见：相机 leaf 只声明设备身份和真实硬件工作点，不声明 FPGA raw trigger lane。相机连接时从 sequencer 发布的 PulseTarget manifest 按稳定的语义 endpoint label 解析物理 lane，再发布窄的`CAPABILITY_READOUT_BINDING`（camera instance、sequencer instance、已解析 trigger channel）；该 runtime binding 不携带 lattice grid、site centers 或任何 Calibration 数据。`grid_rows/grid_columns` 是 Calibration Task 的 authoring intent，site centers 是 Calibration 的检测结果/可选 admission 先验，不能出现在 camera Device Manager schema 或 hardware installation template。采集/传输 timeout 是 adapter/RPC 内部的等待策略，不是用户硬件工作点，也不能作为设备卡的伪硬件旋钮。这样新增相机型号不会把实验几何、Target 接线和软件策略重新塞回设备 leaf。
