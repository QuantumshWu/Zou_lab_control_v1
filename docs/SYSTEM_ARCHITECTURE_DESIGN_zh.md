...
Experiment                         # notebook/application facade，不含领域算法
  ...
  .readout.occupancy_cell_gui(occupancy_ref, selection=...) # 精确同shot物理位点图
...
4. 逐条迁 temperature、MOT、readout、device manager 和 notebook convenience；W4a完成current artifact的冻结`.figure_gui()`，W4b完成current calibration artifact/report的冻结只读报告，W4c完成current occupancy artifact按单一真实output选择的冻结Figure/GUI，W4d完成具名exact cell的同shot原帧+物理occupancy三态图；rolling/overlay、交互fit/edit、cell导航/selector、可zoom/export的live calibration/occupancy grid、saved gallery与其它convenience仍按各自纵切迁移；每条都按纵向切片删除自己的旧路径。
