...
| `U01_GUI_WORKBENCH` | frontend render、Workbench shell、旧frontend与`pulse_gui.py` launcher | 17 | COMPLETE：existing primary paths及最新W5差量复审闭合 | board identity/final gate、virtual/offscreen single-event finite capture、显式ROI scalar history、coherent四panel、fixed-ROI front-bound selector/overlay及raw committed-capture Fit editor/draft/Save产品IMPLEMENTED；raw-only首ROI、TaskConsole/scan/grid Fit、通用scheduler/PanelHost、rolling grid与legacy GUI迁移BACKLOG/NO-GO |
