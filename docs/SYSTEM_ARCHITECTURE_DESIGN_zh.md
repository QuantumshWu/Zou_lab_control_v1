...
| `UX-003` | figure viewer 载入后是可交互 panel，可 zoom/pan、重新 fit 与导出；GridPlot 可 focus/返回 | U0.3a-g 已把单panel HISTOGRAM/CURVE/IMAGE/METER、fit-bearing CURVE/IMAGE，以及单layer METER与HISTOGRAM多cell overview/focus接入同一 typed board；range/cross/zoom/pan/home、relim/cmap/limits、Setting/Edit、原子导出、即时 fit overlay 已恢复。exact saved-fit GridPlot、METER grid与SITE-faceted occupancy counts Histogram都不暗选第一格，只在显式命中真实cell后focus，并从缓存返回overview；稀疏hole保持原逻辑位置。尚未闭合的是multi-layer DataFigure、faceted CURVE/IMAGE及非Fit的其它archive plot kind；报告类多页仍是 frozen raster | 继续按真实consumer逐个恢复typed交互面；typed资格不足时保留完整whole-figure renderer，不以丢fit/轴/series换交互，也不先造通用dashboard框架 | **待用户批准 / 默认必须继续修复**：剩余multi-layer、faceted CURVE/IMAGE与archive plot kind；U0.3a-g及W7已恢复的typed交互、explicit-cell focus/Refit与导出不得回退，报告类多页至少补zoom |
...
| `UX-006` | calibration/site-map/occupancy 图沿统一交互 viewer/GridPlot 获得 focus、selector、zoom 与 export | U0.3f已让单layer SITE-faceted occupancy METER沿统一Figure获得同draw overview、exact-cell focus、缓存返回和两种原子PNG导出；U0.3g又让真实`OccupancyArtifactRef(counts)`的SITE-faceted HISTOGRAM复用同一外壳，并恢复共享bin-domain/count scale、standalone Histogram交互与Back/Esc。两者都不暗选第一site，不丢component validity、sample、dropped count或单位。calibration report、site-map图与其它occupancy图仍未闭合，未提交工作树不能作为证据 | 保留已完成的METER/HISTOGRAM grid；其余路径先闭合immutable report/exact-address与预算/线程所有权，再按真实surface接入既有selector/viewer | **待用户批准 / 默认必须修复**：报告类多页至少zoom；calibration/site-map与剩余occupancy路径按旧surface补适用的focus/selector/export，或逐项提交用户裁决 |
...
**U0.3g实现checkpoint（`GO`）：** `DataFigure.focused_typed_panel()`现在是METER/HISTOGRAM唯一focus派生owner：它以same-draw `Selection`和显式expected intent复核地址，把FACET固定为display-only `FixedIndex`，复用exact series/input/revision且不resolve/re-evaluate；旧METER专名API及alias为零。Workbench只在单layer、单input、同unit、无fit的封闭集合开启typed grid，overview没有默认cell，空白/hole不命中，Back/Esc只恢复缓存front；multi-layer、faceted CURVE/IMAGE仍走完整whole-figure fallback。
