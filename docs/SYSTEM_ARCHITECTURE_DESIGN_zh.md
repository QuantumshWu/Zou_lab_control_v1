...
4. 逐条迁 temperature、MOT、readout、device manager 和 notebook convenience；W4a完成current artifact的冻结`.figure_gui()`，W4b完成current calibration artifact/report的冻结只读报告，W4c完成current occupancy artifact按单一真实output选择的冻结Figure/GUI；rolling/overlay、交互fit/edit、可zoom/export的live calibration/occupancy grid、saved gallery与其它convenience仍按各自纵切迁移；每条都按纵向切片删除自己的旧路径。
