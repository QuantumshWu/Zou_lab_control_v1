...
|---|---|---|---|
| `MonitorTap -> MonitorDataset` bounded live | Workbench live image、rolling curve、histogram、board coherent present | tap 只拥有 admitted backlog；materializer 拥有 keyed cycle/append window、head/EventRef 与 atomic snapshot | 同等 GUI capability 已由更小唯一 owner 交付并迁完全部 panel |
...
规则状态必须诚实记录为 `NOT_STARTED/PARTIAL/COMPLETE`。§22.1固定的11个互斥production owner闭包已经逐一完成，规则1–8的**existing-code追溯审查轴全部COMPLETE**，所以“完成1–7前不得继续迁移”的冻结已经解除；这不等于implementation全部READY。当前明确为GO的是virtual finite capture/pulse owner、BoardController coherence foundation、S3a“committed capture -> calibration ref -> artifact/report reopen”、S3b“committed capture + calibration ref -> occupancy ref -> reopen”和S3c“installation-qualified short sitemap -> raw FINAL -> calibration FINAL/recoverable failure”的virtual/offline纵切。S3a–S3c结案证据包括完整preflight→execute→FINAL→admit current-format手工oracle、1-byte预算与raw recovery、fresh-process notebook不加载SciPy/Qt/Matplotlib、exact `(R,P,*data_shape)`/`(R,P,site)`/component-validity检查、repository close与run borrow生命周期、static DAG/shape/旧Registry扫描，以及每个切片的两路独立对抗审查均`P0=0/P1=0/GO`；没有运行历史pytest。remote/real qCMOS、正式scan物理资格、完整GUI scheduler/render接线、GUI site-map/gridplot和普通用户真实设备入口仍分别NO-GO。每个后续dependency-closed切片仍须按规则1–8做增量审查，完整迁移结束后还必须对最终分支做一次独立对抗验收；局部GO或owner audit COMPLETE都不能改写成全局实现完成。
