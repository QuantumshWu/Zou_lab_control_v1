...
当前W3c只兑现S4中不依赖Q0/real composition的source-neutral软件纵切：direct-camera与camera→occupancy都复用现有一次arm/一次autonomous FIRE exact pipeline，在同一Run提交唯一ScanArtifact；`(R,P,*data_shape)`、PointLayout、ComponentValidity和processor/calibration lineage可重开，final-only Qt panel只接冻结request、RunHandle snapshot与immutable PNG。它明确标为virtual/offline、FINAL-only，不冒充Formal EndAttestation、progressive LiveSweepMonitor或完整TaskConsole editor。真实hardware/Q0/terminal evidence gate仍NO-GO，旧PulseScanNode/TaskConsole consumer也因此尚未删除。
...
| current `ScanArtifact -> DataFigure/FINAL-only panel` | notebook direct/occupancy scan inspect/render、current final-only Qt；后续progressive TaskConsole与scan-fit | W3c已交付source-neutral exact pipeline、processor y、stable output和final-only panel；不得把它夸成LiveSweepMonitor/完整TaskConsole | progressive panel/edit/catalog、scan-fit及最后legacy consumer各自交付后，才dependency-closed删除对应旧路 |
...
最新实现 checkpoint：W3c current FINAL-only ScanWorkbench foundation代码与两项新增白名单纵切已经完成，本段与闭包代码由同一显式提交冻结。public notebook支持direct-camera和camera→occupancy两种source-neutral autonomous request，并可从唯一ScanArtifactRef取得lightweight FigureDocument、bounded DataFigure或FINAL-only Qt raster；scan owner提供同Run exact source/provenance、稳定output identity、pre-FIRE static lineage、一次transform freeze、具名component validity与current-only reopen。frontend仍不认识neutral repository，Qt不接触Port/RunPlan/raw ndarray，notebook不构造BlockId、不reshape PointLayout、不转换RowComponentValidity。W3a Capture/identity-Fit边界保持不变，`fit(scan_ref)`、FigureArtifact persistence、progressive LiveSweepMonitor、selector session与transformed-fit observations display仍明确deferred。

本checkpoint**不是完整TaskConsole SCAN_SLOT结案**。下一dependency-closed切片仍需迁移progressive/live scan panel、Setting/Edit/catalog与真实旧external measurement/processor y编辑面；在最后consumer迁走前不得删除PulseScanNode、MeasurementSpec或TaskConsole。当前闭包证据为12项current scan白名单通过、fresh-process reopen、direct/occupancy值/轴/validity/lineage、纯`inspect_scan`不stage、低预算在base hardware compile前拒绝、metadata-only禁止pulse IR/document/data materialize、render/Qt raster cap、stale-raster与close race，以及语法/import/shape/mode/owner机械扫描。Rule-6当前slice为`+2478/+2236`、对main窄等价`2.153x`，累计`4699/1854=2.535x`。唯一一轮独立对抗审查先判`P0=0/P1=3/NO-GO`，三个同根资源洞全部闭合后由本地逐项反例复核为`P0=0/P1=0/GO`；真实qCMOS Formal仍因Q0/EndAttestation保持typed NO-GO，RTL/bitstream未改。
