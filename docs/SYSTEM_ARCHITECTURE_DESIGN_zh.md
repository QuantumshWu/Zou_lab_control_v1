...
Experiment                         # notebook/application facade，不含领域算法
  ...
  .figure_gui(result_or_ref)        # 懒加载 notebook[workbench]，冻结结果只读显示
...
4. 逐条迁 temperature、MOT、readout、device manager 和 notebook convenience；W4a完成current artifact的冻结`.figure_gui()`，W4b完成current calibration artifact/report的冻结只读报告；rolling/overlay、交互fit/edit、可zoom/export的live calibration grid、saved gallery与其它convenience仍按各自纵切迁移；每条都按纵向切片删除自己的旧路径。
...
当前tracked W4 oracle为`13 passed`：真实5×7/35-site、3-model calibration做paired reopen；逐array对齐、有限invalid filler、unusable NaN threshold、equal quick/formal但gate关闭、四种非方形GridOrder、aggregate source+render budget、Matplotlib lane release、worker/Qt thread ownership、五页atomic present、关闭后Experiment可继续使用，以及全部W4a regression均通过。另有owned-file compile、diff-check、fresh-process headless import、frontend reverse-import与shape-loss机械扫描；一次有界两路对抗审查最初发现5个P1并逐项关闭，限定复核为`P0=0/P1=0/GO`。本局部纵切可以提交；整个迁移仍未完成。下一consumer从Git与本节最新checkpoint恢复后，优先闭合current `OccupancyArtifactRef`的site/component数据到冻结Figure/Workbench路径；若实际consumer矩阵显示另一条依赖边更早，则按最后consumer与纵切准则调整，不重开已经完成的W4a/W4b审查。
