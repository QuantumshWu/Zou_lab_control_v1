...
| `UX-002` | live plot 的 selector、zoom/pan、十字、hover 读值、relim 与 cmap/limits 覆盖旧 plot contract；拖动只冻结当前 panel，其它 panel 不停流，松手补拍 | U0.2a 已让 camera rectangle 改为 Qt per-panel hold，whole-live pause 已删除；仍缺完整 2D A/C/Z/H、其它 plot kind、ViewportTransform 交互与 Setting/Edit display state | 早期窄 leaf 被误当作可长期冻结产品面 | **待用户批准 / 默认必须修复**：所有 live plot kind 的非空交互句柄合同；per-panel interaction freeze/owed beat；三种 relim 与 cmap/lim E2E |
