...
| `UX-002` | live plot 的 selector、zoom/pan、十字、hover 读值、relim 与 cmap/limits 覆盖旧 plot contract；拖动只冻结当前 panel，其它 panel 不停流，松手补拍。旧 1D/Monitor 的 Area 实际画二维矩形却只消费 x，纯水平拖会被误清 | U0.2b 已在 free-running camera monitor IMAGE 恢复完整 A/C/Z/H/hover、ViewportTransform、三种 relim、六种 cmap、同源 Setting/Edit 与 per-panel hold；U0.2c Monitor commit 又在同一 board 恢复 rolling CURVE 的原生 x-span、continuous cross、真实 sample hover、x-only zoom/pan/home、三态 y relim、同源 Setting/Edit 与 panel-local hold。finite capture、progressive SCAN_POINT 1D、Sites、Histogram、Grid 及真实 launcher 的全 plot-kind handle 仍未闭合 | 早期窄 leaf 被误当作可长期冻结产品面；旧 1D selector 的视觉维度与实际 authority 维度不一致 | **待用户批准 / 默认必须继续修复**：finite capture 与剩余 plot kind 的非空交互句柄、各自 relim/axis 语义、grid focus 与 launcher E2E；rolling Monitor 已用原生 x-span（视觉带铺满 y）保留 exact x selection，progressive SCAN_POINT 必须复用同一实现后再闭合；free-running camera IMAGE/CURVE 已通过的行为不得回退 |
| `UX-003` | figure viewer 载入后是可交互 panel，可 zoom/pan、重新 fit 与导出；GridPlot 可 focus/返回 | W4 generic figure 与部分 artifact 查看器冻结为 PNG/display-only；W7 只有 saved-fit focus/export，未成为通用交互 viewer/重 fit 入口 | 以 immutable raster 先闭合线程/内存所有权 | **待用户批准 / 默认必须修复**：通用 figure/archive viewer 恢复交互、re-fit、export；报告类多页可保留 frozen raster，但必须可 zoom |
| `UX-004` | plot/panel 内直接发起 fit，框选与 fit selection 联动，普通 Fit 一步生效；2D box ROI fit 可见 | 当前主 UX 依赖独立 `fit_gui`，TaskConsole `Add Analysis -> Fit` / `Analyze -> Fit`、SelectionCandidate 与 2D box fit 尚未闭合 | 先交付了 typed solver/editor 与 exact artifact 路径 | **待用户批准 / 默认必须修复**：§12.6 两入口、一键 authority draft、框选预填、2D 双空间轴 fit 与即时 overlay E2E |
...
| `UX-006` | calibration/site-map/occupancy 图沿统一交互 viewer/GridPlot 获得 focus、selector、zoom 与 export | 已提交 calibration report 与 exact occupancy cell viewer 是静态 frozen raster；未提交工作树不能作为闭合证据 | 先闭合 immutable report/exact-address 与预算/线程所有权 | **待用户批准 / 默认必须修复**：报告类多页至少 zoom；通用 calibration/occupancy 查看路径按旧 surface 补适用的 focus/selector/export，或逐项提交用户裁决 |
...
FitResultBatch 的 canonical payload codec 属于 `zlc_data`，但 durable identity 必须由具体 artifact owner 持有。当前唯一 durable 用例是 neutral-owned committed `CaptureArtifact -> FitResultBatch`，所以 `CaptureFitResultArtifactRef` 与 `CaptureFitResultRepository` 属于 `zlc_neutral_atom`；repository 只绑定 admitted capture lineage、data-owned result payload 与 `zlc_storage`，不定义 Fit 算法、Processor 或通用 Analysis repository。出现第二种真实非 Capture source 后再判断是否存在共同 adapter；baseline 不建立 generic `FitResultArtifactRef`。`fit.save()` 不要求 frontend，也不把通用 fit 伪装成 neutral 领域 Analysis。Figure 保存仍使用 frontend-owned FigureArtifactRef；二者是不同 artifact kind。
...
Experiment                         # notebook/application facade，不含领域算法
  ...
  .fit_gui(capture_ref, model=None, committed_transform=None) # typed draft overlay + explicit Save
  .load_fit(CaptureFitResultArtifactRef) -> AdmittedCaptureFitResult
  ...
  .figure_gui(result_or_ref, occupancy_output=None)      # 冻结只读显示；saved-fit默认进入exact GridPlot explorer
...
W5交付第一个current交互Fit产品：`.fit_gui(CaptureArtifactRef, model=None)`从已提交raw capture开始，后台从同一schema和closed model catalog生成可用模型，以共享form DSL编辑显式parameter seed/bounds/fixed值；W5首切片拒绝`CommittedTransform`，W8b随后只为W8a已经证明可忠实显示的selection-only子集增加显式`committed_transform=`参数。raw路径仍以`None`逐值通过同一gate；transformed路径要求调用者先提交权威transform，Workbench没有ROI/transform editor，也不读取当前ViewSpec、selector、viewport或display reducer。`FitSpec`冻结具名fit axes，所有其余repeat/point/data axes原样成为batch axes；Qt分别显示不可编辑的authority摘要和axis name/AxisId/role/size/unit，并显示每个batch cell的status/quality。Fit使用独立capacity-one worker lane，普通冻结Figure继续使用原lane；两者只在既有Agg render owner处短暂串行，不让长solver占住Figure materialization。

上述“调用者先提供 transform、Workbench 不读取 selector”只约束独立 `fit_gui` 直达入口，不能约束 TaskConsole/figure 的主 Fit UX。主路径必须由当前 selector 产生 `SelectionCandidate`，经 authority-side `suggest_fit_draft` 预填，并由普通 Fit 动作一键提交；display state 本身仍不能无动作越权成为 authority。

`FitExecution`与唯一未保存draft由headless `CaptureFitDraftAuthority`独占；Qt只得到不可保存的`FitDraftResult`和immutable raster。draft overlay把不可保存的`FitResultBatch`与窗口创建时冻结的`CaptureArtifactRef`重新组合，并在物化前调用data owner的完整source/schema/fit-axis/batch-layout binding validator；不存在把execution交给arbitrary projector的回调。只有显式`Save Fit Result`才通过notebook composition提供的lifecycle-gated save command发布neutral-owned`CaptureFitResultArtifactRef`，随后必须从该exact ref重开overlay；Clear只撤draft/画面，不删除已发布artifact。Save一旦返回ref，后续reopen/render失败或Close请求都不能吞掉ref；Save进行中Close明确defer。

长Fit只在开始/结束时短持Experiment lock并登记一个fit-specific active count，不建立通用workflow/lease framework。Close若与active Fit竞争，原子进入`CLOSING`后fail-fast，禁止新Fit/preview/save；在途Fit退出时转为`FitCancelled`而不能接纳draft，调用者在排空后重试Close，与现有active Run的显式retry语义一致。已进入bounded capture decode不能被强杀；deadline/cancel在lane等待、solver前后和Qt接纳处诚实检查，decode阶段只诚实排空。该产品已经支持调用者显式提供、selection-only且可逐值忠实预览的authoritative ROI transform，但仍不等于ROI/transform editor、TaskConsole `Add Analysis -> Fit`、scan-y fit、live/grid batch editor或calibration edit；这些继续按真实consumer迁移。
...
`figure_document` 由 notebook composition 的 result projector 生成，避免 neutral_atom 反向依赖 frontend.figure。FigureDocument 故意不含 repository/ref/resolver，单独只能编码、检查或编辑 presentation；实际 evaluate/render 必须另给同 revision 的 `ResolvedDatasetMap`。`.figure()` 在 composition guard 内准入并物化 source，退出 guard 后才把 document、snapshot map 与可选 FitResultBatch 交给 DataFigure。render extra 不可用时，采集、分析与 `figure_document` 仍完整工作。GUI launcher 不是 headless notebook baseline；只有安装 `notebook[workbench]` 时 `.task_console()`/`.pulse_gui()`/`.figure_gui()`/`.fit_gui()`/`.readout.calibration_report_gui()`/`.readout.calibration_gui()`/`.readout.calibration_edit_gui()`/`.readout.occupancy_cell_gui()`/`.readout.camera_monitor_gui()` 才通过单向 optional edge 调用 workbench composition，workbench 不反向 import notebook facade。W4a只接收绑定后的`.figure` callable；W4b只接收绑定后的paired-computation loader、typed ref与总预算；W4e只接收绑定后的metadata-navigation loader、exact-cell loader、typed ref、初始Selection与同一个总预算；W5/W8b只接收raw/saved figure factory、不可保存draft-result figure factory、fit prepare/execute/save三个窄command、固定Capture ref与可选的已冻结CommittedTransform，source预览Selection只能由frontend对BoundFit执行W8a窄合同后取得；W6只接收paired-computation loader、formal Run starter、固定editor seed或exact Calibration ref，request/source/binding/budget/deadline由composition冻结；W7只接收一个worker-affine exact-ref view callable、typed fit ref与总预算，该callable内部一次准入并缓存immutable fit/source session，Workbench和Qt既不能取得repository/Experiment，也不能访问缓存数组；M1只接收绑定后的one-shot camera-monitor prepare callable。窗口都不接收/保存Experiment本体；关闭后同一Experiment仍可继续读取或运行。
...
fit_ref = fit.save()  # neutral-owned CaptureFitResultArtifactRef，不要求 renderer
...
analysis owner binder(DomainAnalysisDefinition key, config, typed input slots/refs) -> AnalysisStep
...
Analysis 不访问 Device、Hub/latest、QWidget 或 mutable DatasetBuilder。它消费 OwnedSnapshot（exact DatasetRevisionRef + frozen DataBlock）或 immutable artifact，产生 FitResultBatch、CalibrationArtifact、report 等 typed result。`FitSpec/BoundFit/FitResultBatch` 与 `BoundFit.run()` 全部由 zlc_data 拥有；Calibration/ReadoutFidelity 等带 neutral 物理语义的算法才使用 `DomainAnalysisDefinition`。Workbench 的 `Add Analysis` 只把明确的 zlc_data Fit capability 与 neutral DomainAnalysisDefinition 合并成只读 CatalogView，neutral 的 DefinitionCatalog 不重新注册或包装通用 fit。

neutral runtime 用一个无 Fit 语义的 `bind_analysis(bound_operation, DatasetInputSlot) -> AnalysisStep` 适配器托管执行；baseline 的 data-owned bound operation 只有 BoundFit。Pipeline 编译时 slot 只有 expected DatasetSchema；DatasetBuilder finalize 后，runtime 把该 slot恰好一次解析为携带冻结 block 与 exact revision ref 的 OwnedSnapshot，再调用 `BoundFit.run`。它负责 cancellation、compute lane、terminal propagation 与 result handoff，但不根据字符串 analysis id 选择算法，也不定义 zlc_data result schema。interactive/offline frontend 可以把同一个 BoundFit 与当前 OwnedSnapshot 投递到自己的 executor。依赖方向始终是 neutral/frontend -> zlc_data，不产生新的 fit owner，也不假装未来 dataset 在 bind 时已经存在。只有出现第二个确实共享此绑定合同的通用 Analysis 后，才把 `BoundOperation` 提升成公共泛型/Protocol；当前不为它预建 registry、descriptor hierarchy 或 program DSL。
...
FitResultBatch 是 compact solver-issued report，不是把原始坐标、observation、Jacobian 重复塞进去的 proof-carrying result。构造器/strict codec 验证状态机、静态 domain/constraint、计数、RSS、R²、covariance 的有限性/对称/PSD/fixed-row与 canonical zero；RMSE、effective schema fingerprint、coordinate source、parameter schema/unit 都由已保存的 FitSpec/AxisSpec/catalog 唯一派生，不在 payload 再存第二份真相。raw codec decode 只能得到 untrusted report；public direct 保存只接受 `CaptureFitResultRepository.execute()` 铸造的 process-local `FitExecution`，load 在 source re-admission 与 binding 校验后铸造 capture-specific、不可 replace/pickle/直接构造的 `AdmittedCaptureFitResult`。execution/admission capability 和 repository 都是 final、slotted、普通赋值不可变；repository 每次操作复核 root lease 与 content-store authority，防止标准 `dataclasses.replace` 或公开属性漂移把任意 raw result 洗成 durable authority。outer manifest 只含 current format、repository id、owner-encoded source CaptureArtifactRef 与 result ContentRef：CAS `ContentRef.digest` 是唯一 payload identity，FitResultBatch 不再自造 digest property。execute 路径会先按 repository 的 `materialization_memory_limit_bytes`（默认 512 MiB）把 admitted CaptureFrameSource 物化成 OwnedSnapshot，再进入约 48 MiB 默认 packed-observation budget；它当前不是 chunk-stream solver。load 则由 CaptureFrameSource 的 block/revision/schema 与 capture generation 构造 DatasetRevisionRef，再用 schema-only binding owner 交叉验证 source/spec/axes/layout/present counts；它不 materialize/重打包相机帧、不重跑 solver，也不从 parameters 反推一次历史数值执行。manifest/result blob 在读取前有固定 byte 上限，save 也拒绝发布自身无法重载的 payload；不再为没有实际 wire/persistence consumer 的 result-ref 加 standalone codec，也不预建 producer signature 或 journal。这里的 admission 信任边界是由 OS/process root lease 排他的本地 repository writer 加 CAS 内容完整性；若外部主体能绕过 API 直接改写 repository filesystem，则没有密钥/签名的本地 artifact 都不具备 producer attestation，必须按 untrusted repository 处理。
...
FitSpec 必须包含 input_schema_fingerprint 与显式 fit/batch axes；发生选择/降维时 committed_transform 必须存在，identity path 可以为空。Fit Analysis 只在 DatasetBuilder 完成 EOS/key/validity coverage 并冻结输入 revision 后运行，验证 schema fingerprint、解析 FitProblem 后执行相同的 zlc_data transform/reduction/fit 函数。它不在每个 sample/update 到达时把累计 DataBlock 重新拟合一遍。

Formal Fit step 返回完整 FitResultBatch；下游参数引用、校准更新、scan 决策或“成功物理结论”必须在自己的领域 AnalysisSpec 中解释 numeric status、covariance/counts/RSS/R²并声明哪些具名 batch 必须满足何种物理 policy。generic fit core 只报告数值事实，不能替领域 consumer 发明统一 acceptance。
...
interactive 只意味着 QoS/入口不同，不降低输入 integrity：若输入 DatasetRevisionRef 属于 Formal epoch 且仍为 PROVISIONAL，可以为即时观察运行临时 fit，但 overlay 必须带 `PROVISIONAL` 标记且不能保存为任何 source-specific authoritative fit artifact（当前只有 neutral capture-fit artifact）、不能成为后续 authority input。epoch 转 INVALID 时相关queued/running result按epoch lifetime token丢弃并从正常overlay撤销；只有独立 EpochValidationRecord 证明该revision为 VALID 后，才允许 materialize为正式派生结果。
...
精确物理cell overlay则明确不是普通单dataset Figure：它需要同一个occupancy cell、source capture cell与Calibration SiteMap三方事实，但不产生新的权威join artifact。notebook composition先用具名Selection解析唯一`DatasetCellAddress`，用该地址分别取得occupied/ComponentValidity和同shot frame，再投影成frontend-owned自包含view；renderer看不到neutral ref/repository，neutral也不导入frontend。这个view只能进入冻结`DISPLAY ONLY` raster，不能保存成Fit/Scan/Calibration输入；未来interactive selector只可在同一exact address/revision与ViewportTransform之上生成SelectionCandidate，不能把当前画面升级为权威选择。
...
`SelectionCandidate` 必须可由当前 panel/figure selector 直接产生并预填同一个 authority draft；框选后点普通 `Fit` 不再经过第二个确认窗口。1D range fit 与 2D box ROI fit 都是 baseline：后者显式绑定两根 `SPATIAL_X/SPATIAL_Y` fit axis，保留其余 repeat/point/data axes 为 batch，不得把二维 ROI flatten 成一条匿名轴。fit 求解在有界 worker 中运行，source/live panel 与其它 panel 继续响应；最新合法 draft 的结果按 revision 更新 overlay，旧 completion 不覆盖新 selection。
...
neutral_atom:  CaptureArtifactRef、CaptureFitResultArtifactRef、CalibrationArtifactRef、ScanArtifactRef
```

typed Ref 的 class identity 提供 artifact kind/format 语义；持久 repository ref 至少绑定 repository namespace 与 artifact content/manifest digest，不是任意 filesystem path，也不直接嵌入 mutable Python object。只有 Ref 确实跨 wire、manifest 或独立持久化边界时，值语义 owner 才提供对应 typed codec；没有该 consumer 的 `CaptureFitResultArtifactRef` 不预建 standalone codec。若正式 lineage 必须同时验证更高层 source authority，outer manifest/repository 位于能够单向依赖两侧的最窄 adapter（当前实例是 neutral 的 CaptureFitResultRepository），但不得复制 payload schema 或反向夺走算法所有权。Workbench 通过本地 ArtifactDescriptor adapters 聚合展示，不把 descriptor 反向泄漏给 owner。
...
- headless fit.save 返回 neutral-owned CaptureFitResultArtifactRef且不加载 frontend.render；figure_document 只需 frontend.figure，只有 figure()/GUI 需要 render/workbench extra；
...
|---|---|---|---|---|
| 2D | area + cross + zoom/pan + 双 horizontal clim line | wheel：down 放大、up 缩小；middle drag 双轴 pan；middle double-click 优先 zoom 到 area，否则 home；right click 固定 x/y/z，right double-click 清除 | relim 控 clim；cmap、x/y view pin、color limits | `MUST_CLOSE`；current 只有 camera pixel rectangle |
...
| 1D / Monitor | area + cross + zoom/pan | 只改 x view；固定 cross 显示 x/y | relim 控 y；x pin；无 cmap | camera rolling Monitor `MATCHED`；progressive SCAN_POINT `MUST_CLOSE` |
...
| 真实产品闭环 | camera ROI scalar 是唯一 rolling CURVE；progressive occupancy scan 是当前真实 SCAN_POINT 1D。两者必须消费同一 `CurveDisplayState/CurveViewportTransform/CurvePanelPayload` 与同一 worker-affine `SinglePanelAggRenderer`，但分别保留 rolling newest-first 与 finite scan point 的数据语义 | rolling Monitor `MATCHED`；progressive SCAN_POINT `MUST_CLOSE` |
| Selectors ON/OFF | 旧 dashboard 默认 OFF，但 Area/Cross/ZoomPan 三个handle已建立；ON/OFF原地park/arm，不重建panel、dataset、renderer或selection，OFF时wheel归board scroll。目标保留全局用户开关，但readiness/pending/fault按panel隔离：curve等待新revision不能禁掉仍合法的IMAGE | Monitor `MATCHED`；SCAN_POINT `MUST_CLOSE` |
| Area | 旧画 `[xmin,xmax,ymin,ymax]`，最终1D Selection只消费x且纯水平拖被误清。目标按 `UX-002` 建原生x-span：left drag仅要求非零x宽，overlay可铺满当前y绘图区；candidate仍是DISPLAY ONLY并携exact scalar input/axis/front origin，未来Fit只可由authority seam重新构造请求 | Monitor已实现；`LEDGERED_PENDING_APPROVAL UX-002`；SCAN_POINT `MUST_CLOSE` |
| Cross / hover | right click锁定连续cursor x/y，right double-click清除；cross不是sample snap。goal新增hover必须读真实值：瞬时hover吸附最近的valid sample并显示series，不得把pointer y伪装成信号值；因此front payload可**借用同一份immutable evaluated curve arrays**，不是复制或第二数据真相，并必须计入latest/held生命周期预算 | Monitor `MATCHED`；SCAN_POINT `MUST_CLOSE` |
| Zoom / pan / home | wheel down按`1/1.1` centered zoom-in，up按`1.1` zoom-out；middle drag只改x且以press pixel/frozen limits为基准；middle double-click有area时zoom-to-span，否则回当前home。home由本次完整声明x轴的finite numeric domain求得并随domain revision更新；Clear x pin立即回当前auto并继续follow，不能保留旧画面 | Monitor `MATCHED`；SCAN_POINT `MUST_CLOSE` |
| 轴与provenance | CURVE不能复用`ImageViewportTransform`的规则pixel/SPATIAL_X/Y/cell-snap语义。`EvaluatedAxis`必须保留owner `AxisSpec.role`，curve保留value unit；交互只接受有限numeric、严格单调递增或递减（含不规则）x，单点有确定性非退化home；非数值/非单调仍可静态显示但interaction fail-closed，绝不猜index轴 | Monitor history `MATCHED`；通用静态fallback与SCAN_POINT `MUST_CLOSE` |
| relim / limits | y不是viewport轴；只由共享closed `RelimMode(TIGHT/NORMAL/FIXED)`管理。tight=`min/max ±10% span`，常量5=`4.5..5.5`、常量0=`-0.1..0.1`；normal全非负=`0..1.2*max`且全零=`0..1`，出现负值使用tight数值但保留normal mode；fixed进入时冻结exact painted y limits。所有series的valid finite union共同决定y，空/invalid不伪造0、不回退旧样本；deadband baseline只在对应BoardFrame真正publish成功后推进，ROI同generation retarget也按document/control semantic identity重置 | Monitor `MATCHED`；SCAN_POINT `MUST_CLOSE` |
| worker / geometry | Figure/Axes/Artist继续只属render worker。worker严格按“更新全部series artist → 计算/应用x与y → draw → 冻结最终Agg axes bbox与actual limits → immutable raster+payload”执行；Qt使用draw后的axes bbox从Matplotlib bottom-origin换算到raster top-origin，不能把带标题/刻度/legend margin的整panel当plot rect | Monitor `MATCHED`；SCAN_POINT `MUST_CLOSE` |
| per-panel hold | 任一curve area/pan只保留该panel的immutable RGBA front、exact origin和同front evaluated arrays；底层完整BoardFrame、source和siblings照常前进，release直接露出latest retained dataset，不回放display frame。source/layout/presentation/axis/geometry/revision/fault/disable/hide/close都释放；同一时刻只允许一个pointer hold | Monitor `MATCHED`；SCAN_POINT `MUST_CLOSE` |
| Setting / Edit | curve唯一authored value是`CurveDisplayState(revision, relim_mode, x_view, fixed_y_limits)`；history capacity不在display state，扩大retention必须走source request。Image与Curve只共享已出现第二consumer的有限数值range/relim纯函数，以及一个参数化的revisioned Fluent form widget；不建立generic DisplayState、Viewport protocol、InteractionManager、payload plugin registry或RangePolicy类族。Setting/Edit各实例投影同一owner，dirty/stale/CAS与runtime placeholder不互抄状态 | Monitor `MATCHED`；SCAN_POINT `MUST_CLOSE` |
| 多维与多series | FigureEvaluator已解析的每个series/batch轴全部保留；禁止flatten、series 0、隐式mean或把trailing data axes塞成单个`data_dim`。交互curve要求全部series共享逐值相同的x axis，否则fail-closed；relim使用全部series，hover明确标series | Monitor `MATCHED`；SCAN_POINT `MUST_CLOSE` |
...
| 内存 | pre-arm显式合计evaluator、persistent Agg/artist、candidate、latest raster、Qt detach、display-revision overlap，以及一次pointer hold的最大`RGBA raster + exact evaluated curve arrays`；一个board同时只hold一个panel，因此各可交互panel的hold取max而非求和。模糊raster multiplier即使数值偶然覆盖也不能替代具名hold项；少一字节的反例必须在arm前拒绝 | Monitor `MATCHED`；SCAN_POINT须复测后 `MUST_CLOSE` |

**rolling Monitor 实现 checkpoint（`GO`；完整 U0.2c 仍为 `PARTIAL`）：** `CurveDisplayState`是camera Workbench唯一authored owner，`CurveViewportTransform`是worker draw后冻结的坐标投影，`CurvePanelPayload`把同一front的`EvaluatedInput`、全部series与draw geometry原子交给Qt；Qt与renderer都不反向修改display state。Image/Curve只共享`display_range.py`中的有限range/relim纯函数和直接参数化的`FluentRevisionedFormEditor`生命周期；旧的image-only editor文件已删除，没有保留alias、兼容wrapper或第二套Setting/Edit。`SinglePanelAggRenderer`仍是唯一worker-affine曲线renderer，Figure/Axes/Artist没有越过线程边界。

数据合同保持axis-total：`EvaluatedAxis`携owner声明的`axis_id/name/role/unit/indices/coordinates`，`EvaluatedCurve`保留value unit、逐sample validity与完整batch series；CURVE evaluator若仍有未处理的cell/data axis就fail-closed，绝不使用rank、singleton或length猜role，也没有flatten、`reshape(...)[0]`、series-0显示或隐式mean。交互producer还要求全部series共享exact x axis与value unit；relim消费全部series的valid finite union，hover在全部series中按drawn-pixel距离选择真实valid sample。当前`MONITOR_HISTORY`是有限严格单调numeric轴，因此可进入interactive path；非数值/非单调通用静态fallback和progressive轴资格属于下一提交，不能由本GO外推。

交互合同把三件事分开：Workbench/selector switch持board-wide用户意图，Qt分别持IMAGE/CURVE binding health，Workbench再按当前真正painted provenance原子下发per-family readiness。ROI同generation retarget、display revision pending或旧front retained时，stale CURVE在Qt leaf不产生intent、不记fault，合法IMAGE仍可操作；exact新front到达后CURVE自动恢复。Workbench的CURVE来源门同时核对display revision、BoardFrame/`LiveFrontStatus` sequence、binding fingerprint和control revision；range candidate只由Workbench持`(origin, x_span)`，Qt只画投影，任一完整curve semantic revision变化都清owner与投影。ControlTopic仍保留每次提交自己的revision/terminal/supersede语义，不能因值相同而吞命令。Escape、hide、deactivate、ungrab、disable、source/layout/axis/geometry变化都释放hold；合法但溢出的wheel/pan候选在Qt事件边界内局部拒绝，不能炸出event loop。

ROI retarget不重启camera source Run，不更换RunHandle/RunId、raw/scalar block、stream generation、slot、board topology或source tap；旧完整front保留到同一revision的current/curve/histogram/meter coherent front可发布。curve y deadband baseline只在对应BoardFrame成功publish后推进，binding/control/document语义变化即清；FIXED只冻结用户眼前exact payload。area/pan只额外借用目标panel的一份RGBA与其immutable evaluated arrays，IMAGE和其余scalar siblings继续推进，release直接显示latest。pre-arm预算把这一份最大panel hold作为具名项并有少一字节拒绝反例，不能用模糊raster multiplier代替。

当前验证结论：活动manifest共`92/92`文件、collect `1230`项，逐文件独立进程为`1227 passed + 3 expected skipped`、零失败/错误；curve/display/editor/Qt focused为`131 passed`，真实camera curve、ROI retarget和first-ROI生命周期分别为`4/13/8 passed`。独立数据合同、Qt边界、UX/authority与复杂度审查的runtime结论为`P0=0/P1=0`，故只对camera rolling Monitor判`GO`。下一提交仍必须用真实progressive occupancy scan闭合`SCAN_POINT`、多site series、MHz/ms单位overlay、非interactive静态fallback或显式preflight，并按真实规模复测hover；在此之前不得把完整U0.2c或普通1D判GO。

Rule-6按parent `caf7f7e`的17个实际production path（明确计入删除旧`qt_widgets/image_display.py`及最后一个现行consumer `_capture.py`的3处直接rename，排除并行的`calibration_render.py`等dirty）机械计数：`12279 -> 14884 physical`、`11310 -> 13752 nonblank`、`11223 -> 13665 token-NCLOC`、classes `81 -> 88`、dataclasses `56 -> 63`、enums `9 -> 9`、functions/methods `403 -> 473`，对应`1.212x/1.216x/1.218x/1.086x/1.125x/1.000x/1.174x`。`main`去重后的严格直接UX/运行证据包络为`1532 physical / 1453 nonblank / 1165 token-NCLOC`，本轮净增`2605/2442/2442`为其`1.700x/1.681x/2.096x`；纳入完整用户面后的公平包络为`4962/4647/3685`，净增只为`0.525x/0.526x/0.663x`，全部低于约3倍审查线。净enum为0，9项均至少两成员；旧image-only editor被物理删除，未发现一成员enum、compatibility wrapper、第二renderer/manager/registry。`_CurveCross/_CurveSample`虽各只有一个构造点，但在事件中反复产生且分别表达连续cursor和真实snapped sample，合并会重新混淆两种语义，故保留。

共享架构裁决（后续提交仍遵守）：共享层只提升三项已经有第二consumer的事实——`RelimMode + finite range/target/deadband`纯函数、通用exact `PanelInteractionOrigin`、一个直接参数化而非继承类族的revisioned Fluent form widget。Image和Curve各自保留强类型state、viewport与commit；`PanelFrame`只允许closed `ImagePanelPayload | CurvePanelPayload | None`，不能并列多个optional payload字段。`CurvePanelPayload`必须携scalar `EvaluatedInput`（不是raw camera input）、draw后exact transform、series labels/value unit与同front immutable `EvaluatedCurve`引用；这些引用只为exact hover/cross/display，并且永不进入FitSpec、ScanOutputContract、CommittedTransform或artifact。

Monitor提交的独立oracle已经证明：x=0始终newest；同dataset revision重画、raw-only publish、Setting/Edit和selector toggle零append；跨多source sequence全部保留或明确gap；curve hold时raw IMAGE/HISTOGRAM/METER和source sequence继续而held pixels/samples不变；release包含完整retained history；同generation ROI retarget重置y baseline；stale display/source/layout/axis拒绝；三种relim与deadband精确数值；fixed冻结眼前front；Setting/Edit dirty/stale CAS；current/curve/hist/meter同revision；预算边界。随后1D提交必须用真实progressive scan证明SCAN_POINT、多site series、同owner A/C/Z/H、Setting/Edit、Run不断流和最终artifact authority不受display state影响。两项收口都不得修改TaskConsole数据面、旧Hub/LogicNode或历史tests来迎合中间态。
...
3. formal 路径由 zlc_data `bind_fit` 产生 BoundFit、再由 neutral generic adapter 与 DatasetInputSlot 包成 AnalysisStep；interactive/offline 路径直接把同一个 BoundFit 与冻结 snapshot 投递到各自 QoS executor，三者调用同一 `BoundFit.run()`。
...
6. W3a 已交付 Capture/identity-Fit notebook surface；W3b再由scan owner把`ScanArtifactRef -> canonical output OwnedSnapshot -> DataFigure`接通。output BlockId由logical document、exact source DatasetRevisionRef与ScanOutputContract共同派生，generation/revision继承exact source dataset，schema fingerprint来自output DatasetSchema；包含实际values/provenance/safety bundle的ScanArtifact manifest另行content-addressed。zlc_data用同一峰值函数做pre-FIRE schema估算与实际CommittedTransform，并一次冻结最终DataBlock；scan/notebook都不重做reshape、validity映射或lineage。W8a只为已经存在的selection-only Capture Fit consumer证明raw snapshot精确投影与权威transform等价，不扩FigureEvaluator输入，也不建立通用transformed dataset；只有Reduction/axis-drop等第二个真实consumer出现并且raw投影无法逐值证明时，才评估让evaluator直接消费`TransformedData`。`fit(scan_ref)`当前仍显式拒绝，不建立ScanFit框架。
...
2. Fit 保持 zlc_data 单一算法 owner；W5已交付raw committed-capture的独立`.fit_gui()` editor/draft overlay/explicit Save/reopen/Clear纵切，W8b又把调用者显式冻结的W8a selection-only CommittedTransform接入同一窗口，仍不从display/selector生成权威。后续由同一headless presenter与draft authority接入TaskConsole明确的`Add Analysis -> Fit` / `Analyze -> Fit`，不得复制模型表、solver或把display transform升级为权威；Pulse prepare/fire/safe继续后台托管。
...
该历史审查 gate 的准确含义是 **Rules 1–7 existing-code owner闭包在当时GO**，不是规则8/9永久COMPLETE，也不是任意新便利切片可开始。Calibration/readout/occupancy/raw CaptureArtifact 的main同帧oracle、流式内存、多轴physical context、exact triggered consumer、typed codec以及coordinator-backed FINAL持久化结论保留；direct-CAS `CaptureFitResultRepository` 不在这项FINAL持久提交结论内。real qCMOS Formal仍受E0/Q0 common-aperture、first-edge/run-boundary、ordered-trigger、tail/EOS与EndAttestation真机资格化约束，因此真实硬件capability继续NO-GO。后续每个commit须满足Rules 1–9中适用的增量证据，并先按U0纠正，不能把局部virtual/offline READY改写成全局迁移完成。
...
notebook的`figure_document(scan_ref)`仍只inspect manifest/index/schema，`figure(scan_ref)`才bounded materialize并复用role-driven DataFigure。新增`exp.scan_gui(frozen_request)`是真实、明确标注FINAL-only的Qt产品：start/result/projection/reap均在有界worker，GUI只看revision-gated ViewModel和immutable PNG；新Run立即清除上一轮raster，projection失败保留FINAL ref，close/cancel异步reap。同一个projection cap按顺序而非相加约束`repository materialize`、`input snapshot retained + FigureEvaluator peak`、`evaluated arrays + Agg canvas/PNG`以及`PNG + source/scaled QPixmap`；Qt禁止放大到source raster以上，避免resize绕过预算。它不是progressive LiveSweepMonitor或完整TaskConsole editor。current oracle同时覆盖direct `(2,3,96,128)`、occupancy `(2,2,site)`、ComponentValidity、processor/calibration lineage、fresh-process reopen、metadata-only且禁止heavy-lineage decode的FigureDocument、pre-FIRE static-lineage拒绝、bounded PNG与Qt close/stale-raster race；`fit(scan_ref)`仍typed拒绝。没有删除legacy PulseScanNode/MeasurementSpec/TaskConsole，因为progressive panel、edit/catalog与其余消费者尚未迁完。
...
|---|---|---|---|
| `MonitorTap -> MonitorDataset` bounded live | Workbench live image、rolling curve、histogram、board coherent present | W1已接finite preview；M1/M2c已把独立virtual free-running MOT camera、raw IMAGE、显式typed ROI scalar、独立长scalar history，以及同一scalar revision的CURVE/HISTOGRAM/METER和raw/scalar同源四panel coherent board接入public notebook/Qt，并保留sensor clock、compiled coil physics、EventRef/control lineage、ComponentValidity与Stop撤屏；real Pylon qualification、动态selector/control及剩余legacy panel仍待后续M2/S5 | 同等 GUI capability 已由更小唯一 owner 交付并迁完全部 panel |
...
规则状态必须诚实记录为 `NOT_STARTED/PARTIAL/COMPLETE`。§22.1固定的11个互斥production owner闭包已逐一完成Rules 1–7 existing-code审查，所以“完成1–7前不得继续迁移”的历史冻结曾解除；规则8/9是持续per-commit gate，不能一次永久COMPLETE。virtual finite capture/pulse owner、BoardController coherence foundation、W1 single-event finite exact capture、S3a/S3b/S3c、W3e/W3f与W-UI1a已经证明的底层/authority/lifecycle事实保留，但其缺失UX按§2.2/U0继续MUST_CLOSE。既有结案证据包括preflight→execute→FINAL→admit current-format oracle、预算/recovery、headless import、exact多维轴/component-validity、repository/Run生命周期、DAG/shape扫描与有界对抗审查；从本修正案起，这些机械证据必须叠加活动白名单完整collect+run与适用的UX salvage/E2E。remote/real qCMOS、正式scan物理资格、完整GUI scheduler、rolling/overlay、其它panel的Save/Setting/Edit、GUI site-map/gridplot和普通用户真实设备入口仍分别NO-GO；virtual/offline API segmented不能倒写成真实qCMOS Formal资格。每个后续dependency-closed切片须按Rules 1–9做增量审查，完整迁移结束后再对最终分支做一次独立对抗验收；局部GO或owner audit COMPLETE都不能改写成全局实现完成。
...
**最新实现 checkpoint：W5 raw committed-capture interactive Fit纵切完成。** public `.fit_gui(CaptureArtifactRef, model=None)`现可异步准备可用model、显示具名fit/batch axes、用共享Fluent form编辑catalog-owned parameter constraints、运行每个batch cell的同一个`BoundFit`、显示draft overlay与status/quality，并通过显式Save发布后从exact `CaptureFitResultArtifactRef`重开；Clear只撤当前draft/overlay。整个首切片只接受raw committed capture且拒绝`CommittedTransform`；权威`FitSpec`只来自source schema、closed model catalog和exact form state，所有非fit轴仍是batch axis，没有shape/rank猜测、first/flatten/trailing mean，也没有ViewSpec/selector/display reducer进入请求。多维data axis因此不会被压成`(repeat, point, data_dim)`三项或丢失内部信息。

能力边界由三个最小真实owner闭合。frontend `fit_editor`只把`BoundFit.parameter_definitions/parameter_units`投影到已有`FormSpec`并做exact-key round trip，不复制模型/solver/widget。headless `CaptureFitDraftAuthority`唯一持有未保存`FitExecution`并线性化execute/discard/save/close；Qt只得到无save能力的`FitDraftResult`，不存在把execution交给任意projector的接口。draft overlay只传`FitResultBatch`，notebook composition再把它与窗口固定的Capture ref组合，并调用data owner的完整source revision/schema/fit axes/batch axes/layout/unit/present-count binding validator。Save必须经过同一Experiment lifecycle gate；Save先于Close线性化时完整发布并先保留exact ref，Close先线性化时零发布。Save后reopen/render失败不抹掉ref，Save中窗口Close明确defer。
...
**最新实现 checkpoint：W7 exact saved-fit GridPlot explorer纵切完成。** `Experiment.figure_gui(CaptureFitResultArtifactRef)`在没有自定义intent/selection/preferences时按typed ref分派到saved-fit explorer；携自定义view参数仍走既有generic frozen Figure，不建立第二套projection语义。explorer只读取这个content-addressed exact ref已有的`FitResultBatch`，从不调用solver/`execute()`、不改变`FitSpec`、不创建draft，也不发布新artifact。每根fit batch axis仍按原`AxisSpec`与`AxisLayout`存在：repeat显式为`FACET`而非mean/latest/index-0，其余当前页跨度大于1的轴显式facet；每页逻辑panel上限36。EXPLICIT layout的hole保留在原逻辑位置并带`fit_storage_index=None`，绝不以相邻physical row补位。
...
一次Agg compose同时返回最终panel的normalized实际坐标、完整具名`Selection`与exact stored row；double-click只接受这份hit map。共享`AxisLayoutNavigator`从W4e occupancy抽成唯一Qt owner，按axis count而非axis size建控件，非singleton保持`Select…`，Previous/Next严格沿physical storage order并跳过hole。focus详情只取选中stored row已有的status、parameters、covariance uncertainty、present/valid/used/evaluation counts、RSS-derived per-cell RMSE与R²；failed row不显示伪参数，也不访问/物化整批RMSE。Esc直接恢复缓存的当前page front，不重新solve或重新读取artifact。

性能与内存合同不是“每点重开repository”。窗口得到的仍只是一个窄worker-affine callable；该callable第一次在shared frozen-raster worker中按调用者总预算检查encoded blob、解码后的fit retained bound、source dataset与compact model，随后一次物化exact source snapshot并在私有session中复用。focus、换页和export都继续经过Experiment lifecycle guard，但复用同一个immutable result/snapshot，不再次decode完整fit或materialize qCMOS source；Qt owner不能访问这个session，只持compact `FitGridModel`、bounded page、cell text、PNG与hit regions。以后每个job先从总cap扣掉session、model以及仍保留的page/current front，再把唯一余额交给Figure evaluation/Agg；低预算在decode/materialize/Qt decode前fail-closed。model identity同时冻结artifact target、完整axis values和同一exact `AxisLayout` mapping，不能把同shape的另一张sparse表接到旧navigator。

image export也是display-only：worker先在目标同目录写临时文件，Agg成功后在export/Close共用的线性化锁内复核cancel，再用`os.replace`原子提交。Close先获胜时删除temp且保留任何既有destination；replace先获胜时得到完整新文件，不存在半截成功、stale UI publish或manifest变化。W7复用已有DataFigure evaluator、Agg owner、capacity-one frozen executor、encoded raster lifecycle和Fluent widgets；没有第二repository、renderer、executor、scheduler、gallery registry或fit codec。`FigurePanelRegion`、三种compact fit-grid DTO、两消费者的`AxisLayoutNavigator`与真实`SavedFitGridWindow`是六个新增class的全部；Rule-6同时删掉无调用的region-render helper、只供测试的properties和重复model比较。

W7 Rule-6按20个实际受影响production Python文件相对parent `4cb2f49`机械计数：`9569 -> 11393 physical`、`8644 -> 10328 nonblank`、`8515 -> 10175 token-NCLOC`，净`+1824/+1684/+1660`，整体`1.191x/1.195x/1.195x`；classes`68 -> 74`、dataclasses`43 -> 47`、enums`7 -> 7`、functions/methods`366 -> 428`。净差量相对main严格GridPlot/fit行为包络`3259 physical / 2595 NCLOC`仅`0.560x/0.640x`，相对完整公平包络`8350/6574`为`0.218x/0.253x`。为不隐藏海拔，包含W7复用基础的整个touched closure相对严格包络确为`3.50x/3.92x`，但相对公平完整产品为`1.36x/1.55x`；高出的部分是此前已经挣得的typed fit/DataFigure/repository/Workbench基础，不由本切片重新复制。

验证为W7白名单`7 passed`，直接受影响frontend Figure与W4 Figure/occupancy回归`39 passed`，W5真实save/reopen fresh-process回归`1 passed`；owned `py_compile`、fresh headless import、exact-ref/no-refit、sparse-hole、repeat-FACET、one-load/one-materialize、低预算pre-decode拒绝、atomic export/Close与shape-loss扫描通过。一次有界独立对抗先报出budget传递、非原子export和每次focus全量重读三个P1：第一项在审查结果到达前已由repository budget接线关闭并补上“decode不可到达”oracle，后两项以上述session复用与同目录原子替换关闭。W7纵切现为`P0=0/P1=0/GO`；完整迁移仍为NO-GO。本切片没有dependency-closed legacy删除：旧TaskConsole/Live/GridPlot仍拥有live artist、scan/ROI-transform fit、selector和其它rolling consumer，下一切片必须从最新Git checkpoint重新选择真实consumer，不能为了删文件把这些路径提前打黑。
