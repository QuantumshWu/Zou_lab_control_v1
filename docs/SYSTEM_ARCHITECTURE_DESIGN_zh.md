...
设备参数的语义边界必须保持可见：相机 leaf 只声明设备身份和真实硬件工作点，不声明、解析或存储 FPGA raw trigger lane，也不依赖 PulseTarget manifest。相机连接只建立相机本身的 typed Port；它接收外部触发并按自己的 readout/ordinal contract 采集，但永远不需要知道哪条脉冲线产生了触发。触发端点是 pulse/measurement/calibration request 的语义：该 owner 在已有 pulse target 上把自己的 semantic endpoint（例如 `emCCD`）解析成编译 artifact 的物理 lane，并由 pulse terminal/scan lineage 校验计数。Camera association 只携带 expected event count、cause 与 ordinal boundary，不携带 trigger channel；这样真实相机没有第二套 wiring truth。只有 `VirtualCamera`/`VirtualSequencer` 为了在同一进程里模拟“某条虚拟线产生某个相机帧”才保留内部 route，不能泄漏到真实 camera schema 或 Device Manager。E0/bring-up 若要产生测试脉冲，测试 helper 可以显式接收 pulse endpoint，但这不是 camera connection 的能力或状态。该 binding 不携带 lattice grid、site centers 或任何 Calibration 数据。`grid_rows/grid_columns` 是 Calibration Task 的 authoring intent，site centers 是 Calibration 的检测结果/可选 admission 先验，不能出现在 camera Device Manager schema 或 hardware installation template。采集/传输 timeout 是 adapter/RPC 内部的等待策略，不是用户硬件工作点，也不能作为设备卡的伪硬件旋钮。这样新增相机型号不会把实验几何、Target 接线和软件策略重新塞回设备 leaf。
