...
| `UX-003` | figure viewer 载入后是可交互 panel，可 zoom/pan、重新 fit 与导出；GridPlot 可 focus/返回 | W4 generic figure 与部分 artifact 查看器曾冻结为整板 PNG/display-only；W7 已闭合 saved-fit GridPlot 的 focus/返回与导出；U0.3a 又把恰好一个layer/cell/input的冻结HISTOGRAM DataFigure迁到typed panel，恢复range/cross/zoom/home、bin hover、x pan/pin、bin/count-scale/relim、同源Setting/Edit与当前front原子PNG导出。generic IMAGE/CURVE/METER、多cell/Grid及显式re-fit仍未闭合 | 先以 immutable raster 闭合线程/内存所有权，再按真实consumer逐个恢复typed交互面 | **待用户批准 / 默认必须继续修复**：剩余通用 figure/archive plot kind、Grid focus及`Analyze -> Fit`；U0.3a已经证明的单panel HISTOGRAM行为不得回退，报告类多页可保留 frozen raster 但必须可 zoom |
