...
| `UX-003` | figure viewer 载入后是可交互 panel，可 zoom/pan、重新 fit 与导出；GridPlot 可 focus/返回 | U0.3a-f 已把单panel HISTOGRAM/CURVE/IMAGE/METER、fit-bearing CURVE/IMAGE及单layer METER多cell overview/focus接入同一 typed board；range/cross/zoom/pan/home、relim/cmap/limits、Setting/Edit、原子导出、即时 fit overlay 已恢复。exact saved-fit GridPlot与METER grid都不暗选第一格，只在显式命中真实cell后focus，并从缓存返回overview；稀疏hole保持原逻辑位置。尚未闭合的是multi-layer DataFigure、faceted CURVE/HISTOGRAM/IMAGE及非Fit的其它archive plot kind；报告类多页仍是 frozen raster | 继续按真实consumer逐个恢复typed交互面；typed资格不足时保留完整whole-figure renderer，不以丢fit/轴/series换交互，也不先造通用dashboard框架 | **待用户批准 / 默认必须继续修复**：剩余multi-layer、其它faceted intent与archive plot kind；U0.3a-f及W7已恢复的typed交互、explicit-cell focus/Refit与导出不得回退，报告类多页至少补zoom |
...
| `UX-006` | calibration/site-map/occupancy 图沿统一交互 viewer/GridPlot 获得 focus、selector、zoom 与 export | U0.3f已让单layer SITE-faceted occupancy METER沿统一Figure获得同draw overview、exact-cell focus、缓存返回和两种原子PNG导出；它不暗选第一site、不把component validity或单位丢掉。calibration report、site-map图与其它occupancy图仍是静态 frozen raster，未提交工作树不能作为闭合证据 | 保留已完成的METER grid；其余路径先闭合immutable report/exact-address与预算/线程所有权，再按真实surface接入既有selector/viewer | **待用户批准 / 默认必须修复**：报告类多页至少zoom；calibration/site-map与非METER occupancy路径按旧surface补适用的focus/selector/export，或逐项提交用户裁决 |
...
边界只到“单layer、多cell、全部真实panel为METER、无fit overlay”。多layer当前没有production producer，只有codec/value测试；其overlay、z-order、共享轴和跨layer分析语义都未成立，因此继续使用完整whole-figure静态回退，不产品化dashboard。faceted CURVE/HISTOGRAM/IMAGE复用同一overview/focus外壳是下一位真实consumer出现后的独立纵切，不在本轮顺带泛化。验收覆盖单位不丢、valid/invalid/nonfinite、live meter exact payload/coherence、multi-site overview无默认focus、精确第二cell命中、sparse hole不前移、Back/Esc使用缓存、overview/focus原子导出、预算差一字节在重型分配前拒绝、Close期间无late-present，以及multi-layer仍走静态fallback。
...
最终focused oracle为`10 passed`，U0.3a-f直接闭包为`69 passed`，frontend Figure为`34 passed`，import-DAG为`25 passed`；owned compile、fresh headless import、diff-check、current-format ratchet与新增shape-loss扫描通过。活动manifest为`110/110`文件、完整collect `1420`项；按process-lifetime installation边界逐文件新进程执行为`1417 passed + 3 expected platform skips`，零failed/error/xfailed/xpassed。两路独立终审先后抓到Histogram property owner错位、focus identity漏exact ref、O(N²)/全grid focus路径、冗余预算DTO以及派生DTO早于预算拒绝等问题；全部以反例整改后结论均为`P0=0/P1=0/P2=0/GO`。该GO只关闭generic单panel METER与单layer METER multi-cell explorer；multi-layer、其它faceted intent、calibration/site-map report及完整迁移继续`NO-GO`。
