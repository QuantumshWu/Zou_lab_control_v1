...
| `UX-003` | figure viewer 载入后是可交互 panel，可 zoom/pan、重新 fit 与导出；GridPlot 可 focus/返回 | W4 generic figure 与部分 artifact 查看器曾冻结为整板 PNG/display-only；W7 已闭合 saved-fit GridPlot 的 focus/返回与导出；U0.3a/U0.3b 已把恰好一个layer/cell/input的冻结HISTOGRAM及**无fit overlay、有限numeric严格单调共享X**的CURVE DataFigure迁到同一个typed numeric panel，恢复range/cross/zoom/home、x pan/pin、三态relim、同源Setting/Edit与当前front原子PNG导出；HISTOGRAM另保留bin hover/bin/count-scale。fit-bearing CURVE、generic IMAGE/METER、多cell/Grid及显式re-fit仍未闭合，所有fallback在窗口摘要中显示明确unavailable reason | 先以 immutable raster 闭合线程/内存所有权，再按真实consumer逐个恢复typed交互面；display typed资格不足时保留完整whole-figure renderer，不以丢fit/轴/series换交互 | **待用户批准 / 默认必须继续修复**：剩余通用 IMAGE/METER、fit-bearing CURVE与archive plot kind、多cell/Grid及`Analyze -> Fit`；U0.3a/U0.3b已经证明的单panelHISTOGRAM/CURVE行为不得回退，报告类多页可保留 frozen raster 但必须可 zoom |
