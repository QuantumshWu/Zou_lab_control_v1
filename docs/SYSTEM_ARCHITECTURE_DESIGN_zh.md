...
Experiment                         # notebook/application facade，不含领域算法
  ...
  .figure_gui(result_or_ref, occupancy_output=None)      # 懒加载冻结结果只读显示
...
| `U01_GUI_WORKBENCH` | frontend render、Workbench shell、旧frontend与`pulse_gui.py` launcher | 17 | COMPLETE：existing primary paths及最新W6差量复审闭合 | board identity/final gate、virtual/offscreen single-event finite capture、显式ROI scalar history、coherent四panel、fixed-ROI front-bound selector/overlay、raw committed-capture Fit editor/draft/Save及formal calibration creation/edit产品IMPLEMENTED；raw-only首ROI、TaskConsole/scan/grid Fit、通用scheduler/PanelHost、rolling grid与legacy GUI迁移BACKLOG/NO-GO |
...
验证为W6白名单`7 passed`，直接受影响W4 paired-load/report回归`2 passed`、W5共享frozen-raster回归`1 passed`；owned `py_compile`、fresh headless import、import-DAG与`git diff --check`通过。authority与lifecycle各做一次有边界只读终审，前者确认空间/validity/display防火墙，后者复现并关闭commit evidence与旧QImage front生命周期漏洞；最终均为`P0=0/P1=0/GO`。完整迁移仍为NO-GO；下一纵切是saved-fit GridPlot，之后继续live calibration/grid、TaskConsole与其它未迁真实consumer，不重开W6或维护历史测试迎合中间态。
