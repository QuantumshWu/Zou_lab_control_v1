...
10. 屏幕 raster 的几何合同是 `physical raster px = qRound(logical panel px × 当前窗口所在屏幕的 devicePixelRatioF())`；worker按`DESIGN_DPI × PANEL_DISPLAY_SCALE × DPR`构图，Qt board固定逻辑尺寸并一对一落到物理像素，禁止先按DPR=1渲染后由Qt放大。窗口首次取得native screen、跨屏或screen DPI改变时，presentation owner必须更新DPR并请求新revision；125%/150%/175%等分数DPR也必须让Agg raster与Qt physical target逐像素同尺寸，不能容忍一像素整图重采样。导出仍只用`savefig.dpi`，不得把屏幕DPR带入文件。该合同只改变采样密度，不改变图形样式、布局或交互。每个plot kind的axes/data box与margin只能由size/kind render token纯函数决定；viewport、动态tick、越界annotation和任意data artist不得进入layout solver。交互只改变typed transform；同一结构revision的title、row label与plot bounds在pan/zoom全程固定，xlabel继续沿用Matplotlib基于当前tick文本的既有定位。
