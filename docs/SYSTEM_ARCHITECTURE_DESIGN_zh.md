...
| `UX-002` | live plot 的 selector、zoom/pan、十字、hover 读值、relim 与 cmap/limits 覆盖旧 plot contract；拖动只冻结当前 panel，其它 panel 不停流，松手补拍 | U0.2b 已在 free-running camera monitor IMAGE 恢复完整 A/C/Z/H/hover、ViewportTransform、三种 relim、六种 cmap、同源 Setting/Edit 与 per-panel hold；finite capture 与 1D/Monitor、Sites、Histogram、Grid 及真实 launcher 的全 plot-kind handle 仍未闭合 | 早期窄 leaf 被误当作可长期冻结产品面 | **待用户批准 / 默认必须继续修复**：finite capture 与剩余 plot kind 的非空交互句柄、各自 relim/axis 语义、grid focus 与 launcher E2E；free-running camera IMAGE 已通过的行为不得回退 |
