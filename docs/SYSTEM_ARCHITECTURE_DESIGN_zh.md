...
Experiment                         # notebook/application facade，不含领域算法
  ...
  .fit_gui(capture_ref, model=None) # typed draft overlay + explicit Save
...
W5交付第一个current交互Fit产品：`.fit_gui(CaptureArtifactRef, model=None)`只接受已提交raw capture，后台从同一schema和closed model catalog生成可用模型，以共享form DSL编辑显式parameter seed/bounds/fixed值；首切片拒绝`CommittedTransform`，不读取当前ViewSpec、selector、viewport或display reducer。`FitSpec`冻结具名fit axes，所有其余repeat/point/data axes原样成为batch axes；Qt明确显示axis name/AxisId/role/size/unit和每个batch cell的status/quality。Fit使用独立capacity-one worker lane，普通冻结Figure继续使用原lane；两者只在既有Agg render owner处短暂串行，不让长solver占住Figure materialization。
...
长Fit只在开始/结束时短持Experiment lock并登记一个fit-specific active count，不建立通用workflow/lease framework。Close若与active Fit竞争，原子进入`CLOSING`后fail-fast，禁止新Fit/preview/save；在途Fit退出时转为`FitCancelled`而不能接纳draft，调用者在排空后重试Close，与现有active Run的显式retry语义一致。已进入bounded capture decode不能被强杀；deadline/cancel在lane等待、solver前后和Qt接纳处诚实检查，decode阶段只诚实排空。该产品尚不等于TaskConsole `Add Analysis -> Fit`、authoritative ROI/transform fit、scan-y fit、live/grid batch editor或calibration edit，这些继续按真实consumer迁移。
...
2. Fit 保持 zlc_data 单一算法 owner；W5已交付raw committed-capture的独立`.fit_gui()` editor/draft overlay/explicit Save/reopen/Clear纵切。后续由同一headless presenter与draft authority接入TaskConsole明确的`Add Analysis -> Fit` / `Analyze -> Fit`，不得复制模型表、solver或把display transform升级为权威；Pulse prepare/fire/safe继续后台托管。
