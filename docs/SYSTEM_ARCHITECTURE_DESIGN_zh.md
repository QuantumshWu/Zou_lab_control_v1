...
本轮 Qt leaf 只画一张 immutable GRAY8、保持声明的 `(Y,X)` 方向并做 aspect-fit；它没有 `ViewportTransform`、ROI/crosshair/selection/hover、zoom/pan、固定色阶 control、axes/colorbar、save/export、Matplotlib/Agg、多 panel compose、多 source join或 continuous/free-running monitor。上述终态条款不因这个 component oracle 放宽；完整 Workbench composition 与真实用户可见 UI 仍为 NO-GO。
...
当前只完成 S1#3 的 virtual/component 子集：finite exact source 可附着 capacity-one preview，纯 FigureEvaluator/GRAY8、单 panel BoardController 与可选 Qt presenter 的手写 oracle 已闭合；没有真实 app composition call site、driver-buffer reuse/qCMOS profile、overlay/viewport、save、free-running monitor或旧 camera panel 替换，所以 S0.5 与 S1 均不得标 COMPLETE，也不得据此删除任何旧 GUI producer/consumer。
...
迁移结构尚未闭合期间**不执行 pytest，也不修改任何历史测试来迎合中间态**。架构迁移会主动删除旧入口、旧对象图和旧调用语义，此时逐步追赶历史测试只会把已删除的 `DeviceSet/Registry/session` 兼容面重新带回目标实现。`tests/migration_active_tests.txt` 仅保留为终态测试重建时的 target inventory，不是当前 runner；曾误加并直接调用`pytest.main()`的`tests/run_migration_active.py`已在P01/H01追溯轮物理删除，不保留替代脚本。迁移完成、目标 API 与边界稳定后，才按新架构逐项重写或删除清单内外测试，并一次性建立可执行入口。此前不得直接或间接运行 `pytest`，也不得因任何旧测试的预期修改 target production。

当前整改闭包证据只允许机械证据：changed-file/owner 账本、AST/symbol/consumer 搜索、静态 import-DAG、canonical/digest/版本常量/重复 validator 扫描、`py_compile`/AST parse、main 行数与物理算法只读对照、手写current-format确定性oracle，以及必要的真实 profile。它只能证明当前 owner 的结构与实现已收敛，不能冒充迁移完成后的行为验收；完整行为测试统一在终态结构稳定后重建并执行。本节之前账本里出现的`N passed`、`pytest items`或`collect-only`数字全部解释为规则8冻结前的历史checkpoint，只可说明当时曾观察什么，不得重跑、不得作为当前owner COMPLETE的必要或充分证据；出现“当前证据集合”时也按此限定解释。

全部历史测试在迁移完成前视为不存在：不执行、不修改，也不因其失败改变目标实现。迁移结束时再按终态架构整批删除或重写。旧 `DeviceSet/Registry/session/GUI/public compatibility` 语义只允许登记“分支改过什么、最后消费者、删除切片”，或在dependency-closed切片中删除；禁止为它们新增适配、修守卫或追求旧测试通过。尚未迁址、但仍被target endpoint作为唯一真实硬件owner消费的adapter文件可以接受证据驱动的物理正确性修复，例如DCAM count/ring/metadata边界；这种修复不得恢复旧composition、registry或public API。main仍可只读用于行数锚点和已验证物理/算法oracle。本轮`discovery.py/registry.py`的worktree变化是精确恢复main、撤销此前错误的target适配，不是把旧接口接回目标包。教程、手册与旧GUI说明在迁移完成前不做同步；本设计文档例外，任何计划级取舍必须与代码同commit更新。
...
|---|---|---|---|
| `MonitorTap -> MonitorDataset` bounded live | Workbench live image、rolling curve、histogram、board coherent present | capacity-one finite-preview component已接入一个snapshot-first单panel consumer；tap只拥有admitted backlog，materializer拥有keyed cycle/append window、head/EventRef与atomic snapshot；其余rolling/multipanel composition仍待S5 | 同等 GUI capability 已由更小唯一 owner 交付并迁完全部 panel |
...
规则状态必须诚实记录为 `NOT_STARTED/PARTIAL/COMPLETE`。§22.1固定的11个互斥production owner闭包已经逐一完成，规则1–8的**existing-code追溯审查轴全部COMPLETE**，所以“完成1–7前不得继续迁移”的冻结已经解除；这不等于implementation全部READY。当前明确为GO的是virtual finite capture/pulse owner、BoardController coherence foundation、finite exact capture的capacity-one live-image component/oracle、S3a“committed capture -> calibration ref -> artifact/report reopen”、S3b“committed capture + calibration ref -> occupancy ref -> reopen”和S3c“installation-qualified short sitemap -> raw FINAL -> calibration FINAL/recoverable failure”的virtual/offline纵切。S3a–S3c结案证据包括完整preflight→execute→FINAL→admit current-format手工oracle、1-byte预算与raw recovery、fresh-process notebook不加载SciPy/Qt/Matplotlib、exact `(R,P,*data_shape)`/`(R,P,site)`/component-validity检查、repository close与run borrow生命周期、static DAG/shape/旧Registry扫描，以及每个切片的两路独立对抗审查均`P0=0/P1=0/GO`；live-image component另有snapshot-first、source invalidation、multiworker serialization、large-frame materialize与artifact failure-isolation手写oracle。没有运行历史pytest。remote/real qCMOS、正式scan物理资格、完整GUI scheduler/render接线与状态/overlay/save层、GUI site-map/gridplot和普通用户真实设备入口仍分别NO-GO。每个后续dependency-closed切片仍须按规则1–8做增量审查，完整迁移结束后还必须对最终分支做一次独立对抗验收；局部GO或owner audit COMPLETE都不能改写成全局实现完成。
