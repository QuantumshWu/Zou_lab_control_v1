...
\begin{document}
\frontmatter
\zlctitlepage{Zou\_lab\_control.frontend 中文手册}{Jupyter / GUI 实验绘图前端}{统一 array 数据契约、live session、fit、selector 与 PDF notes}{2026-06-10}
\tableofcontents
\cleardoublepage
\mainmatter
...
\begin{figure}[htbp]
  ...
  \caption{Scan 标签页的真实截图。顶部列出扫描表的各列（每个绑定的 slot 一列，含它绑的字段与单位）；代码框上方是 Load Program 与两个模板按钮，下方是 Run / Save Array 按钮；右侧是只读的扫描表预览。}
...
\begin{enumerate}
  ...
  \item 打开 \tfocus{Scan} 页。页面顶部三个按钮可以一键填充编辑器：\pyapi{Load Program}（从 \filepath{.py} 载入一段程序）、\pyapi{Template: column\_stack}（每个 slot 一列，\tfocus{默认}模板）、\pyapi{Template: grid}（两个数组的外积网格）。两个模板都\tfocus{按当前 slot 数自动适配}；默认显示 column\_stack 模板。也可以直接写 Python，把一个 \pyapi{N_points x N_slots} 数组赋给变量 \pyapi{scan_table}（命名空间里有 \pyapi{np}、\pyapi{math}、\pyapi{n_slots}）。
...
\begin{description}
\item[延迟输入框] 作用：给\tfocus{这一个通道的所有边沿}加上一个\tfocus{固定的时间偏移}，用来补偿线缆长度、设备响应等带来的固有时延。它是一个\tfocus{固定}值，\tfocus{不是扫描量}——延迟字段没有扫描圆点（要扫的是 duration 或 DAC 值，不是延迟）。可接受的值：一个时间数值，\tfocus{可正、可负、可零}——正值把该通道的所有上升/下降沿整体\tfocus{往后推}，负值整体\tfocus{往前提}，零表示不延迟。\tfocus{大小是有界的}：TTL 延时由\tfocus{事件调度器}实现（存翻转、按 \pyapi{out[t]=in[t-d]} 物理放出），上限是 32 位字段（约 $\pm42.9$ s，毫秒级 emCCD 延时直接可用）；DAC 总线延时仍走 2048 拍环（约 $\pm41\,\mu$s）。\tfocus{超界会被自动夹回}对应上限（编辑完成时自动发生，TTL 与 DAC 各按各的上限），输入框的悬浮提示也写明了当前上限；并且该框\tfocus{只接受数字、小数点、e 记数法与正负号}，其它字符在键入时直接被拒绝。它\tfocus{不会改变任何周期（period）的时长}，只是把该通道的输出整体平移——该通道在 fire 之后\tfocus{安静}到延迟时间才开始，\tfocus{第一帧是真实的}（不是预览里那种循环绕回的样子）。输入后会发生什么：该数值先被\tfocus{量化}成节拍的整数倍，再把该通道在时间轴上的所有跳变按这个偏移挪动。常见的坑：（1）以为延迟会拉长或缩短脉冲——不会，它只平移边沿，脉冲宽度和周期长度不变；（2）忘了它是\tfocus{按通道}独立的，改了 A 通道不影响 B 通道，也\tfocus{绝不}打扰别的通道的时序；（3）负延时等价于给\tfocus{其它所有}通道各加一个正延时（编译器用一个全局平移 $G$ 实现），因此一组延时的\tfocus{跨度}（最大减最小）也必须 $\le$ \pyapi{DELAY_DEPTH}，否则编译时会报清楚的错。实用提示：先用示波器量出某条线相对基准慢了多少 ns，把这个值（取正补偿滞后）填进去即可；负延迟同样合法，适合让某通道\tfocus{提前}触发。模拟总线（DAC）的值\tfocus{也能}用同样的方式延时（同一深度上限），且一条 DAC 总线的 10 个 bit \tfocus{共享一个}延时。
\end{description}

\subsection{每个通道一行：单位下拉框与 X 隐藏按钮}

\begin{description}
\item[单位下拉框（ns / us / ms / s）] 作用：选择该行延迟数值的\tfocus{时间单位}。TTL 延迟是\tfocus{物理延时}，上限由 32 位字段决定（约 $\pm42.9$ s），所以 \pyapi{ns}/\pyapi{us}/\pyapi{ms}/\pyapi{s} \tfocus{都可选}（毫秒级 emCCD 延时直接填即可——旧 $\sim$41\,$\mu$s 环形缓冲时代"只有 ns/us"的限制已取消）；只有 \pyapi{str (ns)}（仿射表达式单位）对一个固定、不可扫描的延迟没有意义，所以下拉里不提供。（DAC 总线延迟仍受 2048 拍环约束，约 $\pm41\,\mu$s。）点击后会发生什么：下拉展开，选中某个单位后，左边输入框里的数字就按这个单位解释（例如填 \pyapi{1} 配 \pyapi{us} 等于 1 微秒）。常见的坑：单位选错会让延迟差出 1000 倍（把 \pyapi{us} 当成 \pyapi{ns}）；另外最终值仍会被时钟节拍量化，所以填一个比节拍还小的组合可能被对齐成 \pyapi{0}。实用提示：日常多用 \pyapi{ns}，较长的固定延迟用 \pyapi{us}/\pyapi{ms}。
...
\item[clk 按钮（把通道接到 FPGA 时钟）] 作用：把\tfocus{这一条通道的输出引脚直接接到 FPGA 的 clk}（相当于 verilog 里 \pyapi{assign 这个引脚 = clk}），用来给外部设备（比如 DAC）提供时钟。点击后会发生什么：该通道被标记为 clk 通道——它\tfocus{从脉冲引擎里移除}（不再由边沿表驱动，所以引擎\tfocus{绝不会覆盖}它的 clk 行为），它这一行的\tfocus{延迟框和单位框都变灰禁止编辑}（对一条 clk 通道没有意义），并且\tfocus{从 Preview 里隐去}（它输出的是时钟方波，不是脉冲）。再点一次恢复成普通的引擎驱动通道。实现上，编译时把该通道的 mask 位强制清 0，并把一个 62 位的 \pyapi{clk_enable} 掩码随程序下发；FPGA 顶层用 \pyapi{out_final[n] = clk_en[n] ? clk : out[n]} 把 clk 接到该引脚（运行时由 CTRL 寄存器选择，换 clk 通道\tfocus{不需要重新综合}，但首次启用这套 mux 需要一次重新构建 bitstream）。常见的坑：DAC 总线（da\_dipole 等）的成员位\tfocus{不能}单独设成 clk（会破坏 DAC），所以这些行的 clk 按钮是禁用的。实用提示：要给外部 DAC 喂时钟，就把对应那条通道（如某个 da\_clk 线）设成 clk，不用再在脉冲里手动翻转它。
\end{description}
...
\begin{description}
\item[Save] \tfocus{作用}：把当前脉冲保存成磁盘文件。\tfocus{接受什么}：动作按钮，无需在按钮上输入参数（实际文件名通过保存流程确定）。\tfocus{点下去会发生什么}：它会写出一个 \pyapi{.json} 脉冲文件，\tfocus{同时}附带写出一张预览图 \pyapi{.png}；并且\tfocus{如果当前存在扫描表（scan table）}，还会再写出一个 \pyapi{<name>_scan.npy} 数组文件。也就是说一次 Save 最多产出三个配套文件，方便你完整复现这次实验设置。\tfocus{状态指示（confocal 同款）}：有\tfocus{未保存的修改}时按钮显示\tfocus{黄色}且文字带星号（\pyapi{Save*}），标题栏同时带星号；内容保存干净后变回\tfocus{蓝色}的 \pyapi{Save}。\tfocus{实用提示}：把「\pyapi{Save} 蓝色 + 标题栏星号消失」当作「已存盘」的可靠信号；运行前如果看到 \pyapi{Save*} 还是黄色，说明你最新的改动还没落盘，建议先 Save 再 On Pulse，避免事后找不到这次用的是哪份配置。
...
\begin{itemize}
  ...
  \item \tfocus{通道延时}：上限现在约 $\pm 42.9$ s（FPGA 端事件调度器；毫秒级 emCCD 延时直接填入即可）。这是\tfocus{真物理延时}：\pyapi{out[t]=in[t-d]} 逐 tick 成立，首个 $d$ 拍沉默（\tfocus{首帧正确}），\tfocus{不做任何按周期取模}。硬件约束是"该通道任一延时窗口内的\tfocus{在途翻转}数 $\le$ 事件 FIFO 深度（默认 256，可在板级配置里重配）"——编译器对\tfocus{完整日程}（每个扫描点、repeat 回绕缝、跨多帧的长窗口）做精确计数，稀疏触发信号天然满足，超限会在编译时给出明确报错（并报出最长可用物理延时）。能延时的只有\tfocus{18 路真 TTL 输出}：DA 总线位与 \pyapi{da_clk} 时钟脚没有事件 FIFO，它们的延时框会置灰、API 也会拒绝。
\end{itemize}
