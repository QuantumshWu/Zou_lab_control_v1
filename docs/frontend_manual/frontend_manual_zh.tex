...
\section{布局与参数}
每张面板卡的头部是：标题框、网格位置 \pyapi{R/C}（行/列）与跨度 \pyapi{H/W}（高/宽，格子数）、各类型自己的参数（2D 的 colormap、滚动轨迹的历史长度、直方图的 bin 数）、移除按钮 \pyapi{X}。标题栏的 \pyapi{Add Panel} 按所选类型加新面板；\pyapi{Pause/Resume} 暂停/恢复刷新；\pyapi{Save}/\pyapi{Load} 把\tfocus{整个布局}（面板、位置、表达式、参数）存成一个 JSON（默认在 \filepath{tasks/} 目录），\pyapi{Save*} 黄色星号语义与脉冲编辑器一致。面板值的形状变化（换表达式 / 改跨度 / 改参数）会自动重建该面板的图。
