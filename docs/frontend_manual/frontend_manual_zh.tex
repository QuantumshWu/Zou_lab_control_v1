...
\section{它是什么}
Task 控制台（\filepath{task\_console.bat} / \pyapi{frontend.show\_task\_console}）是\tfocus{实验侧}的可配置仪表盘：一个网格里摆任意多个\tfocus{实时面板}（2D 图像 / 1D 向量 / 滚动轨迹 / 分布直方图），每个面板自带参数控件，数据来源用一小段\tfocus{表达式}接到实验信号上。典型布局：左上一张 $2\times2$ 的原子装载图像，右边 $2\times1$ 的计数分布（自动双高斯拟合）与 $2\times1$ 的装载率滚动轨迹，底部一条 $4\times2$ 的逐站装载率。
...
\begin{description}
  ...
  \item[Feed（\filepath{neutral\_atom/operations/feeds.py}）] 生产者循环。\pyapi{VirtualLoadingFeed} 是自带的虚拟原子装载实验：内部用 \pyapi{VirtualTrapArray} 渲染荧光帧，并用与真实流水线\tfocus{相同}的 \pyapi{find\_site\_centers}/\pyapi{roi\_counts}/Otsu 阈值自标定，每个 shot 发布 \pyapi{frame}（相机帧）、\pyapi{counts}（逐站计数）、\pyapi{occupied}（占据）、\pyapi{rate}（滚动装载率）、\pyapi{rate\_sites}/\pyapi{rate\_grid}（逐站/网格装载率图）、\pyapi{shot}。可同步驱动（\pyapi{step()}，测试/Notebook）或后台线程（\pyapi{start(rate\_hz=...)}）。
  \item[面板（panel）] 即现有 live 绘图类的复用：2D=\pyapi{Live2DDis}（自带右侧分布 + 可拖 clim 上下限），分布=\pyapi{HistogramFigure}（自动双高斯拟合 + 可拖阈值线 + 保真度文本），滚动=\pyapi{LiveLiveDis}（轨迹 + 右侧分布），1D=\pyapi{Live1D}。所有图内交互（滚轮缩放、中键平移、右键十字、框选）原样可用。
\end{description}

\section{面板数据来源：表达式而不是下拉框}
每个面板的数据来源是一小段 Python（与 Scan 页代码框同一套信任模型：本地工具、只跑你自己写的代码），把结果赋给 \pyapi{value}：
\begin{codeblock}[Python]
value = frame                          # 最新相机帧（任何 2D 数组表达式都行）
value = rate                           # 标量 -> 滚动轨迹面板
value = history('counts', 200).ravel() # 最近 200 shot 的逐站计数 -> 分布面板
value = rate_grid - b_rate_grid        # 两个实验的装载率图相减 -> 2D 面板
value = np.log1p(frame[20:60, 30:90])  # 任意 numpy 运算 / 切片
\end{codeblock}
命名空间里有：\tfocus{每个信号名}（最新值直接可用）、\pyapi{history(name, n)}（最近 $n$ 个 shot 堆叠，axis 0 是 shot）、\pyapi{latest(name)}、\pyapi{names()}、\pyapi{shot}、\pyapi{np}、\pyapi{math}。改完点 \pyapi{Apply}（未应用的编辑显示 \pyapi{Apply*} 星号）。某个面板表达式报错时，错误只显示在\tfocus{该面板自己的状态行}（红字），其余面板照常刷新。

\section{布局与参数：模数化 plot 区域（confocal 规则）}
面板尺寸是\tfocus{有限的预设组合}（\pyapi{frontend.PANEL\_SIZES}：\pyapi{1x2 / 2x2 / 1x4 / 2x4}，行$\times$列的"半单元"数）。设计规则与 confocal 一致：\tfocus{所有面板类型共享同一块 plot 区域和同一组边距}——\pyapi{2x2} 就是 frontend 的标准 plot 区域（数据区 $480\times360$），\pyapi{1x2} 是它的半高，\pyapi{2x4} 是双宽。2D 类型在\tfocus{自己的区域内部}做 $[0.75, 0.1, 0.1]$ 水平分割（主图 + 侧分布 + colorbar），所以 \pyapi{2x2} 的主图恰好 $360\times360$ \tfocus{正方形}且与侧分布、colorbar 顶底对齐——这正是不同类型面板能整齐排版的原因。接口只有 \pyapi{panel\_plot(data, kind=..., size=...)} 与 \pyapi{panel\_plot\_spec(size)}（\filepath{frontend/live.py}）：\tfocus{尺寸是唯一旋钮，几何与美术不对外暴露}；dpi 与全部字号在任何尺寸下不变。Notebook 里也可以直接调它拿一张"仪表盘规格"的图。

每个面板是一张\tfocus{带标题的卡片}：卡框标题显示\tfocus{面板类型}（2D image / Distribution / Rolling trace / 1D vector），标题行右端一个 \pyapi{Setting} 按钮，画布下方、卡片底部一行灰色状态行（当前 shot 与来源表达式回显）。\tfocus{全部配置都收在 Setting 弹窗里}（confocal 设置风格）：面板标题、\pyapi{size} 预设下拉、各类型自己的参数（2D 的 colormap、滚动轨迹的历史长度、直方图的 bin 数）、来源表达式 + \pyapi{Apply}、\pyapi{Remove} 移除。表达式出错时 \pyapi{Setting} 按钮变红，错误内容在按钮提示与卡内状态行里。

排版不用填行列数字：\tfocus{直接按住卡片边框（画布以外的任何空白处）拖动}，松手时卡片自动\tfocus{吸附到网格槽}。卡片外框大小恰好等于整数个布局槽——图本身的边距不随数据区折半，所以卡框的状态行区按行数吸收这点差额，使得\tfocus{一张 $2\times2$ 卡严格等于两张 $1\times2$ 卡叠起来加一道间隙}。因此任意尺寸混排都严丝合缝，同尺寸卡片完全一致。标题栏的 \pyapi{Add Panel} 按所选类型加新面板；\pyapi{Pause/Resume} 暂停/恢复刷新；\pyapi{Save}/\pyapi{Load} 把\tfocus{整个布局}（面板、位置、尺寸、表达式、参数）存成一个 JSON（默认在 \filepath{tasks/} 目录），\pyapi{Save*} 黄色星号语义与脉冲编辑器一致。面板值的形状变化（换表达式 / 换尺寸 / 改参数）会自动重建该面板的图。窗口大小沿用脉冲编辑器的规则按主屏自适应，超出窗口的部分用滚动条到达。

\section{启动与接线}
双击 \filepath{task\_console.bat}（或 \pyapi{python task\_console.py}）即以\tfocus{虚拟实验}启动：主 feed + 一个 \pyapi{b\_} 前缀的副 feed（装载率不同），所以 $A-B$ 类表达式开箱有数据。常用参数：\pyapi{--rate 4}（shot/s）、\pyapi{--seed 7}（可复现）、\pyapi{--single}（只开主 feed）、\pyapi{--state xxx.json}（启动即载入布局）、\pyapi{--no-feed}（不开虚拟源，自己接 hub）。

从 Notebook 接\tfocus{真实实验}：建一个 hub，把控制台开起来，然后在你的实验循环里每个 shot publish 一次——控制台自动跟上：
\begin{codeblock}[Python]
...
for shot in range(1000):
    ...
    hub.publish({"frame": frame, "counts": det.counts,
                 "occupied": det.occupied.astype(float)})
\end{codeblock}
