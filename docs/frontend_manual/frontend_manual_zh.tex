...
\section{布局与参数：有限尺寸预设 + 钉格子}
面板尺寸不是自由数值，而是\tfocus{有限的预设组合}（\pyapi{frontend.PANEL\_SIZES}：\pyapi{2x1 / 2x2 / 3x1 / 3x2 / 4x1 / 4x2}，单位是网格格子，列$\times$行）。这套预设由 frontend 的公共接口实现——\pyapi{panel\_plot\_spec(kind, size)} 与 \pyapi{panel\_plot(...)}（\filepath{frontend/live.py}），遵守与 \pyapi{pulse\_plot\_spec} 相同的设计纪律：\tfocus{尺寸变化只改数据区像素，dpi 与全部字号（标题/坐标label/刻度）保持不变}，所以不同大小的面板共享同一套视觉语言。Notebook 里也可以直接调它拿一张"仪表盘规格"的图。格子的像素尺寸是 frontend 设计常量（不进布局文件），所以布局 JSON \tfocus{跨机器一致}。

每张面板卡的头部一行：标题框、\pyapi{R/C}（钉到的网格行/列）、\pyapi{size} 预设下拉、各类型自己的参数（2D 的 colormap、滚动轨迹的历史长度、直方图的 bin 数）、移除按钮 \pyapi{X}。卡片的实际大小\tfocus{严格等于}它跨过的格子（含吞掉的格间距），所以所有卡片在网格上严丝合缝。标题栏的 \pyapi{Add Panel} 按所选类型加新面板；\pyapi{Pause/Resume} 暂停/恢复刷新；\pyapi{Save}/\pyapi{Load} 把\tfocus{整个布局}（面板、位置、尺寸、表达式、参数）存成一个 JSON（默认在 \filepath{tasks/} 目录），\pyapi{Save*} 黄色星号语义与脉冲编辑器一致。面板值的形状变化（换表达式 / 换尺寸 / 改参数）会自动重建该面板的图。窗口大小沿用脉冲编辑器的规则按主屏自适应，超出窗口的格子用滚动条到达。
