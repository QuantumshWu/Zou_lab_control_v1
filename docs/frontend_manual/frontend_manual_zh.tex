...
\begin{document}
\frontmatter
\zlctitlepage{Zou\_lab\_control.frontend 中文手册}{Jupyter / GUI 实验绘图前端}{统一 array 数据契约、live session、fit、selector 与 PDF notes}{2026-06-12}
\tableofcontents
\cleardoublepage
\mainmatter
...
\begin{description}
  ...
  \item[\pyapi{Delay \& Scan}] \pyapi{Clock}（\tfocus{只读}显示当前 sequencer 时钟，如 \pyapi{50 MHz · 20 ns}，由硬件固定、不可编辑）、每条可见 channel 的固定延时、以及 \pyapi{Load Array}（载入扫描表文件，旁边显示已载入文件名）和一个\tfocus{来源开关}（关 = 用 Scan 页生成的扫描表；开 = 用载入文件的数组）。Scan 页本身从顶部的 \pyapi{Scan} 标签直接进入（不再有 Scan Tab 按钮）。
...
扫描以\tfocus{命名 slot}为单位（\pyapi{s0, s1, ...}），没有 x/y。只有 \tfocus{duration 和 DAC 值}两类字段可扫，channel 延时\tfocus{不可扫}。
...
\section{保存与读取：完整的一包（pulse + 预览 + 程序 + 扫描表）}
\tfocus{Save 一次写一整包}，四件文件落在同一目录、共享同一个文件名前缀：
\begin{itemize}
  ...
  \item \filepath{<名字>_scan.npy} ——（仅当存在扫描表时）扫描表原始数组。
\end{itemize}
某一件写失败不会让整个保存失败：状态栏会写明哪些写成功、哪些被跳过及原因。

\tfocus{Load 把整包读回来}：在 Edit 页点读取并选 \filepath{<名字>.json}，编辑器还原脉冲本体；若同目录有配套的 \filepath{<名字>_scan.npy}，它会被\tfocus{自动重新挂回}"载入文件"来源（Delay / Scan 面板会显示该文件名），内嵌在 .json 里的扫描表则照常成为"生成"来源——两者在保存时刻数值相同，来源开关保持在"生成"。\tfocus{误选了} \filepath{_program.json} 或 \filepath{_scan.npy}？只要同目录有对应的脉冲 \filepath{.json}，软件会自动改为加载脉冲本体并提示一句。\filepath{.png} 与 \filepath{_program.json} 是衍生物，load 后由当前状态随时重新生成，无需单独读取。
...
\begin{description}
\item[Load Array 按钮] 作用：打开一个\tfocus{文件选择对话框}，从磁盘载入一份\tfocus{扫描表（scan table）}文件，存为“载入”来源。可接受的文件类型是 \pyapi{.npy} / \pyapi{.csv} / \pyapi{.txt} / \pyapi{.json}。文件的格式约定非常重要：\tfocus{每一行代表一个扫描点，每一列代表一个已绑定的扫描槽}。点击后会发生什么：弹出系统文件对话框，你选中文件后，表里的数值被读进来并\tfocus{自动 snap}（duration 对齐节拍、DAC 夹到带符号范围 \pyapi{-512..+511}），同时\tfocus{右边的标签显示该文件路径的末尾十几个字符}，并把来源开关\tfocus{自动打开}（即开始使用这份载入数组）。常见的坑：行列别搞反（行=点、列=槽）；列数和槽数对不上时会被截断/补零。实用提示：不想手写文件就用 Scan 页的代码生成；Load Array 更适合从外部仪器或脚本导出的现成数据。
...
\item[去哪写扫描代码] 旧版那个“Scan Tab”按钮已\tfocus{取消}——直接点界面顶部的 \pyapi{Scan} 标签即可进入 Scan 页写代码（见“按字段扫描点工作流”一节）。
\end{description}
...
\begin{description}
\item[单位下拉框（ns / us）] 作用：选择该行延迟数值的\tfocus{时间单位}。延迟的可选单位\tfocus{只有 \pyapi{ns} 和 \pyapi{us} 两个}——延迟被延时线深度限制在约 $\pm40\,\mu$s 以内，\pyapi{ms}/\pyapi{s} 一定超界，而 \pyapi{str (ns)}（仿射表达式单位）对一个固定、不可扫描的延迟也没有意义，所以这里不再提供。点击后会发生什么：下拉展开，选中某个单位后，左边输入框里的数字就按这个单位解释（例如填 \pyapi{1} 配 \pyapi{us} 等于 1 微秒）。常见的坑：单位选错会让延迟差出 1000 倍（把 \pyapi{us} 当成 \pyapi{ns}）；另外最终值仍会被时钟节拍量化，所以填一个比节拍还小的组合可能被对齐成 \pyapi{0}。实用提示：日常多用 \pyapi{ns}，较长的固定延迟用 \pyapi{us}。
...
\item[clk 按钮（把通道接到 FPGA 时钟）] 作用：把\tfocus{这一条通道的输出引脚接到 FPGA 时钟}，用来给外部设备（比如 DAC）提供锁存选通。点击后会发生什么：该通道被标记为 clk 通道——它\tfocus{从脉冲引擎里移除}（不再由边沿表驱动，所以引擎\tfocus{绝不会覆盖}它的 clk 行为），它这一行的\tfocus{延迟框和单位框都变灰禁止编辑}（对一条 clk 通道没有意义），并且\tfocus{从 Preview 里隐去}（它输出的是时钟方波，不是脉冲）。再点一次恢复成普通的引擎驱动通道。实现上，编译时把该通道的 mask 位强制清 0，并把一个 62 位的 \pyapi{clk_enable} 掩码随程序下发；FPGA 顶层用 \pyapi{out_final[n] = clk_en[n] ? ~clk : out[n]} 把\tfocus{反相的 clk}接到该引脚（运行时由 CTRL 寄存器选择，换 clk 通道\tfocus{不需要重新综合}，但首次启用这套 mux 需要一次重新构建 bitstream）。\tfocus{为什么是反相 clk}：DAC 的并行数据位是在 clk 上升沿改变的，若选通也用上升沿，DAC 会在数据正在跳变的瞬间锁存而锁进\tfocus{错误的中间码}（曾表现为"两个 DA edge 段之间偶发出现第三种值"）；用反相 clk 让 DAC 改在下降沿（数据眼图正中、最稳）锁存，从根本上消除这个竞争。常见的坑：DAC 总线（da\_dipole 等）的成员位\tfocus{不能}单独设成 clk（会破坏 DAC），所以这些行的 clk 按钮是禁用的。实用提示：要给外部 DAC 喂时钟，就把对应那条通道（如某个 da\_clk 线）设成 clk，不用再在脉冲里手动翻转它。
\end{description}
...
\item[常见坑] \tfocus{时长为 0 是不允许的}——一个 period 必须有正的持续时间，填 0 会被拒绝。想让时长可扫描，\tfocus{点扫描圆点}即可,别去找"输入表达式"的入口（已不再需要手动切单位）。
...
\begin{description}
\item[Save] \tfocus{作用}：把当前脉冲保存成\tfocus{完整的一包}磁盘文件。\tfocus{接受什么}：动作按钮，无需在按钮上输入参数（实际文件名通过保存流程确定）。\tfocus{点下去会发生什么}：它会写出 \pyapi{.json} 脉冲本体 + \pyapi{.png} 预览图 + \pyapi{<name>_program.json}\tfocus{（编译好的、FPGA 可直接上载的程序——无论是否扫描都会保存，与 On Pulse 实际上传的内容一致）}；如果当前存在扫描表（scan table），还会再写出 \pyapi{<name>_scan.npy} 数组文件。也就是说一次 Save 产出三到四个配套文件，方便你完整复现这次实验设置；某一件写失败时状态栏会写明跳过了哪件及原因。\tfocus{状态指示（confocal 同款）}：有\tfocus{未保存的修改}时按钮显示\tfocus{黄色}且文字带星号（\pyapi{Save*}），标题栏同时带星号；内容保存干净后变回\tfocus{蓝色}的 \pyapi{Save}。\tfocus{实用提示}：把「\pyapi{Save} 蓝色 + 标题栏星号消失」当作「已存盘」的可靠信号；运行前如果看到 \pyapi{Save*} 还是黄色，说明你最新的改动还没落盘，建议先 Save 再 On Pulse，避免事后找不到这次用的是哪份配置。

\item[Load] \tfocus{作用}：从磁盘\tfocus{载入}一个脉冲 \pyapi{.json} 文件，并\tfocus{把保存时的整包关联一起恢复}。\tfocus{接受什么}：动作按钮；会让你选择一个之前保存过的 \pyapi{.json}。\tfocus{点下去会发生什么}：选中的脉冲文件被读入，画布上的周期、通道、延时、扫描等设置会替换成文件里的内容；若同目录有配套的 \pyapi{<name>_scan.npy}，它会被自动重新挂回"载入文件"来源（Delay / Scan 面板显示该文件名）。\tfocus{误选}了 \pyapi{_program.json} 或 \pyapi{_scan.npy} 时，只要同目录有对应的脉冲 \pyapi{.json}，会自动改为加载脉冲本体并提示。\tfocus{常见坑}：Load 会用文件内容\tfocus{覆盖}当前编辑区——如果你当前有未保存的改动（Save 还是黄色、标题栏有星号），载入前务必先 Save，否则改动会丢失。\tfocus{实用提示}：Save 与 Load 是一对——一包文件同名成套（\pyapi{.json}、\pyapi{.png}、\pyapi{_program.json}、\pyapi{_scan.npy}），日后一眼认出哪份配置对应哪次实验，复现和归档都更省心。
\end{description}

\begin{notebox}[一次 Save 到底产生几个文件？]
通常三个：\pyapi{.json} 脉冲本体 + \pyapi{.png} 预览图 + \pyapi{<name>_program.json} 编译好的 FPGA 程序（\tfocus{无论是否扫描都有}）。设置了扫描表时是四个（再加 \pyapi{<name>_scan.npy} 扫描数组）。看到 Save 变蓝、标题栏星号消失，就代表这一套都已经写好了。
\end{notebox}
...
\begin{description}
\item[它显示什么] 每行形如 \pyapi{s0: Period 4 duration [ns] (nominal 50000)}、\pyapi{s1: da\_dipole (period 2) [value]}。冒号前是槽号（与代码里要用的变量名一一对应），冒号后是这个槽绑定到的字段的人类可读描述：是哪个周期的 duration，还是哪个周期的某个 DAC 总线值。
\item[单位约定] 方括号里的单位非常关键。时间类的槽（duration、delay）单位是 \pyapi{[ns]}（纳秒），你在代码里填的数字就按纳秒理解；DAC 类的槽单位是 \pyapi{[value]}，表示\tfocus{带符号 DAC 值}（LSB，0 = 0 V），不是电压、也不是纳秒，取值范围是整数 \pyapi{-512} 到 \pyapi{+511}（线上自动换算成 offset-binary 码）。把这两类单位搞混是新手最常见的错误。
...
\item[必须满足的契约] 代码运行完，命名空间里\tfocus{必须存在}一个名为 \pyapi{scan\_table} 的变量，且它是一个二维数组：行数 = 扫描点个数（你自己定，比如 21、50、100），列数 = \pyapi{n\_slots}。第 \pyapi{j} 列填的就是槽 \pyapi{s\{j\}} 在它\tfocus{显示单位}下的数值（时间槽填 ns、DAC 槽填 -512..+511 的带符号值，0 = 0 V）。
...
\item[Template: column\_stack 按钮] 作用：把\tfocus{默认模板}填进编辑器——每个槽一列、用 \pyapi{np.column\_stack} 拼装。它\tfocus{按当前绑定的槽数自动适配}（一个槽给单列，两个槽给"反相关、总和恒定"的示例）。这是新手最该先点的按钮。
...
\begin{enumerate}
\item \tfocus{在 Edit 页绑定字段}：找到你想扫的字段（某周期的 duration、或某周期的 DAC 总线值），点它旁边的\tfocus{扫描圆点}。第一个绑定的字段成为 \pyapi{s0}，第二个成为 \pyapi{s1}，依次类推。绑定后该字段会被高亮标记并显示槽号。
...
如果你绑定的是一个 DAC 总线值字段（\pyapi{[value]} 单位），它对应带符号 DAC 值（0 = 0 V），合法范围是整数 \pyapi{-512} 到 \pyapi{+511}。假设它是唯一绑定的槽 \pyapi{s0}，下面在整个量程上扫 \pyapi{32} 个点：
...
二维扫描就是把两个槽的取值做\tfocus{笛卡尔网格}，每个组合是一个扫描点。假设 \pyapi{s0} 是 duration（ns），\pyapi{s1} 是 DAC 带符号值（-512..+511）。下面构造一个 \pyapi{11 × 8 = 88} 点的网格：
...
\item[提示] 想加第三维（比如再扫一个 DAC 值槽 \pyapi{s2}），把它也放进 \pyapi{meshgrid} 并在 \pyapi{column\_stack} 里加一列即可。
\end{description}

\begin{notebox}[同时扫多个物理量]
DAC 值与 duration 两类槽\tfocus{可以在同一轮里一起扫}（通道延时是固定值，不参与扫描）。你在 Edit 页绑定几个字段、Scan 页就给几列填数，硬件会按行无缝执行这张多列扫描表，每个扫描点照常发一次相机触发。这让"沿某条轨迹同步改变多个参数"或"多维网格扫描"成为一次 On Pulse 就能完成的事。
\end{notebox}

\section{Sync 按钮、UNSYNCED 状态与 period 选择（2026-06 新增）}
...
\begin{itemize}
  ...
  \item \tfocus{UNSYNCED（\pyapi{On Pulse*} 星号 + 橙色状态点）}：设备在 RUNNING/PREPARED 状态下，\tfocus{任何}编辑（duration/delay/DA/scan/clk/显隐/载入……）都会给 On Pulse 按钮加上 confocal 同款的\tfocus{星号}并把\tfocus{标题栏状态点}变橙——提示"设备上跑的已不是你眼前的参数，按一下应用"。按钮本身\tfocus{保持绿色}（颜色不用于表达状态，避免与 Remove/Load/Sync 的常驻橙色混淆）。若把修改改回原样，星号与橙点自动消失；重新 On Pulse 即清除。Save 的 \pyapi{Save*} 黄色+星号语义表示"未保存到文件"，与运行状态相互独立。
  ...
  \item \tfocus{Clear All（标题栏右端，橙色）}：一键把\tfocus{日程}清空——删除所有 period 与所有通道/总线延时，只留下\tfocus{一个 1 \textmu s 的空白 period}（没有任何通道为 on，DA 总线为 Hold 空闲）；扫描绑定、扫描表与 repeat bracket 同时清除。\tfocus{保留}的是硬件接线层信息：通道名/语义标签、显示/隐藏状态、DA 总线分组与 clk 指派。清空属于普通编辑：会照常触发 \pyapi{Save*}（未保存）与 UNSYNCED 星号，不会自动写盘或上传——从零搭一个新脉冲前先点它最方便。
  \item \tfocus{单 period 保护}：只剩一个 period 时 Remove 被禁用（脉冲表至少要有一个 period）。
  \item \tfocus{拖动排序}：拖动时光标下有半透明卡片缩略图，插入位置竖线为覆盖层（不再引起整行重排抖动），拖到视口边缘会自动滚动——后面的 period 始终可达。
  \item \tfocus{预览横轴自动加宽}：与纵轴的行块一致，每 5 个 period 增加一个基础宽度块，最多 3 倍（\pyapi{live.PULSE_X_PERIODS_PER_BLOCK} / \pyapi{PULSE_X_MAX_WIDTH_FACTOR} 可配置）。深度缩放下刻度标签按间距自适应精度，不再退化成一排相同的 "8ms"。
...
打开后默认停在 \tfocus{Edit} 页。顶部三个标签 \pyapi{Edit / Preview / Scan}，顶部状态条显示可见数、period 数、step、总时长等；状态条右端是 \pyapi{Clear All} 按钮（一键清空日程到一个 1 \textmu s 的空白 period，通道名/显隐/总线分组/clk 指派保留——从零开始搭新脉冲时先点它）。
...
数据通路分三层，互相只通过\tfocus{信号集线器（SignalHub）}耦合：
\begin{description}
  ...
  \item[Feed（\filepath{neutral\_atom/operations/feeds.py}）] 生产者循环。\pyapi{VirtualLoadingFeed} 是自带的虚拟原子装载实验：内部用 \pyapi{VirtualTrapArray} 渲染荧光帧，并用与真实流水线\tfocus{相同}的 \pyapi{find\_site\_centers}/\pyapi{roi\_counts}/Otsu 阈值自标定，每个 shot 发布 \pyapi{frame}（相机帧）、\pyapi{counts}（逐站计数）、\pyapi{occupied}（占据）、\pyapi{rate}（滚动装载率）、\pyapi{rate\_sites}/\pyapi{rate\_grid}（逐站/网格装载率）、\pyapi{centers}（标定的站点中心，$(N,2)$ 像素坐标）、\pyapi{thresholds}（逐站阈值）、\pyapi{shot}。可同步驱动（\pyapi{step()}，测试/Notebook）或后台线程（\pyapi{start(rate\_hz=...)}）。
  \item[面板（panel）] 即 frontend live 绘图类的复用，所有图内交互（滚轮缩放、中键平移、右键十字、框选、拖 clim/阈值线）原样可用。
\end{description}

\section{五种面板类型}
\begin{description}
  \item[2D image（\pyapi{Live2DDis}）] 任意二维数组（相机帧、装载率网格、两台实验之差）。自带右侧分布与 colorbar，可在分布带上拖动 clim 上下限。
  \item[Site map（\pyapi{LiveSiteMap}）] \tfocus{原子阵列站点图}：每个阱位一个实心圆，颜色编码该站的值（占据 0/1、逐站装载率、保真度……），底下可垫一张实时相机帧。站点位置来自一个 \pyapi{centers} 信号（$(N,2)$ 相机像素坐标，真实实验里就是 \pyapi{TrapCalibration.centers}）。主图与 colorbar 的几何与 2D 面板逐像素对齐。
  \item[Distribution（\pyapi{HistogramFigure}）] 直方图 + 自动双高斯拟合 + 可拖阈值线 + 保真度文本——把 \pyapi{history('counts', 200).ravel()} 接进来就是读出阈值的实时体检。
  \item[Rolling trace（\pyapi{LiveLiveDis}）] 标量滚动轨迹 + 右侧分布，图内右上角常显最新值。装载率、保真度、温度等"一条曲线盯一个数"的场景用它。
  \item[1D vector（\pyapi{Live1D}）] 一维向量逐点连线（逐站装载率、逐站计数……横轴是站号）。
\end{description}

\section{Setting：每个面板的全部配置}
卡片标题行右端的 \pyapi{Setting} 文字按钮打开该面板的设置弹窗（confocal 风格），从上到下三组——\tfocus{你想换数据就动 Source，想换显示就动 Display，全部即时生效}：
\begin{description}
  ...
  \item[Display（怎么显示）] \pyapi{size} 尺寸预设 + 该类型自己的参数，全部\tfocus{改动即生效}：2D 的 colormap；Site map 的 \pyapi{centers}/\pyapi{image} 信号名与 colormap（\pyapi{image} 留空＝不垫相机帧）；Rolling trace 的历史长度；Distribution 的 bin 数。这些参数是\tfocus{声明式}的（\pyapi{PANEL\_PARAMS} 里一行一个 \pyapi{ParamSpec}），控件由声明自动生成，并随布局 JSON 一起保存。
  \item[Panel（这张卡）] 面板标题（显示在图内）与 \pyapi{Remove} 移除。
\end{description}
常用表达式示例：
\begin{codeblock}[Python]
...
每个面板是一张\tfocus{带阴影的标题卡片}（fluent 卡片设计），卡框标题＝面板类型，画布下方、卡片底部一行灰色状态行（当前 shot 与来源表达式回显）。排版不用填行列数字：\tfocus{按住卡片边框（画布以外的任何空白处）拖动}，松手时卡片自动\tfocus{吸附到网格槽}；\tfocus{卡片间不允许重叠}——落点被占时，被压住的卡会被向下\tfocus{挤开}（级联让位），拖动的卡始终落在你放下的位置。卡片外框恰好等于整数个布局槽（卡框状态行区吸收图边距的模数差额），所以\tfocus{一张 $2\times2$ 卡严格等于两张 $1\times2$ 卡叠起来加一道间隙}，任意尺寸混排严丝合缝。标题栏：\pyapi{Add Panel} 按所选类型加面板，\pyapi{Pause/Resume} 暂停刷新，面板值形状变化（换表达式/尺寸/参数）自动重建该面板。

\section{命名任务仪表盘：设计一次，随处载回}
布局（面板、位置、尺寸、表达式、参数）整包存成\tfocus{一个 JSON}，文件名就是任务名。三个入口完全等价：
\begin{itemize}
  \item \tfocus{GUI}：标题栏的 \pyapi{Presets…} 下拉列出全部可载任务（内置 + \filepath{tasks/} 目录里的每个 JSON），选中即整套载入；\pyapi{Save} 默认存到 \filepath{tasks/任务名.json}，存完立即出现在下拉里。\pyapi{Save*} 黄星＝有未保存改动（与脉冲编辑器同语义）。
  \item \tfocus{命令行}：\pyapi{task\_console.bat --task atom\_loading\_monitor}。
  \item \tfocus{Notebook}：\pyapi{zf.show\_task\_console(hub=hub, task="atom\_loading\_monitor")}。
\end{itemize}
内置两套出厂任务：\pyapi{atom\_loading\_monitor}（装载图 + 占据 Site map + 计数分布 + 装载率轨迹 + 逐站条）与 \pyapi{loading\_rate\_live}（轻量装载率监控，无相机大图）。\tfocus{在 \filepath{tasks/} 里保存同名 JSON 即覆盖内置版本}——想改造出厂布局，载入、拖一拖、Save 就完成了。配置一套新任务（比如 \pyapi{atom\_temp\_monitor}）的完整流程：载入最接近的现有任务 → 增删面板、把 Source 指到温度实验发布的信号 → 改任务名 → \pyapi{Save}。

\section{启动与接线}
双击 \filepath{task\_console.bat}（或 \pyapi{python task\_console.py}）即以\tfocus{虚拟实验}启动：主 feed + 一个 \pyapi{b\_} 前缀的副 feed，$A-B$ 类表达式开箱有数据。常用参数：\pyapi{--task 名字}（命名任务）、\pyapi{--rate 4}（shot/s）、\pyapi{--seed 7}（可复现）、\pyapi{--single}（只开主 feed）、\pyapi{--state xxx.json}（按路径载入）、\pyapi{--no-feed}（不开虚拟源，自己接 hub）。

从 Notebook 接\tfocus{真实实验}：建一个 hub，开控制台，在实验循环里每个 shot publish 一次——发布的信号集尽量沿用虚拟 feed 的命名（\pyapi{frame}/\pyapi{counts}/\pyapi{occupied}/\pyapi{centers}/\pyapi{rate}……），这样同一套命名任务在虚拟和真机之间\tfocus{零修改通用}：
\begin{codeblock}[Python]
...
\section{给维护者：扩展面板}
给某类面板\tfocus{加一个参数}＝在 \filepath{frontend/task\_console.py} 的 \pyapi{PANEL\_PARAMS} 里加一行 \pyapi{ParamSpec(键, 标签, 类型, 默认值, ...)}（类型 \pyapi{choice}/\pyapi{int}/\pyapi{signal}），Setting 控件、JSON 保存与图重建全部自动跟上；在对应的 \pyapi{\_build\_plot} 分支里消费 \pyapi{config.params[键]}。\tfocus{加一种新面板类型}：在 \filepath{frontend/live.py} 写一个 \pyapi{BaseLivePlot} 子类并进 \pyapi{plot()} 工厂的 kind 分发（几何必须走 \pyapi{panel\_plot\_spec} 的共享区域与边距），然后在 \pyapi{PANEL\_KINDS}/\pyapi{\_DEFAULT\_SOURCES}/\pyapi{PANEL\_PARAMS} 各加一项、\pyapi{\_coerce}/\pyapi{\_build\_plot}/\pyapi{\_feed} 各加一个分支。可视化改动必须用 \pyapi{devtools.capture\_user\_view} 三档缩放截图验收（\pyapi{parity} 目标同屏对比两个 GUI 的控件尺寸）。
