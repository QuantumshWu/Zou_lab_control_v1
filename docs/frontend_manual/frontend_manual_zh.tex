...
\begin{itemize}
  ...
  \item \tfocus{UNSYNCED（橙色 On Pulse）}：设备在 RUNNING/PREPARED 状态下，\tfocus{任何}编辑（duration/delay/DA/scan/clk/显隐/载入……）都会把 On Pulse 按钮和状态点变橙——提示"设备上跑的已不是你眼前的参数，按一下应用"。若把修改改回原样，橙色自动恢复；重新 On Pulse 即清除。Save 的黄色+星号语义不变（未保存到文件）。
  ...
  \item \tfocus{通道延时}：上限现在约 $\pm 42.9$ s（FPGA 端事件调度器；毫秒级 emCCD 延时直接填入即可）。约束变为"任一延时窗口内该通道翻转 $\le 16$ 次"——稀疏触发信号天然满足，超限会在编译时给出明确报错。
\end{itemize}
