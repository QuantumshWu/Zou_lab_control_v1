...
\begin{itemize}
  ...
  \item \tfocus{通道延时}：上限现在约 $\pm 42.9$ s（FPGA 端事件调度器；毫秒级 emCCD 延时直接填入即可）。硬件约束是"该通道任一延时窗口内的\tfocus{在途翻转}数 $\le$ 事件 FIFO 深度（默认 16）"——编译器对\tfocus{完整日程}（每个扫描点、repeat 回绕缝、跨多帧的长窗口）做精确计数，稀疏触发信号天然满足，超限会在编译时给出明确报错。无限循环（repeat forever）下超过一个循环周期的延时会自动按周期取模写入硬件（稳态时序逐 tick 不变，只是启动期提前开始输出）。
\end{itemize}
