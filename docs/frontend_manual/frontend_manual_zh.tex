...
无论用 Run、Load Array 还是 API，扫描表在背后都会被\tfocus{自动 snap 成硬件能跑的合法值}：\tfocus{duration} 列对齐到\tfocus{整数个节拍}（且至少 1 个节拍，不会塌成 0），\tfocus{DAC} 列四舍五入成\tfocus{整数 code} 并夹到该总线位宽对应的范围（10-bit 即 \pyapi{0..1023}）。Scan 页信息栏会逐列写明每个 slot 的允许取值与 snap 规则，所以预览/保存的表与实际下发的完全一致。
\end{notebox}
...
\begin{description}
\item[Load Array 按钮] 作用：打开一个\tfocus{文件选择对话框}，从磁盘载入一份\tfocus{扫描表（scan table）}文件，存为“载入”来源。可接受的文件类型是 \pyapi{.npy} / \pyapi{.csv} / \pyapi{.txt} / \pyapi{.json}。文件的格式约定非常重要：\tfocus{每一行代表一个扫描点，每一列代表一个已绑定的扫描槽}。点击后会发生什么：弹出系统文件对话框，你选中文件后，表里的数值被读进来并\tfocus{自动 snap}（duration 对齐节拍、DAC 夹到 \pyapi{0..1023}），同时\tfocus{右边的标签显示该文件路径的末尾十几个字符}，并把来源开关\tfocus{自动打开}（即开始使用这份载入数组）。常见的坑：行列别搞反（行=点、列=槽）；列数和槽数对不上时会被截断/补零。实用提示：不想手写文件就用 Scan 页的代码生成；Load Array 更适合从外部仪器或脚本导出的现成数据。
...
\item[接受什么值] 一个 \pyapi{0} 到 \pyapi{1023} 之间的\tfocus{整数}码值（10 位）。这个框\tfocus{只接受整数}（小数点、字母等字符在键入时被拒绝），并把值约束在 \pyapi{[0, 1023]}——键入明显过大的数会被挡住，编辑完成时也会再夹一次。\tfocus{请注意这是原始码值，不是电压}——它直接对应 DAC 的数字输入，至于换算成多少伏，取决于硬件配置，本框里只管填码。
