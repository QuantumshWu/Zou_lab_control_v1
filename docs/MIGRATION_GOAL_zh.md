...
**工作单元 = 一个窗口**,而每个窗口的**第一刀必须是切入口**。

> **顺序铁律(2026-07-20 第二次栽同一个坑后写死)**:
> **入口先切,缝后清。** 缝(TENDRILS)是 Z0 的事——让旧文件能被 `zlc_frontend` import;
> 它和"入口返回哪个窗口"**没有任何依赖关系**。旧 console 今天就能构造(行为 gate 每次跑
> 都在构造它),所以入口可以**立刻**指向它。
>
> 我第一次栽这个坑写下了"禁止只搬基础设施不切入口",然后**又干了 8 个 commit 的同一件事**
> (F1–F4 + Z 守卫 + shape_text + measurement_slug + 端口),因为我错误地以为
> "切入口要先清缝"。用户打开 .bat 看到的东西整整几个小时**没有任何变化**,
> `git log --name-only` 里**一个** `workbench/` / 根启动器 / `.bat` 文件都没有。
>
> **机械判据(每个 window 切片必须先满足,否则不许写别的代码)**:
> `git log --name-only` 里这一刀**必须**包含入口文件
> (`Zou_lab_control/notebook/facade.py` 或 `Zou_lab_control/workbench/__init__.py`
> 或根 `task_console.py`/`pulse_gui.py`/`figure_viewer.py`)。
> **测试必须断言"入口返回的窗口",绝不许直接构造类**——直接构造类测的是用户打不开的东西。

每窗的 commit 顺序:**① 入口切到旧 UI(允许面板暂时是空的,但必须能开)→ ② 逐块把数据源
换成新架构 → ③ 删掉被取代的重画壳及其专属测试**。缝清理随时可插空做,但**永远不是前置条件**。
...
**F1–F4 与 §四 Z 守卫均已闭合。现在起,每一刀都必须先动入口。**

W1 的下一刀 = **把 `exp.task_console()` / 根启动器切到旧 console**,接受面板先是空的;
`test_w1_console_ux_oracle.py` 同时改为断言**入口返回的窗口**(现在它直接构造 `TaskConsole`,
测的是用户打不开的东西——已由用户指出,是真错)。顺序:
