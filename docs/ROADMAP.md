...
6. **MOT 磁场寻优全链(commit de46622,2026-07)**:第二只相机 `monitor_camera` 设备对(`PylonCamera` / `VirtualMotCamera`);`mot_roi_intensity`(disc−annulus 背景扣除)住在 task 内部,是一键寻优的数学单源;live 标量 y 由通用 ROI crop 链(2d 面板框选 → `<slug>_roi_value`)提供,专用 MOT-intensity processor 已删。一键 `Optimize MOT field` 要求模板三根 DAC 都绑定 hardware scan slots，整张三维表一次上传，由 task 读取相机与强度并求最优点。通用 PulseScan 不拥有相机，但同时支持 `scan_slot` 整表执行与 `api_slot` 逐点有限 pulse；MOT-field 模板只允许前者。
...
**13 问轮(2026-07-10)全落地(无暂缓)**
- **一批修(逐主题 commit)**:#7 pulse-scan 默认 y 解绑(不再硬编码外部 `rate`,blank=plot 面板同款"未选";declared==published:`declared_output_keys` 单源经 `spec.metadata['declared_keys']`)· #12 task 信号统一声明形状语法(`contract_shape_label` 单源 `R × P × (data)`;`_describe_from_schema` 让 task/hub 分支同源,cali/mot-field 不再多套外层括号)· #6 `CameraDevice.arm()` 有界+可取消(`stop`/`timeout`,只在锁被占用时才等待/取消;MOT-field 卡 0% 且 Stop 救不回的根因)· #2 面板板改**顺序驱动**严格左上重力打包(`pack(order)`+`drop_index`,Add 追加到底、无中洞、点击/改尺寸确定性重排;删 `_gravity_slot/_compact/_snap_drop_anchor/_spans_mostly_overlap`)· #5 fit clear 恢复原 alpha(单 overlay owner,99a4757)· #10 选择动作**对称拆除**(`selection_clear_sink(card,action)` 单源:离开 roi/fit 即停+删对应 hub 节点)· #1 Edit tab 快照刷新 best-effort(`SignalHistoryGap` 不再令 tab 打不开)。
- **fit 子系统统一(#8/#3/#9/#11/#1b,commit 6a53f4c/02c99ec/0bd8c53/e5c213f/7c5c477)**:fit 唯一状态=`config.params['fit_request']` 在场⟺fit-on(**删** `selection_action=='fit'`,`selection_action` 收敛 {none,roi});`PanelCard.set_fit_request` 单 mutator,Setting 开关与 Edit combo 都经 `_apply_analysis_state_to_widgets` 从此键派生(#8);按 `render_family` 的 `live.general_fit_models` 能力表让 hist 与 bimodal 并存 gaussian/bi-gaussian、sites 返 `[]`(#3);umbrella UI 更名 **"Analysis"**(动作 {none, curve fit, ROI});`build_fit_request` + `_FitFixSeedEditor` 走 typed `FitRequest.fixed/initial` 恢复"固定参数"(#1b);`_draw_fit_result` snapshot/restore xlim/ylim/clim/autoscale + 图像族不 dim data-line alpha,overlay 全程不改视图(#11);`FitProcessor.provides`/`transform`/`_bare_output_specs` 由 `FitModel.names` 派生、`FIT_SPEC_NAME`→"Frame fit"、`fit_size`→`fit_radius`(#9)。
- **kind-agnostic ROI(#4,commit 26c8b4b/5bc9f73)**:`RoiProcessor` 停携带四像素端点,改**携带 `Selection`+`reduce`**(`set_selection` mutator,零 per-kind 分支);删 `x_min/x_max/y_min/y_max`/`region_values`/`region_to_acquisition_parameters`/`_bounds`/`data_shape!=2` 硬门。`roi_value` 恒有(每 kind:对落在 Selection 内的格 reduce),`roi_frame`=轴 crop(image/1d 连续轴)或 value-mask(hist)时 NaN 掩码 passthrough(稳定 schema)。**单源**:`live.region_binding(kind,...)` 是 `coerce_panel_value` 的逆,轴绑定经 `Selection.metadata['binding']` 以纯数据下发(`neutral_atom` 不 import frontend);`kind_supports_roi`/`ROI_KINDS` 是 Analysis combo 唯一来源(grid/pulse 除外),删 `_card_y_is_view_axis` ROI 门。守卫:`test_roi_kind_agnostic`(1d/hist/sites 返有限 roi_value;`region_binding` 逐 kind 往返 `coerce_panel_value`)+ `test_fit_overlay_view`。

- **Analysis 一等 processor + region 控制信号(goal4 #1/#2/#7)**:目录合一为单条 `Analysis` spec(`processors/analysis.py`,`AnalysisProcessor` 继承 fit/roi 两策略,`action` choice 单源 `ANALYSIS_ACTIONS`,声明键走 `declared_keys` deriver;手动表单可直接打 region 边界);panel↔row 关联=**持久单源**(panel `params['region_signal']`==row `values['region']`,运行时 dict 全删,save/load 天然重关联、改名不脱钩、跨面板不可能串线);region 信号改为**定长 JSON payload+字节稳定 schema**(`region_tensor`,`role='control'` 单源分类:picker 池过滤+GC 豁免同一判据),re-drag 永不 replace schema、`RegionProcessor._drain_region` gap 自愈;stop 语义=冻结(停行重拖=原地 retarget+提示,不删不自启);selectors-off 走 `set_fit_request(None)` 消 zombie fit 态。守卫:`test_analysis_lifecycle` + 重写的 selector/roi/fit 契约组。
...
> `references/` 是 git-ignored 的本地历史归档，不是 production 依赖，也不天然具有设计或算法权威。它只可用于追溯和提供测试输入；具体子系统必须服从用户指定的已验证实现（calibration/readout 当前以 `main@6c337d49` 为唯一物理与算法基准），不得从归档反向发明 production 合同。
