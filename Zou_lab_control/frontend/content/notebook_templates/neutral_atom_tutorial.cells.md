...
- `exp.device_manager()`：**设备管理 GUI（config 编辑器）**——左栏按角色类型（Camera / Sequencer / Trap array / 未来的 RF …）逐台列出设备：名字、类型下拉（按角色过滤全部已注册设备类）、由设备类自声明的参数表单（`$device:` 交叉引用是下拉、容器参数是 JSON 字面量，永不 eval），可增删设备、改名、New/Load/Save 整份 config；右栏 “Scan hardware” 扫总线（发现行一键 “Add to config”）+ 已载实例列表（Snapshot / “Open devices” 初始化硬件）。底栏 **Apply** 把编辑中的 config 应用到会话（对应 `exp.load_config(dict)`，坏 config 不伤现场）；改动未应用/未保存分别有状态点与星号提示。**它也是 init device 的入口**：没有会话时 `na.device_manager()` 直接打开编辑器，配好点 “Init devices” 即 `na.connect(该 config)`。task console 顶栏也有个 “Devices” 按钮开它。它是 `na.load_devices` / `na.discover_devices` 的图形面。
