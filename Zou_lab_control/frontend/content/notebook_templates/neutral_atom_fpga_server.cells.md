...
MAX_EDGES = int(os.environ.get("ZLC_PS_MAX_EDGES", "512"))
MAX_SCAN_POINTS = int(os.environ.get("ZLC_PS_MAX_SCAN_POINTS", "256"))
...
For scans, the GUI/API sends one named scan table, for example
`# vars: camera_exposure_ns(ns), trig_delay(ns)`. The host currently packs at
most two active timing parameters into the two hardware scan columns. The
compiler accepts a scan template only when edge ordering stays the same for
every scan row. If two symbolic edges would swap order, split the scan or
