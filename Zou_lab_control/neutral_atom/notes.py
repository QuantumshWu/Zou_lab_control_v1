...
scan = exp.readout.detection_time(times, shots=30, live=False)
fit_result, popt = scan.data_figure.decay()
scan.summary(), fit_result, popt
\end{{codeblock}}

\pyapi{{detection_time}} 默认 \pyapi{{live=True}}。cell 会很快返回，scan 在 frontend-owned worker 里继续跑；worker 负责采集并写共享 \pyapi{{data_y}}，plot timer 只刷新图。这样采集不会阻塞 Jupyter UI，也给未来 PyQt 主线程绘图留出边界。上面的教程调用显式使用 \pyapi{{live=False}}，只是为了在同一个 cell 里立刻得到 completed \pyapi{{scan}} 并直接调用 \pyapi{{scan.data\_figure.decay()}}。
...
\section{{中断 live scan：一个对外 stop}}
长扫描对外只提供一个 stop：
\begin{{codeblock}}[Python]
interrupt_scan = exp.readout.detection_time(np.linspace(0.2e-3, 20e-3, 200), shots=30)
interrupt_scan.stop()
...
拟合前要保证数据已经采完。交互式实验里可以等 live 图跑完再执行拟合；教程和脚本里如果要立刻拟合，就把这一次 scan 设成 \pyapi{{live=False}}，让它在当前 cell 内采完。
