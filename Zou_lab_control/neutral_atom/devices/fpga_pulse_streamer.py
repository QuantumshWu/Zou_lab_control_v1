...
class PulseStreamerProbeNames:
    ...
    bus_prog_start_tick_coeffs: str = "zlc_bus_prog_start_tick_coeffs"
    bus_prog_stop_tick_coeffs: str = "zlc_bus_prog_stop_tick_coeffs"
    ...
    bus_prog_value_select: str = "zlc_bus_prog_value_select"
...
def generate_pulse_streamer_top_example(
    *,
    ...
    value_select_width = _edge_addr_width(num_slots + 1)
...
//   probe_out11 {probe_names.prog_tick_coeffs} width {num_slots * coeff_width}
//   probe_out12 {probe_names.scan_enable} width 1
//   probe_out13 {probe_names.scan_prog_we} width 1
//   probe_out14 {probe_names.scan_prog_addr} width {scan_addr_width}
//   probe_out15 {probe_names.scan_prog_values} width {num_slots * tick_width}
//   probe_out16 {probe_names.scan_count} width {scan_addr_width + 1}
//   probe_out17 {probe_names.loop_end_coeffs} width {num_slots * coeff_width}
//   probe_out18 {probe_names.bus_prog_we} width 1
//   probe_out19 {probe_names.bus_prog_bus} width {bus_index_width}
//   probe_out20 {probe_names.bus_prog_addr} width {bus_seg_addr_width}
//   probe_out21 {probe_names.bus_prog_start_tick} width {tick_width}
//   probe_out22 {probe_names.bus_prog_stop_tick} width {tick_width}
//   probe_out23 {probe_names.bus_prog_start_value} width {bus_width}
//   probe_out24 {probe_names.bus_prog_stop_value} width {bus_width}
//   probe_out25 {probe_names.bus_prog_mode} width 2
//   probe_out26 {probe_names.bus_counts} width {bus_counts_width}
//   probe_out27 {probe_names.bus_prog_value_select} width {value_select_width}
//   probe_out28 {probe_names.bus_prog_start_tick_coeffs} width {num_slots * coeff_width}
//   probe_out29 {probe_names.bus_prog_stop_tick_coeffs} width {num_slots * coeff_width}
...
{outputs}
    ...
    wire [{num_slots * coeff_width - 1}:0] {probe_names.loop_end_coeffs};
    wire [31:0] {probe_names.loop_count};
    wire [{num_slots * coeff_width - 1}:0] {probe_names.prog_tick_coeffs};
    wire {probe_names.scan_enable};
    wire {probe_names.scan_prog_we};
    wire [{scan_addr_width - 1}:0] {probe_names.scan_prog_addr};
    wire [{num_slots * tick_width - 1}:0] {probe_names.scan_prog_values};
    ...
    wire [{value_select_width - 1}:0] {probe_names.bus_prog_value_select};
    wire [{num_slots * coeff_width - 1}:0] {probe_names.bus_prog_start_tick_coeffs};
    wire [{num_slots * coeff_width - 1}:0] {probe_names.bus_prog_stop_tick_coeffs};
...
{assigns}
    ...
    {core_module_name} #(
        ...
        .prog_tick_coeffs({probe_names.prog_tick_coeffs}),
        ...
        .loop_end_coeffs({probe_names.loop_end_coeffs}),
        ...
        .scan_prog_values({probe_names.scan_prog_values}),
        ...
        .bus_prog_start_tick_coeffs({probe_names.bus_prog_start_tick_coeffs}),
        .bus_prog_stop_tick_coeffs({probe_names.bus_prog_stop_tick_coeffs}),
        ...
        .bus_prog_value_select({probe_names.bus_prog_value_select}),
    ...
    vio_0 zlc_vio_i (
        ...
        .probe_out11({probe_names.prog_tick_coeffs}),
        .probe_out12({probe_names.scan_enable}),
        .probe_out13({probe_names.scan_prog_we}),
        .probe_out14({probe_names.scan_prog_addr}),
        .probe_out15({probe_names.scan_prog_values}),
        .probe_out16({probe_names.scan_count}),
        .probe_out17({probe_names.loop_end_coeffs}),
        .probe_out18({probe_names.bus_prog_we}),
        .probe_out19({probe_names.bus_prog_bus}),
        .probe_out20({probe_names.bus_prog_addr}),
        .probe_out21({probe_names.bus_prog_start_tick}),
        .probe_out22({probe_names.bus_prog_stop_tick}),
        .probe_out23({probe_names.bus_prog_start_value}),
        .probe_out24({probe_names.bus_prog_stop_value}),
        .probe_out25({probe_names.bus_prog_mode}),
        .probe_out26({probe_names.bus_counts}),
        .probe_out27({probe_names.bus_prog_value_select}),
        .probe_out28({probe_names.bus_prog_start_tick_coeffs}),
        .probe_out29({probe_names.bus_prog_stop_tick_coeffs})
    );
endmodule
"""
...
def main(argv=None) -> int:
    ...
    if action == "capacity_estimate":
        print(capacity_estimate_text(
            ...
            max_scan_points=args.max_scan_points or _env_int("ZLC_PS_MAX_SCAN_POINTS", DEFAULT_MAX_SCAN_POINTS),
            tick_width=args.tick_width or _env_int("ZLC_PS_TICK_WIDTH", DEFAULT_TICK_WIDTH),
            num_slots=args.num_slots or _env_int("ZLC_PS_NUM_SLOTS", DEFAULT_NUM_SLOTS),
...
def _vivado_common_tcl(
    *,
    ...
    return [
        ...
        f"set zlc_prog_tick_coeffs_probe {{{probe_names.prog_tick_coeffs} probe_out11}}",
        f"set zlc_scan_enable_probe {{{probe_names.scan_enable} probe_out12}}",
        f"set zlc_scan_prog_we_probe {{{probe_names.scan_prog_we} probe_out13}}",
        f"set zlc_scan_prog_addr_probe {{{probe_names.scan_prog_addr} probe_out14}}",
        f"set zlc_scan_prog_values_probe {{{probe_names.scan_prog_values} probe_out15}}",
        f"set zlc_scan_count_probe {{{probe_names.scan_count} probe_out16}}",
        f"set zlc_loop_end_coeffs_probe {{{probe_names.loop_end_coeffs} probe_out17}}",
        f"set zlc_bus_prog_we_probe {{{probe_names.bus_prog_we} probe_out18}}",
        f"set zlc_bus_prog_bus_probe {{{probe_names.bus_prog_bus} probe_out19}}",
        f"set zlc_bus_prog_addr_probe {{{probe_names.bus_prog_addr} probe_out20}}",
        f"set zlc_bus_prog_start_tick_probe {{{probe_names.bus_prog_start_tick} probe_out21}}",
        f"set zlc_bus_prog_stop_tick_probe {{{probe_names.bus_prog_stop_tick} probe_out22}}",
        f"set zlc_bus_prog_start_value_probe {{{probe_names.bus_prog_start_value} probe_out23}}",
        f"set zlc_bus_prog_stop_value_probe {{{probe_names.bus_prog_stop_value} probe_out24}}",
        f"set zlc_bus_prog_mode_probe {{{probe_names.bus_prog_mode} probe_out25}}",
        f"set zlc_bus_counts_probe {{{probe_names.bus_counts} probe_out26}}",
        f"set zlc_bus_prog_value_select_probe {{{probe_names.bus_prog_value_select} probe_out27}}",
        f"set zlc_bus_prog_start_tick_coeffs_probe {{{probe_names.bus_prog_start_tick_coeffs} probe_out28}}",
        f"set zlc_bus_prog_stop_tick_coeffs_probe {{{probe_names.bus_prog_stop_tick_coeffs} probe_out29}}",
...
def _prepare_edge_indices(program: RuntimeSequenceProgram, previous_program: RuntimeSequenceProgram | None = None) -> list[int]:
    ...
    if float(previous_program.clock_hz) != float(program.clock_hz):
        return full
    previous_coeffs = list(previous_program.tick_slot_coeffs or [[] for _ in previous_program.ticks])
    current_coeffs = list(program.tick_slot_coeffs or [[] for _ in program.ticks])
    changed = {
        index
        ...
        or index >= len(previous_coeffs)
        or index >= len(current_coeffs)
        or int(previous_program.ticks[index]) != int(tick)
        or int(previous_program.masks[index]) != int(mask)
        or [int(c) for c in previous_coeffs[index]] != [int(c) for c in current_coeffs[index]]
    }
...
def _prepare_scan_indices(program: RuntimeSequenceProgram, previous_program: RuntimeSequenceProgram | None = None) -> list[int]:
    ...
    return [
        ...
        or [int(v) for v in previous_points[index]] != [int(v) for v in point]
    ]
...
def _prepare_tcl(
    ...
    *,
    ...
    tick_slot_coeffs = list(program.tick_slot_coeffs or [[] for _ in program.ticks])
    ...
    lines.extend(
        ...
        f"lappend zlc_batch [zlc_stage_probe $vio $zlc_loop_end_coeffs_probe {_pack_signed(program.loop_end_slot_coeffs or [], DEFAULT_SCAN_COEFF_WIDTH, DEFAULT_NUM_SLOTS)}]",
    ...
    if first_write_index is not None:
        ...
        coeffs_word = _pack_signed(tick_slot_coeffs[first_write_index], DEFAULT_SCAN_COEFF_WIDTH, DEFAULT_NUM_SLOTS)
        lines.extend(
            [
                ...
                f"lappend zlc_batch [zlc_stage_probe $vio $zlc_prog_tick_coeffs_probe {coeffs_word}]",
    ...
    for index in remaining_write_indices:
        ...
        coeffs_word = _pack_signed(tick_slot_coeffs[index], DEFAULT_SCAN_COEFF_WIDTH, DEFAULT_NUM_SLOTS)
        lines.extend(
            [
                "set zlc_prog_we_toggle_value [expr {$zlc_prog_we_toggle_value ? 0 : 1}]",
                "set zlc_batch {}",
    ...
    for bus_index, bus_addr, segment in bus_rows:
        ...
        start_coeffs_word = _pack_signed(start_coeffs, DEFAULT_SCAN_COEFF_WIDTH, DEFAULT_NUM_SLOTS)
        stop_coeffs_word = _pack_signed(stop_coeffs, DEFAULT_SCAN_COEFF_WIDTH, DEFAULT_NUM_SLOTS)
        lines.extend(
            [
                ...
                f"lappend zlc_batch [zlc_stage_probe $vio $zlc_bus_prog_start_tick_coeffs_probe {start_coeffs_word}]",
                f"lappend zlc_batch [zlc_stage_probe $vio $zlc_bus_prog_stop_tick_coeffs_probe {stop_coeffs_word}]",
                ...
                f"lappend zlc_batch [zlc_stage_probe $vio $zlc_bus_prog_value_select_probe {value_select}]",
                "lappend zlc_batch [zlc_stage_probe $vio $zlc_bus_prog_we_probe $zlc_bus_prog_we_toggle_value]",
                "zlc_commit_probes $zlc_batch",
            ]
        )
    for index in scan_indices:
        values_word = _pack_signed(scan_points[index], DEFAULT_TICK_WIDTH, DEFAULT_NUM_SLOTS)
        lines.extend(
            [
                ...
                f"lappend zlc_batch [zlc_stage_probe $vio $zlc_scan_prog_values_probe {values_word}]",
...
def _pack_signed(values, width: int, count: int) -> int:
    """Pack ``count`` signed values little-endian into one unsigned integer."""

    word = 0
    for index in range(count):
        value = int(values[index]) if index < len(values) else 0
        word |= _signed_to_unsigned(value, width) << (index * width)
    return word
