...
def _compact_retained_upper_bound(
    ...
    axis_bytes = 0
    for axis in axes:
        axis_bytes += 512 + len(axis.name.encode("utf-8"))
        if axis.unit is not None:
            axis_bytes += len(axis.unit.encode("utf-8"))
    ...
    layout_bytes = layout.storage_size * (256 + 64 * len(axes))
    return positive_integer(
        16 * 1024 + axis_bytes + layout_bytes,
...
class FitGridModel:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            ...
            _compact_retained_upper_bound(axes, self.layout),
        )
    ...
    def summary(self) -> str:
        ...
        batch_axes = ", ".join(
            f"{axis.name} ({axis.role.value}, {axis.size})" for axis in self.axes
        ) or "one scalar batch"
        statuses = ", ".join(
            f"{status.value.lower()}={count}"
    ...
    def page(self, address: tuple[int, ...] | None = None) -> FitGridPage:
        ...
        for axis, span, page_index in zip(
            ...
            bounds.append(f"{axis.name}[{start}:{stop}]")
