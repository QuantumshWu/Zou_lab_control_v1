...
# Register the Qt guard the moment this module is imported: from then on the env-var / fixture arms it.
# The registration point is ``zlc_data.curve_fitting``, which is where ``register_solve_thread_guard``
# actually lives -- the legacy ``neutral_atom.core.fitting`` path this used to name is itself a
# forwarding shim to it, and naming a shim as the owner is how a second apparent source is born.
from zlc_data.curve_fitting import register_solve_thread_guard as _register_guard
