"""The live plot FIGURE object and its npz save / load / replay.

NAME WARNING: this is NOT ``zlc_frontend.data_figure``.  That module is the
static notebook facade over one FROZEN frontend evaluation; this one is the
legacy mutable figure a plot hands back - the thing you fit on, save, reopen and
replay.  Two different concepts wore the same file name in the two trees, so the
moved copy is named for what it holds instead: the plot figure.

Salvaged verbatim from ``Zou_lab_control/frontend/data_figure.py``; the only
edits are the import repoints its new home requires.
"""
...
from zlc_data.curve_fitting import (
    FitRequest,
    FitResult,
    fit_data,
    fit_image,
