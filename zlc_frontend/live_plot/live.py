...
class BaseLivePlot:
    ...
    def __init__(
        ...
        *,
        ...
        self.repeat_cur = 1
        self.repeat_label = 1
        self.update_time = positive_float(update_time, "update_time")
    ...
    def _draw_fit_overlay(self, result) -> None:
        ...
        from .plot_figure import _CENTER_DOT_SIZE, _CENTER_RING_LW, _CENTER_RING_ALPHA
    ...
    def to_data_figure(self):
        from .plot_figure import DataFigure
...
def coerce_panel_value(kind, value, *, structure=None, params=None, repeat_mode="last"):
    ...
    if kind == "1d":
        ...
        if points == 1 and not params.get("xy") and sum(int(n) > 1 for n in ds) <= 1:
            # A SINGLE point whose data blob IS a 1-D sequence -- the external-datum framing a raw curve
            ...
            # ``external`` branch above applies.  Gate on the blob being GENUINELY 1-D (at most one axis
            # longer than 1): a real 2-D frame (``data_shape=(H,W)``, two axes > 1) is NOT a curve and
            # must fall through to the multidimensional-rejection guard below, not be silently flattened.
...
class _GridData:
    ...
    def save(self, path: str = "", *, extra_info=None, image_ext: str = "png", **kwargs):
        ...
        import time
        from .plot_figure import _write_saved_npz, resolve_save_base
...
class GridPlot(BaseLivePlot):
    ...
    def __init__(self, cell: GridCell, *, grid_shape: tuple[int, int] | None = None,
        ...
        # #6b CONSOLE display mode: the per-panel FitProcessor node solves every cell on its worker and
        # publishes per-cell params; the console pushes them here (``apply_published_cell_fits``).  When
        # this is a dict, ``_refit_axes`` RECONSTRUCTS each cell's curve from its published popt via a
        # ``solve=False`` request (NO curve_fit on any thread) instead of an in-place solve; ``None`` is
        # the NOTEBOOK path (no hub/console), which keeps solving in place.
        self._published_cell_popt: dict | None = None
        self._published_fit_model: str | None = None
    ...
    def _refit_axes(self, ax, df, request, *, key, is_display: bool = False) -> tuple:
        ...
        # #6b CONSOLE display mode: reconstruct THIS cell's curve from the node's PUBLISHED params via a
        # ``solve=False`` request (draw a known curve, no solver on any thread) instead of solving in
        # place.  A cell with no published fit yet clears.  (Notebook path: _published_cell_popt is None
        # -> solve in place, below.)
        if self._published_cell_popt is not None:
            cell_index = self._focused if key == "focus" else key
            popt = self._published_cell_popt.get(int(cell_index)) if isinstance(cell_index, int) else None
            if request is None or popt is None:
                apply_fit_to_figure(df, None, is_display=is_display)
                return ()
            request = self._display_only_request(request, popt)
    ...
    @staticmethod
    def _display_only_request(request, popt):
    ...
    def apply_published_cell_fits(self, cell_popts, *, model: str | None = None) -> None:
        """CONSOLE display push (#6b): adopt the per-panel FitProcessor node's PUBLISHED per-cell params
        (``{cell_index: (p0, p1, ...)}`` in the model's parameter order) as this grid's display fits.  Puts
        the grid in DISPLAY-ONLY mode (``_refit_axes`` reconstructs from these via ``solve=False``, never an
        in-place solve), fills each cell's ``_general_popt_store`` so ``{popt}`` title templates keep
        working, then re-draws the fit + re-titles on THIS (the overlay-push) side."""
        self._published_cell_popt = {int(k): tuple(float(v) for v in vec) for k, vec in (cell_popts or {}).items()}
        self._published_fit_model = None if model is None else str(model)
        self.cell_renderer._general_popt_store = dict(self._published_cell_popt)
        self._apply_view_knobs()            # reconstruct every cell's curve from the published params
        self._retitle_cells()               # {popt} titles refresh on the overlay-push side (#6b)
        self.draw()
...
def build_grid_figure(recipe: Mapping[str, Any], *, fig: plt.Figure | None = None,
    ...
    from .plot_figure import _validate_recipe
...
def load(path, *, kind: str | None = None, display: bool = True):
    ...
    """
    from .plot_figure import load_figure
